...
def mock_subprocess(mocker):
    ...
    subprocess._output = {mocker.sentinel.stdout: SpecialBytesIO(b'', fh=mocker.sentinel.stdout), mocker.sentinel.stderr: SpecialBytesIO(b'', fh=mocker.sentinel.stderr)}
...
class TestRunCommandArgs:
    ARGS_DATA = ((['/bin/ls', 'a', 'b', 'c'], [b'/bin/ls', b'a', b'b', b'c'], b'/bin/ls a b c'), ('/bin/ls a " b" "c "', [b'/bin/ls', b'a', b' b', b'c '], b'/bin/ls a " b" "c "'))

    @pytest.mark.parametrize('cmd, expected, shell, stdin', ((arg, cmd_str if sh else cmd_lst, sh, {}) for ((arg, cmd_lst, cmd_str), sh) in product(ARGS_DATA, (True, False))), indirect=['stdin'])
    def test_args(self, cmd, expected, shell, rc_am):
        (cmd,  expected,  shell,  stdin) = ((arg, cmd_str if sh else cmd_lst, sh, {}) for ((arg, cmd_lst, cmd_str), sh) in product(ARGS_DATA, (True, False)))[0]
        rc_am = rc_am()
...
class TestRunCommandPrompt:
    ...
    def test_prompt_match_wo_data(self, mocker, rc_am):
        stdin = {}
        rc_am = rc_am()
        rc_am._subprocess._output = {mocker.sentinel.stdout: SpecialBytesIO(b'Authentication required!\nEnter password: ', fh=mocker.sentinel.stdout), mocker.sentinel.stderr: SpecialBytesIO(b'', fh=mocker.sentinel.stderr)}
        (rc, _, _) = rc_am.run_command('foo', prompt_regex='[pP]assword:', data=None)
        assert rc == 257
...
class TestRunCommandOutput:
    ...
    def test_ascii_stdout(self, mocker, rc_am):
        stdin = {}
        rc_am = rc_am()
        rc_am._subprocess._output = {mocker.sentinel.stdout: SpecialBytesIO(b'hello', fh=mocker.sentinel.stdout), mocker.sentinel.stderr: SpecialBytesIO(b'', fh=mocker.sentinel.stderr)}
    ...
    def test_utf8_output(self, mocker, rc_am):
        stdin = {}
        rc_am = rc_am()
        rc_am._subprocess._output = {mocker.sentinel.stdout: SpecialBytesIO(u'Žarn§'.encode('utf-8'), fh=mocker.sentinel.stdout), mocker.sentinel.stderr: SpecialBytesIO(u'لرئيسية'.encode('utf-8'), fh=mocker.sentinel.stderr)}
...
def test_OpenBytesIO_close():
    ret = OpenBytesIO().close()

def test_SpecialBytesIO_fileno():
    ret = SpecialBytesIO().fileno()

def test_SpecialBytesIO___eq__():
    ret = SpecialBytesIO().__eq__()

def test_MockSelector_register():
    ret = MockSelector().register()

def test_MockSelector_unregister():
    ret = MockSelector().unregister()

def test_MockSelector_select():
    ret = MockSelector().select()

def test_MockSelector_get_map():
    ret = MockSelector().get_map()

def test_MockSelector_close():
    ret = MockSelector().close()

def test_TestRunCommandArgs_test_args():
    ret = TestRunCommandArgs().test_args()

def test_TestRunCommandArgs_test_tuple_as_args():
    ret = TestRunCommandArgs().test_tuple_as_args()

def test_TestRunCommandCwd_test_cwd():
    ret = TestRunCommandCwd().test_cwd()

def test_TestRunCommandCwd_test_cwd_relative_path():
    ret = TestRunCommandCwd().test_cwd_relative_path()

def test_TestRunCommandCwd_test_cwd_not_a_dir():
    ret = TestRunCommandCwd().test_cwd_not_a_dir()

def test_TestRunCommandPrompt_test_prompt_bad_regex():
    ret = TestRunCommandPrompt().test_prompt_bad_regex()

def test_TestRunCommandPrompt_test_prompt_no_match():
    ret = TestRunCommandPrompt().test_prompt_no_match()

def test_TestRunCommandPrompt_test_prompt_match_wo_data():
    ret = TestRunCommandPrompt().test_prompt_match_wo_data()

def test_TestRunCommandRc_test_check_rc_false():
    ret = TestRunCommandRc().test_check_rc_false()

def test_TestRunCommandRc_test_check_rc_true():
    ret = TestRunCommandRc().test_check_rc_true()

def test_TestRunCommandOutput_test_text_stdin():
    ret = TestRunCommandOutput().test_text_stdin()

def test_TestRunCommandOutput_test_ascii_stdout():
    ret = TestRunCommandOutput().test_ascii_stdout()

def test_TestRunCommandOutput_test_utf8_output():
    ret = TestRunCommandOutput().test_utf8_output()