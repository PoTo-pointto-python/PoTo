...
from ..io import make_dirs, read_json_file
from ..util_common import ResultType, write_text_test_results
from ..env import get_ansible_version
from ..data import data_context
from .combine import command_coverage_combine
from . import run_coverage, CoverageConfig
...
def _add_cobertura_package(packages, package_name, package_data):
    ...
    for (path, results) in package_data.items():
        ...
        elem_class.attrib.update({'branch-rate': '0', 'complexity': '0', 'filename': filename, 'line-rate': str(round(lines_hit / line_count, 4)) if line_count else '0', 'name': class_name})
        ...
        for (number, hits) in results.items():
            ...
            elem_line.attrib.update(hits=str(hits), number=str(number))
    elem_package.attrib.update({'branch-rate': '0', 'complexity': '0', 'line-rate': str(round(total_lines_hit / total_line_count, 4)) if total_line_count else '0', 'name': package_name})
