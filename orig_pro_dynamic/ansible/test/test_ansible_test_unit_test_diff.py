...
import subprocess
import pytest
from ansible_test._internal.util import to_text, to_bytes
from ansible_test._internal.diff import parse_diff, FileDiff
...
def get_parsed_diff(base, head=None):
    ...
    for item in items:
        ...
        for line_range in item.new.ranges:
            assert line_range[1] >= line_range[0] > 0
    return items
RANGES_TO_TEST = (('f31421576b00f0b167cdbe61217c31c21a41ac02', 'HEAD'), ('b8125ac1a61f2c7d1de821c78c884560071895f1', '32146acf4e43e6f95f54d9179bf01f0df9814217'))

@pytest.mark.parametrize('base, head', RANGES_TO_TEST)
def test_parse_diff(base, head):
    (base,  head) = RANGES_TO_TEST[0]
