...
class AzureRMModuleBase(object):
    ...
    def __init__(self, derived_arg_spec, bypass_checks=False, no_log=False, mutually_exclusive=None, required_together=None, required_one_of=None, add_file_common_args=False, supports_check_mode=False, required_if=None, supports_tags=True, facts_module=False, skip_exec=False):
        ...
        if not HAS_MSRESTAZURE:
            self.fail(msg=missing_required_lib('msrestazure'), exception=HAS_MSRESTAZURE_EXC)
        if not HAS_AZURE:
    ...
    def storage_client(self):
        ...
        if not self._storage_client:
            self._storage_client = self.get_mgmt_svc_client(StorageManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2018-07-01')
    ...
    def network_client(self):
        ...
        if not self._network_client:
            self._network_client = self.get_mgmt_svc_client(NetworkManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2019-06-01')
    ...
    def compute_client(self):
        ...
        if not self._compute_client:
            self._compute_client = self.get_mgmt_svc_client(ComputeManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2019-07-01')
    ...
    def dns_client(self):
        ...
        if not self._dns_client:
            self._dns_client = self.get_mgmt_svc_client(DnsManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2018-05-01')
    ...
    def web_client(self):
        ...
        if not self._web_client:
            self._web_client = self.get_mgmt_svc_client(WebSiteManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2018-02-01')
    ...
    def containerservice_client(self):
        ...
        if not self._containerservice_client:
            self._containerservice_client = self.get_mgmt_svc_client(ContainerServiceClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2017-07-01')
    ...
    def managedcluster_client(self):
        ...
        if not self._managedcluster_client:
            self._managedcluster_client = self.get_mgmt_svc_client(ContainerServiceClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2018-03-31')
    ...
    def sql_client(self):
        ...
        if not self._sql_client:
            self._sql_client = self.get_mgmt_svc_client(SqlManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def postgresql_client(self):
        ...
        if not self._postgresql_client:
            self._postgresql_client = self.get_mgmt_svc_client(PostgreSQLManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def mysql_client(self):
        ...
        if not self._mysql_client:
            self._mysql_client = self.get_mgmt_svc_client(MySQLManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def mariadb_client(self):
        ...
        if not self._mariadb_client:
            self._mariadb_client = self.get_mgmt_svc_client(MariaDBManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def containerregistry_client(self):
        ...
        if not self._containerregistry_client:
            self._containerregistry_client = self.get_mgmt_svc_client(ContainerRegistryManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2017-10-01')
    ...
    def containerinstance_client(self):
        ...
        if not self._containerinstance_client:
            self._containerinstance_client = self.get_mgmt_svc_client(ContainerInstanceManagementClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2018-06-01')
    ...
    def marketplace_client(self):
        ...
        if not self._marketplace_client:
            self._marketplace_client = self.get_mgmt_svc_client(MarketplaceOrderingAgreements, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def traffic_manager_management_client(self):
        ...
        if not self._traffic_manager_management_client:
            self._traffic_manager_management_client = self.get_mgmt_svc_client(TrafficManagerManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def monitor_client(self):
        ...
        if not self._monitor_client:
            self._monitor_client = self.get_mgmt_svc_client(MonitorManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def log_analytics_client(self):
        ...
        if not self._log_analytics_client:
            self._log_analytics_client = self.get_mgmt_svc_client(LogAnalyticsManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def servicebus_client(self):
        ...
        if not self._servicebus_client:
            self._servicebus_client = self.get_mgmt_svc_client(ServiceBusManagementClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def automation_client(self):
        ...
        if not self._automation_client:
            self._automation_client = self.get_mgmt_svc_client(AutomationClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def IoThub_client(self):
        ...
        if not self._IoThub_client:
            self._IoThub_client = self.get_mgmt_svc_client(IotHubClient, base_url=self._cloud_environment.endpoints.resource_manager)
    ...
    def lock_client(self):
        ...
        if not self._lock_client:
            self._lock_client = self.get_mgmt_svc_client(ManagementLockClient, base_url=self._cloud_environment.endpoints.resource_manager, api_version='2016-09-01')
...
class AzureRMAuth(object):
    ...
    def __init__(self, auth_source='auto', profile=None, subscription_id=None, client_id=None, secret=None, tenant=None, ad_user=None, password=None, cloud_environment='AzureCloud', cert_validation_mode='validate', api_profile='latest', adfs_authority_url=None, fail_impl=None, **kwargs):
        if fail_impl:
        ...
        self.credentials = self._get_credentials(dict(auth_source=auth_source, profile=profile, subscription_id=subscription_id, client_id=client_id, secret=secret, tenant=tenant, ad_user=ad_user, password=password, cloud_environment=cloud_environment, cert_validation_mode=cert_validation_mode, api_profile=api_profile, adfs_authority_url=adfs_authority_url))
        ...
        self._cert_validation_mode = cert_validation_mode or self.credentials.get('cert_validation_mode') or os.environ.get('AZURE_CERT_VALIDATION_MODE') or 'validate'
        ...
        if self.credentials.get('credentials') is not None:
            ...
            self.azure_credentials = self.acquire_token_with_username_password(self._adfs_authority_url, self._resource, self.credentials['ad_user'], self.credentials['password'], self.credentials['client_id'], self.credentials['tenant'])
...
def test_SDKProfile_default_api_version():
    ret = SDKProfile().default_api_version()

def test_AzureRMModuleBase_check_client_version():
    ret = AzureRMModuleBase().check_client_version()

def test_AzureRMModuleBase_exec_module():
    ret = AzureRMModuleBase().exec_module()

def test_AzureRMModuleBase_fail():
    ret = AzureRMModuleBase().fail()

def test_AzureRMModuleBase_deprecate():
    ret = AzureRMModuleBase().deprecate()

def test_AzureRMModuleBase_log():
    ret = AzureRMModuleBase().log()

def test_AzureRMModuleBase_validate_tags():
    ret = AzureRMModuleBase().validate_tags()

def test_AzureRMModuleBase_update_tags():
    ret = AzureRMModuleBase().update_tags()

def test_AzureRMModuleBase_has_tags():
    ret = AzureRMModuleBase().has_tags()

def test_AzureRMModuleBase_get_resource_group():
    ret = AzureRMModuleBase().get_resource_group()

def test_AzureRMModuleBase_parse_resource_to_dict():
    ret = AzureRMModuleBase().parse_resource_to_dict()

def test_AzureRMModuleBase_serialize_obj():
    ret = AzureRMModuleBase().serialize_obj()

def test_AzureRMModuleBase_get_poller_result():
    ret = AzureRMModuleBase().get_poller_result()

def test_AzureRMModuleBase_check_provisioning_state():
    ret = AzureRMModuleBase().check_provisioning_state()

def test_AzureRMModuleBase_get_blob_client():
    ret = AzureRMModuleBase().get_blob_client()

def test_AzureRMModuleBase_create_default_pip():
    ret = AzureRMModuleBase().create_default_pip()

def test_AzureRMModuleBase_create_default_securitygroup():
    ret = AzureRMModuleBase().create_default_securitygroup()

def test_AzureRMModuleBase__validation_ignore_callback():
    ret = AzureRMModuleBase()._validation_ignore_callback()

def test_AzureRMModuleBase_get_api_profile():
    ret = AzureRMModuleBase().get_api_profile()

def test_AzureRMModuleBase__ansible_get_models():
    ret = AzureRMModuleBase()._ansible_get_models()

def test_AzureRMModuleBase_get_mgmt_svc_client():
    ret = AzureRMModuleBase().get_mgmt_svc_client()

def test_AzureRMModuleBase_add_user_agent():
    ret = AzureRMModuleBase().add_user_agent()

def test_AzureRMModuleBase_generate_sas_token():
    ret = AzureRMModuleBase().generate_sas_token()

def test_AzureRMModuleBase_get_data_svc_client():
    ret = AzureRMModuleBase().get_data_svc_client()

def test_AzureRMModuleBase_credentials():
    ret = AzureRMModuleBase().credentials()

def test_AzureRMModuleBase__cloud_environment():
    ret = AzureRMModuleBase()._cloud_environment()

def test_AzureRMModuleBase_subscription_id():
    ret = AzureRMModuleBase().subscription_id()

def test_AzureRMModuleBase_storage_client():
    ret = AzureRMModuleBase().storage_client()

def test_AzureRMModuleBase_storage_models():
    ret = AzureRMModuleBase().storage_models()

def test_AzureRMModuleBase_network_client():
    ret = AzureRMModuleBase().network_client()

def test_AzureRMModuleBase_network_models():
    ret = AzureRMModuleBase().network_models()

def test_AzureRMModuleBase_rm_client():
    ret = AzureRMModuleBase().rm_client()

def test_AzureRMModuleBase_rm_models():
    ret = AzureRMModuleBase().rm_models()

def test_AzureRMModuleBase_compute_client():
    ret = AzureRMModuleBase().compute_client()

def test_AzureRMModuleBase_compute_models():
    ret = AzureRMModuleBase().compute_models()

def test_AzureRMModuleBase_dns_client():
    ret = AzureRMModuleBase().dns_client()

def test_AzureRMModuleBase_dns_models():
    ret = AzureRMModuleBase().dns_models()

def test_AzureRMModuleBase_web_client():
    ret = AzureRMModuleBase().web_client()

def test_AzureRMModuleBase_containerservice_client():
    ret = AzureRMModuleBase().containerservice_client()

def test_AzureRMModuleBase_managedcluster_models():
    ret = AzureRMModuleBase().managedcluster_models()

def test_AzureRMModuleBase_managedcluster_client():
    ret = AzureRMModuleBase().managedcluster_client()

def test_AzureRMModuleBase_sql_client():
    ret = AzureRMModuleBase().sql_client()

def test_AzureRMModuleBase_postgresql_client():
    ret = AzureRMModuleBase().postgresql_client()

def test_AzureRMModuleBase_mysql_client():
    ret = AzureRMModuleBase().mysql_client()

def test_AzureRMModuleBase_mariadb_client():
    ret = AzureRMModuleBase().mariadb_client()

def test_AzureRMModuleBase_sql_client():
    ret = AzureRMModuleBase().sql_client()

def test_AzureRMModuleBase_containerregistry_client():
    ret = AzureRMModuleBase().containerregistry_client()

def test_AzureRMModuleBase_containerinstance_client():
    ret = AzureRMModuleBase().containerinstance_client()

def test_AzureRMModuleBase_marketplace_client():
    ret = AzureRMModuleBase().marketplace_client()

def test_AzureRMModuleBase_traffic_manager_management_client():
    ret = AzureRMModuleBase().traffic_manager_management_client()

def test_AzureRMModuleBase_monitor_client():
    ret = AzureRMModuleBase().monitor_client()

def test_AzureRMModuleBase_log_analytics_client():
    ret = AzureRMModuleBase().log_analytics_client()

def test_AzureRMModuleBase_log_analytics_models():
    ret = AzureRMModuleBase().log_analytics_models()

def test_AzureRMModuleBase_servicebus_client():
    ret = AzureRMModuleBase().servicebus_client()

def test_AzureRMModuleBase_servicebus_models():
    ret = AzureRMModuleBase().servicebus_models()

def test_AzureRMModuleBase_automation_client():
    ret = AzureRMModuleBase().automation_client()

def test_AzureRMModuleBase_automation_models():
    ret = AzureRMModuleBase().automation_models()

def test_AzureRMModuleBase_IoThub_client():
    ret = AzureRMModuleBase().IoThub_client()

def test_AzureRMModuleBase_IoThub_models():
    ret = AzureRMModuleBase().IoThub_models()
...
def test_AzureRMModuleBase_lock_client():
    ret = AzureRMModuleBase().lock_client()

def test_AzureRMModuleBase_lock_models():
    ret = AzureRMModuleBase().lock_models()

def test_AzureSASAuthentication_signed_session():
    ret = AzureSASAuthentication().signed_session()

def test_AzureRMAuth_fail():
    ret = AzureRMAuth().fail()

def test_AzureRMAuth__default_fail_impl():
    ret = AzureRMAuth()._default_fail_impl()

def test_AzureRMAuth__get_profile():
    ret = AzureRMAuth()._get_profile()

def test_AzureRMAuth__get_msi_credentials():
    ret = AzureRMAuth()._get_msi_credentials()

def test_AzureRMAuth__get_azure_cli_credentials():
    ret = AzureRMAuth()._get_azure_cli_credentials()

def test_AzureRMAuth__get_env_credentials():
    ret = AzureRMAuth()._get_env_credentials()

def test_AzureRMAuth__get_credentials():
    ret = AzureRMAuth()._get_credentials()

def test_AzureRMAuth_acquire_token_with_username_password():
    ret = AzureRMAuth().acquire_token_with_username_password()

def test_AzureRMAuth_log():
    ret = AzureRMAuth().log()