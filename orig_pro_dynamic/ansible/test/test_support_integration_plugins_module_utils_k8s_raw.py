...
class KubernetesRawModule(KubernetesAnsibleModule):
    ...
    def condition_spec(self):
        return dict(type=dict(), status=dict(default=True, choices=[True, False, 'Unknown']), reason=dict())
    ...
    def __init__(self, k8s_kind=None, *args, **kwargs):
        self.client = None
        self.warnings = []
        mutually_exclusive = [('resource_definition', 'src'), ('merge_type', 'apply')]
        ...
        if self.append_hash:
            if not HAS_K8S_CONFIG_HASH:
                self.fail_json(msg=missing_required_lib('openshift >= 0.7.2', reason='for append_hash'), exception=K8S_CONFIG_HASH_IMP_ERR)
        ...
        if not resource_definition and (not src):
            implicit_definition = dict(kind=self.kind, apiVersion=self.api_version, metadata=dict(name=self.name))
            if self.namespace:
...
def test_KubernetesRawModule_validate_spec():
    ret = KubernetesRawModule().validate_spec()

def test_KubernetesRawModule_condition_spec():
    ret = KubernetesRawModule().condition_spec()

def test_KubernetesRawModule_argspec():
    ret = KubernetesRawModule().argspec()

def test_KubernetesRawModule_flatten_list_kind():
    ret = KubernetesRawModule().flatten_list_kind()

def test_KubernetesRawModule_execute_module():
    ret = KubernetesRawModule().execute_module()

def test_KubernetesRawModule__prepend_resource_info():
    ret = KubernetesRawModule()._prepend_resource_info()

def test_KubernetesRawModule_validate():
    ret = KubernetesRawModule().validate()

def test_KubernetesRawModule_set_defaults():
    ret = KubernetesRawModule().set_defaults()

def test_KubernetesRawModule_perform_action():
    ret = KubernetesRawModule().perform_action()

def test_KubernetesRawModule_patch_resource():
    ret = KubernetesRawModule().patch_resource()

def test_KubernetesRawModule_create_project_request():
    ret = KubernetesRawModule().create_project_request()

def test_KubernetesRawModule__wait_for_elapsed():
    ret = KubernetesRawModule()._wait_for_elapsed()

def test_KubernetesRawModule__wait_for():
    ret = KubernetesRawModule()._wait_for()

def test_KubernetesRawModule__deployment_ready():
    ret = KubernetesRawModule()._deployment_ready()

def test_KubernetesRawModule__pod_ready():
    ret = KubernetesRawModule()._pod_ready()

def test_KubernetesRawModule__daemonset_ready():
    ret = KubernetesRawModule()._daemonset_ready()

def test_KubernetesRawModule__custom_condition():
    ret = KubernetesRawModule()._custom_condition()

def test_KubernetesRawModule__resource_absent():
    ret = KubernetesRawModule()._resource_absent()

def test_KubernetesRawModule_wait():
    ret = KubernetesRawModule().wait()