...
from ansible.module_utils.common.network import to_bits, is_netmask, is_masklen, to_netmask, to_masklen, to_subnet, to_ipv6_network, VALID_MASKS
try:
...
def transform_commands(module):
    transform = ComplexList(dict(command=dict(key=True), output=dict(), prompt=dict(type='list'), answer=dict(type='list'), newline=dict(type='bool', default=True), sendonly=dict(type='bool', default=False), check_all=dict(type='bool', default=False)), module)
...
class Template:
    ...
    def __init__(self):
        if not HAS_JINJA2:
            raise ImportError('jinja2 is required but does not appear to be installed.  It can be installed using `pip install jinja2`')
...
def test_Entity_serialize():
    ret = Entity().serialize()

def test_Entity_to_dict():
    ret = Entity().to_dict()

def test_Entity___call__():
    ret = Entity().__call__()

def test_EntityCollection___call__():
    ret = EntityCollection().__call__()

def test_Template___call__():
    ret = Template().__call__()

def test_Template_contains_vars():
    ret = Template().contains_vars()