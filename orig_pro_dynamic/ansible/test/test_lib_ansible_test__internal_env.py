...
from .io import write_json_file, read_json_file
from .util import display, find_executable, SubprocessError, ApplicationError, get_ansible_version, get_available_python_versions
from .util_common import data_context, write_json_test_results, ResultType
from .docker_util import docker_info, docker_version
from .thread import WrappedThread
from .constants import TIMEOUT_PATH
from .test import TestTimeout
from .executor import SUPPORTED_PYTHON_VERSIONS
...
def set_timeout(args):
    ...
    if args.explain:
        return
    if deadline:
        data = dict(duration=args.timeout, deadline=deadline)
...
def configure_test_timeout(args):
    ...
    if timeout_remaining <= datetime.timedelta():
        ...
        raise ApplicationError('The %d minute test timeout expired %s ago at %s.' % (timeout_duration, timeout_remaining * -1, timeout_deadline))
    display.info('The %d minute test timeout expires in %s at %s.' % (timeout_duration, timeout_remaining, timeout_deadline), verbosity=1)
...
def get_docker_details(args):
    ...
    docker_details = dict(executable=docker, info=info, version=version)
