...
from ansible.module_utils.postgres import connect_to_db, get_conn_params, PgMembership, postgres_common_argument_spec
...
PRIV_TO_AUTHID_COLUMN = dict(SUPERUSER='rolsuper', CREATEROLE='rolcreaterole', CREATEDB='rolcreatedb', INHERIT='rolinherit', LOGIN='rolcanlogin', REPLICATION='rolreplication', BYPASSRLS='rolbypassrls')
executed_queries = []
...
def grant_database_privileges(cursor, user, db, privs):
    privs = ', '.join(privs)
    if user == 'PUBLIC':
        query = 'GRANT %s ON DATABASE %s TO PUBLIC' % (privs, pg_quote_identifier(db, 'database'))
    else:
        query = 'GRANT %s ON DATABASE %s TO "%s"' % (privs, pg_quote_identifier(db, 'database'), user)
    executed_queries.append(query)
    cursor.execute(query)
...
def get_comment(cursor, user):
    """Get user's comment."""
    query = "SELECT pg_catalog.shobj_description(r.oid, 'pg_authid') FROM pg_catalog.pg_roles r WHERE r.rolname = %(user)s"
    cursor.execute(query, {'user': user})
    ...
    if state == 'present':
        ...
        if comment is not None:
            try:
                ...
                module.fail_json(msg='Unable to add comment on role: %s' % to_native(e), exception=traceback.format_exc())
