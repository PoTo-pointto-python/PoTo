            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    if attempt == retries or total_pause >= max_pause:
                        ...
                        msg = u'network_cli_retry: attempt: %d, caught exception(%s), pausing for %d seconds' % (attempt + 1, to_text(e, errors='surrogate_or_strict'), pause)
...
def test_Connection_paramiko_conn():
    ret = Connection().paramiko_conn()

def test_Connection__get_log_channel():
    ret = Connection()._get_log_channel()

def test_Connection_get_prompt():
    ret = Connection().get_prompt()
...
def test_Connection_set_check_prompt():
    ret = Connection().set_check_prompt()

def test_Connection_update_cli_prompt_context():
    ret = Connection().update_cli_prompt_context()
...
def test_Connection__handle_buffer_read_timeout():
    ret = Connection()._handle_buffer_read_timeout()

def test_Connection__handle_command_timeout():
    ret = Connection()._handle_command_timeout()

def test_Connection__strip():
    ret = Connection()._strip()

def test_Connection__handle_prompt():
    ret = Connection()._handle_prompt()

def test_Connection__sanitize():
    ret = Connection()._sanitize()

def test_Connection__find_prompt():
    ret = Connection()._find_prompt()

def test_Connection__validate_timeout_value():
    ret = Connection()._validate_timeout_value()

def test_Connection_transport_test():
    ret = Connection().transport_test()

def test_Connection__get_terminal_std_re():
    ret = Connection()._get_terminal_std_re()