...
def ensure_routes(connection=None, module=None, route_table=None, route_specs=None, propagating_vgw_ids=None, check_mode=None, purge_routes=None):
    ...
    for route_spec in route_specs:
        ...
        if match is None:
            if route_spec.get('DestinationCidrBlock'):
                ...
                module.warn('Skipping creating {0} because it has no destination cidr block. To add VPC endpoints to route tables use the ec2_vpc_endpoint module.'.format(route_spec))
        else:
            if match[0] == 'replace':
                if route_spec.get('DestinationCidrBlock'):
    ...
    if purge_routes:
        for r in routes_to_match:
            if not r.get('DestinationCidrBlock'):
                module.warn('Skipping purging route {0} because it has no destination cidr block. To remove VPC endpoints from route tables use the ec2_vpc_endpoint module.'.format(r))
                continue
...
def ensure_subnet_association(connection=None, module=None, vpc_id=None, route_table_id=None, subnet_id=None, check_mode=None):
...
def ensure_subnet_associations(connection=None, module=None, route_table=None, subnets=None, check_mode=None, purge_subnets=None):
    current_association_ids = [a['RouteTableAssociationId'] for a in route_table['Associations'] if not a['Main']]
    new_association_ids = []
    changed = False
    for subnet in subnets:
        result = ensure_subnet_association(connection=connection, module=module, vpc_id=route_table['VpcId'], route_table_id=route_table['RouteTableId'], subnet_id=subnet['SubnetId'], check_mode=check_mode)
...
def ensure_propagation(connection=None, module=None, route_table=None, propagating_vgw_ids=None, check_mode=None):
    changed = False
    ...
    if to_add:
        ...
        if not check_mode:
            for vgw_id in to_add:
                try:
                    connection.enable_vgw_route_propagation(RouteTableId=route_table['RouteTableId'], GatewayId=vgw_id)
                except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
...
def ensure_route_table_absent(connection, module):
    ...
    if not module.check_mode:
        ensure_subnet_associations(connection=connection, module=module, route_table=route_table, subnets=[], check_mode=False, purge_subnets=purge_subnets)
        try:
