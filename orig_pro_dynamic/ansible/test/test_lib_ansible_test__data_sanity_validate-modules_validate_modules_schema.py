...
author_line = re.compile('^\\w.*(\\(@([\\w-]+)\\)|!UNKNOWN)(?![\\w.])|^Ansible Core Team$|^Michael DeHaan$|^OpenStack Ansible SIG$')
...
def collection_name(v, error_code=None):
    if not isinstance(v, string_types):
        raise _add_ansible_error_code(Invalid('Collection name must be a string'), error_code or 'collection-invalid-name')
    m = COLLECTION_NAME_RE.match(v)
    if not m:
        raise _add_ansible_error_code(Invalid('Collection name must be of format `<namespace>.<name>`'), error_code or 'collection-invalid-name')
    return v
...
seealso_schema = Schema([Any({Required('module'): Any(*string_types), 'description': Any(*string_types)}, {Required('ref'): Any(*string_types), Required('description'): Any(*string_types)}, {Required('name'): Any(*string_types), Required('link'): Any(*string_types), Required('description'): Any(*string_types)})])
argument_spec_types = ['bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str']
argument_spec_modifiers = {'mutually_exclusive': sequence_of_sequences(min=2), 'required_together': sequence_of_sequences(min=2), 'required_one_of': sequence_of_sequences(min=2), 'required_if': sequence_of_sequences(min=3, max=4), 'required_by': Schema({str: Any(list_string_types, tuple_string_types, *string_types)})}
...
def option_deprecation(v):
    if v.get('removed_in_version') or v.get('removed_at_date'):
        if v.get('removed_in_version') and v.get('removed_at_date'):
            raise _add_ansible_error_code(Invalid('Only one of removed_in_version and removed_at_date must be specified'), error_code='deprecation-either-date-or-version')
        if not v.get('removed_from_collection'):
            raise _add_ansible_error_code(Invalid('If removed_in_version or removed_at_date is specified, removed_from_collection must be specified as well'), error_code='deprecation-collection-missing')
        return
    if v.get('removed_from_collection'):
        raise Invalid('removed_from_collection cannot be specified without either removed_in_version or removed_at_date')

def argument_spec_schema(for_collection):
    any_string_types = Any(*string_types)
    schema = {any_string_types: {'type': Any(is_callable, *argument_spec_types), 'elements': Any(*argument_spec_types), 'default': object, 'fallback': Any((is_callable, list_string_types), [is_callable, list_string_types]), 'choices': Any([object], (object,)), 'required': bool, 'no_log': bool, 'aliases': Any(list_string_types, tuple(list_string_types)), 'apply_defaults': bool, 'removed_in_version': version(for_collection), 'removed_at_date': date(), 'removed_from_collection': collection_name, 'options': Self, 'deprecated_aliases': Any([Any({Required('name'): Any(*string_types), Required('date'): date(), Required('collection_name'): collection_name}, {Required('name'): Any(*string_types), Required('version'): version(for_collection), Required('collection_name'): collection_name})])}}
...
json_value = Schema(Any(None, int, float, [Self], *list(({str_type: Self} for str_type in string_types)) + list(string_types)))
        ...
        if isinstance(version_added, string_types):
            if v.get('version_added_collection') == 'ansible.builtin':
                ...
                try:
                    ...
                    raise _add_ansible_error_code(Invalid('version_added (%r) is not a valid ansible-base version: %s' % (version_added, exc)), error_code=error_code)
            else:
                try:
...
def list_dict_option_schema(for_collection):
    suboption_schema = Schema({Required('description'): Any(list_string_types, *string_types), 'required': bool, 'choices': list, 'aliases': Any(list_string_types), 'version_added': version(for_collection), 'version_added_collection': collection_name, 'default': json_value, 'type': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str'), 'elements': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str'), 'suboptions': Any(None, *list(({str_type: Self} for str_type in string_types)))}, extra=PREVENT_EXTRA)
    list_dict_suboption_schema = [{str_type: suboption_schema} for str_type in string_types]
    option_schema = Schema({Required('description'): Any(list_string_types, *string_types), 'required': bool, 'choices': list, 'aliases': Any(list_string_types), 'version_added': version(for_collection), 'version_added_collection': collection_name, 'default': json_value, 'suboptions': Any(None, *list_dict_suboption_schema), 'type': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str'), 'elements': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str')}, extra=PREVENT_EXTRA)
...
def return_schema(for_collection):
    return_contains_schema = Any(All(Schema({Required('description'): Any(list_string_types, *string_types), 'returned': Any(*string_types), Required('type'): Any('bool', 'complex', 'dict', 'float', 'int', 'list', 'str'), 'version_added': version(for_collection), 'version_added_collection': collection_name, 'sample': json_value, 'example': json_value, 'contains': Any(None, *list(({str_type: Self} for str_type in string_types))), 'elements': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str')}), Schema(return_contains), Schema(partial(version_added, error_code='option-invalid-version-added'))), Schema(type(None)))
    list_dict_return_contains_schema = [{str_type: return_contains_schema} for str_type in string_types]
    return Any(All(Schema({any_string_types: {Required('description'): Any(list_string_types, *string_types), Required('returned'): Any(*string_types), Required('type'): Any('bool', 'complex', 'dict', 'float', 'int', 'list', 'str'), 'version_added': version(for_collection), 'version_added_collection': collection_name, 'sample': json_value, 'example': json_value, 'contains': Any(None, *list_dict_return_contains_schema), 'elements': Any(None, 'bits', 'bool', 'bytes', 'dict', 'float', 'int', 'json', 'jsonarg', 'list', 'path', 'raw', 'sid', 'str')}}), Schema({any_string_types: return_contains}), Schema({any_string_types: partial(version_added, error_code='option-invalid-version-added')})), Schema(type(None)))

def deprecation_schema(for_collection):
    main_fields = {Required('why'): Any(*string_types), Required('alternative'): Any(*string_types), Required('removed_from_collection'): collection_name, 'removed': Any(True)}
    ...
    if for_collection:
        ...
        version_schema = {Required('removed_in'): Any('2.2', '2.3', '2.4', '2.5', '2.6', '2.8', '2.9', '2.10', '2.11', '2.12', '2.13', '2.14')}
    version_schema.update(main_fields)
    return Any(Schema(version_schema, extra=PREVENT_EXTRA), Schema(date_schema, extra=PREVENT_EXTRA))
