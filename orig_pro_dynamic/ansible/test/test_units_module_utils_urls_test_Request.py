...
import datetime
import os
from ansible.module_utils.urls import Request, open_url, urllib_request, HAS_SSLCONTEXT, cookiejar, RequestWithMethod, UnixHTTPHandler, UnixHTTPSConnection, httplib
...
def test_Request_fallback(urlopen_mock, install_opener_mock, mocker):
    urlopen_mock = urlopen_mock()
    install_opener_mock = install_opener_mock()
    cookies = cookiejar.CookieJar()
    request = Request(headers={'foo': 'bar'}, use_proxy=False, force=True, timeout=100, validate_certs=False, url_username='user', url_password='passwd', http_agent='ansible-tests', force_basic_auth=True, follow_redirects='all', client_cert='/tmp/client.pem', client_key='/tmp/client.key', cookies=cookies, unix_socket='/foo/bar/baz.sock', ca_path='/foo/bar/baz.pem')
    fallback_mock = mocker.spy(request, '_fallback')
    r = request.open('GET', 'https://ansible.com')
    calls = [call(None, False), call(None, True), call(None, 100), call(None, False), call(None, 'user'), call(None, 'passwd'), call(None, 'ansible-tests'), call(None, True), call(None, 'all'), call(None, '/tmp/client.pem'), call(None, '/tmp/client.key'), call(None, cookies), call(None, '/foo/bar/baz.sock'), call(None, '/foo/bar/baz.pem')]
    fallback_mock.assert_has_calls(calls)
    assert fallback_mock.call_count == 14
