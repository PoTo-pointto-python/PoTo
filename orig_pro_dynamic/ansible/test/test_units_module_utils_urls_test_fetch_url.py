...
def test_fetch_url_httperror(open_url_mock, fake_ansible_module):
    open_url_mock = open_url_mock()
    fake_ansible_module = fake_ansible_module()
    open_url_mock.side_effect = urllib_error.HTTPError('http://ansible.com/', 500, 'Internal Server Error', {'Content-Type': 'application/json'}, StringIO('TESTS'))
    (r, info) = fetch_url(fake_ansible_module, 'http://ansible.com/')
...
def test_FakeAnsibleModule_exit_json():
    ret = FakeAnsibleModule().exit_json()

def test_FakeAnsibleModule_fail_json():
    ret = FakeAnsibleModule().fail_json()