    ...
    def connect(self):
        try:
            if not self.connected:
                ...
                self.log('connected to %s:%s using %s' % (self.params['host'], self.params['port'], self.params['transport']))
...
def test_Config_save_config():
    ret = Config().save_config()

def test_NetworkModule_cli():
    ret = NetworkModule().cli()

def test_NetworkModule_config():
    ret = NetworkModule().config()

def test_NetworkModule_connected():
    ret = NetworkModule().connected()

def test_NetworkModule__load_params():
    ret = NetworkModule()._load_params()

def test_NetworkModule_connect():
    ret = NetworkModule().connect()

def test_NetworkModule_disconnect():
    ret = NetworkModule().disconnect()

def test_LocalResourceConnection_get():
    ret = LocalResourceConnection().get()