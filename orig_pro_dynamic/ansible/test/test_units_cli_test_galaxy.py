...
@pytest.mark.parametrize('cli_args, expected', [(['ansible-galaxy', 'collection', 'init', 'abc.def'], 0), (['ansible-galaxy', 'collection', 'init', 'abc.def', '-vvv'], 3), (['ansible-galaxy', '-vv', 'collection', 'init', 'abc.def'], 2), (['ansible-galaxy', '-vv', 'collection', 'init', 'abc.def', '-v'], 1), (['ansible-galaxy', '-vv', 'collection', 'init', 'abc.def', '-vvvv'], 4), (['ansible-galaxy', '-vvv', 'init', 'name'], 3), (['ansible-galaxy', '-vvvvv', 'init', '-v', 'name'], 1)])
def test_verbosity_arguments(cli_args, expected, monkeypatch):
    (cli_args,  expected) = (['ansible-galaxy', 'collection', 'init', 'abc.def'], 0)
...
def test_collection_default(collection_skeleton):
    collection_skeleton = ('ansible_test.my_collection', None)
...
def test_collection_skeleton(collection_skeleton):
    collection_skeleton = ('ansible_test.delete_me_skeleton', os.path.join(os.path.split(__file__)[0], 'test_data', 'collection_skeleton'))
    collection_skeleton = collection_skeleton()
    meta_path = os.path.join(collection_skeleton, 'galaxy.yml')
    with open(meta_path, 'r') as galaxy_meta:
        metadata = yaml.safe_load(galaxy_meta)
...
def test_collection_build(collection_artifact):
    collection_skeleton = ('ansible_test.build_collection', None)
    ...
    with tarfile.open(tar_path, mode='r') as tar:
        ...
        valid_files = ['MANIFEST.json', 'FILES.json', 'roles', 'docs', 'plugins', 'plugins/README.md', 'README.md', 'runme.sh']
...
def test_collection_install_with_names(collection_install):
    ...
    assert mock_install.call_args[0][0] == [('namespace.collection', '*', None, None), ('namespace2.collection', '1.0.1', None, None)]
...
def test_collection_install_with_requirements_file(collection_install):
    ...
    assert "The specified collections path '%s' is not part of the configured Ansible collections path" % output_dir in mock_warning.call_args[0][0]
    assert mock_install.call_count == 1
    assert mock_install.call_args[0][0] == [('namespace.coll', '*', None, None), ('namespace2.coll', '>2.0.1', None, None)]
    assert mock_install.call_args[0][1] == collection_path
    assert len(mock_install.call_args[0][2]) == 1
    assert mock_install.call_args[0][2][0].api_server == 'https://galaxy.ansible.com'
    assert mock_install.call_args[0][2][0].validate_certs is True
...
def test_parse_requirements_file_that_doesnt_exist(requirements_cli, requirements_file):
    ...
    requirements_cli = requirements_cli()
...
def test_parse_requirements_file_that_isnt_yaml(requirements_cli, requirements_file):
    requirements_file = 'not a valid yml file: hi: world'
    requirements_cli = requirements_cli()
    requirements_file = requirements_file()
...
def test_parse_requirements(requirements_cli, requirements_file):
    ...
    requirements_cli = requirements_cli()
    requirements_file = requirements_file()
    expected = {'roles': [], 'collections': [('namespace.collection1', '*', None, None), ('namespace.collection2', '*', None, None)]}
...
def test_TestGalaxy_setUpClass():
    ret = TestGalaxy().setUpClass()

def test_TestGalaxy_makeTar():
    ret = TestGalaxy().makeTar()

def test_TestGalaxy_tearDownClass():
    ret = TestGalaxy().tearDownClass()

def test_TestGalaxy_setUp():
    ret = TestGalaxy().setUp()

def test_TestGalaxy_tearDown():
    ret = TestGalaxy().tearDown()

def test_TestGalaxy_test_init():
    ret = TestGalaxy().test_init()

def test_TestGalaxy_test_display_min():
    ret = TestGalaxy().test_display_min()

def test_TestGalaxy_test_display_galaxy_info():
    ret = TestGalaxy().test_display_galaxy_info()

def test_TestGalaxy_test_run():
    ret = TestGalaxy().test_run()

def test_TestGalaxy_test_execute_remove():
    ret = TestGalaxy().test_execute_remove()

def test_TestGalaxy_test_exit_without_ignore_without_flag():
    ret = TestGalaxy().test_exit_without_ignore_without_flag()

def test_TestGalaxy_test_exit_without_ignore_with_flag():
    ret = TestGalaxy().test_exit_without_ignore_with_flag()

def test_TestGalaxy_test_parse_no_action():
    ret = TestGalaxy().test_parse_no_action()

def test_TestGalaxy_test_parse_invalid_action():
    ret = TestGalaxy().test_parse_invalid_action()

def test_TestGalaxy_test_parse_delete():
    ret = TestGalaxy().test_parse_delete()

def test_TestGalaxy_test_parse_import():
    ret = TestGalaxy().test_parse_import()

def test_TestGalaxy_test_parse_info():
    ret = TestGalaxy().test_parse_info()

def test_TestGalaxy_test_parse_init():
    ret = TestGalaxy().test_parse_init()

def test_TestGalaxy_test_parse_install():
    ret = TestGalaxy().test_parse_install()

def test_TestGalaxy_test_parse_list():
    ret = TestGalaxy().test_parse_list()

def test_TestGalaxy_test_parse_login():
    ret = TestGalaxy().test_parse_login()

def test_TestGalaxy_test_parse_remove():
    ret = TestGalaxy().test_parse_remove()

def test_TestGalaxy_test_parse_search():
    ret = TestGalaxy().test_parse_search()

def test_TestGalaxy_test_parse_setup():
    ret = TestGalaxy().test_parse_setup()

def test_ValidRoleTests_setUpRole():
    ret = ValidRoleTests().setUpRole()

def test_ValidRoleTests_tearDownClass():
    ret = ValidRoleTests().tearDownClass()

def test_ValidRoleTests_test_metadata():
    ret = ValidRoleTests().test_metadata()

def test_ValidRoleTests_test_readme():
    ret = ValidRoleTests().test_readme()

def test_ValidRoleTests_test_main_ymls():
    ret = ValidRoleTests().test_main_ymls()

def test_ValidRoleTests_test_role_dirs():
    ret = ValidRoleTests().test_role_dirs()

def test_ValidRoleTests_test_travis_yml():
    ret = ValidRoleTests().test_travis_yml()

def test_ValidRoleTests_test_readme_contents():
    ret = ValidRoleTests().test_readme_contents()

def test_ValidRoleTests_test_test_yml():
    ret = ValidRoleTests().test_test_yml()

def test_TestGalaxyInitDefault_setUpClass():
    ret = TestGalaxyInitDefault().setUpClass()

def test_TestGalaxyInitDefault_test_metadata_contents():
    ret = TestGalaxyInitDefault().test_metadata_contents()

def test_TestGalaxyInitAPB_setUpClass():
    ret = TestGalaxyInitAPB().setUpClass()

def test_TestGalaxyInitAPB_test_metadata_apb_tag():
    ret = TestGalaxyInitAPB().test_metadata_apb_tag()

def test_TestGalaxyInitAPB_test_metadata_contents():
    ret = TestGalaxyInitAPB().test_metadata_contents()

def test_TestGalaxyInitAPB_test_apb_yml():
    ret = TestGalaxyInitAPB().test_apb_yml()

def test_TestGalaxyInitAPB_test_test_yml():
    ret = TestGalaxyInitAPB().test_test_yml()

def test_TestGalaxyInitContainer_setUpClass():
    ret = TestGalaxyInitContainer().setUpClass()

def test_TestGalaxyInitContainer_test_metadata_container_tag():
    ret = TestGalaxyInitContainer().test_metadata_container_tag()

def test_TestGalaxyInitContainer_test_metadata_contents():
    ret = TestGalaxyInitContainer().test_metadata_contents()

def test_TestGalaxyInitContainer_test_meta_container_yml():
    ret = TestGalaxyInitContainer().test_meta_container_yml()

def test_TestGalaxyInitContainer_test_test_yml():
    ret = TestGalaxyInitContainer().test_test_yml()

def test_TestGalaxyInitSkeleton_setUpClass():
    ret = TestGalaxyInitSkeleton().setUpClass()

def test_TestGalaxyInitSkeleton_test_empty_files_dir():
    ret = TestGalaxyInitSkeleton().test_empty_files_dir()

def test_TestGalaxyInitSkeleton_test_template_ignore_jinja():
    ret = TestGalaxyInitSkeleton().test_template_ignore_jinja()

def test_TestGalaxyInitSkeleton_test_template_ignore_jinja_subfolder():
    ret = TestGalaxyInitSkeleton().test_template_ignore_jinja_subfolder()

def test_TestGalaxyInitSkeleton_test_template_ignore_similar_folder():
    ret = TestGalaxyInitSkeleton().test_template_ignore_similar_folder()

def test_TestGalaxyInitSkeleton_test_skeleton_option():
    ret = TestGalaxyInitSkeleton().test_skeleton_option()