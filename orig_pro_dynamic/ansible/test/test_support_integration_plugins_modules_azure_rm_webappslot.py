...
try:
    ...
    pass
swap_spec = dict(action=dict(type='str', choices=['preview', 'swap', 'reset'], default='preview'), target_slot=dict(type='str'), preserve_vnet=dict(type='bool', default=True))
...
class AzureRMWebAppSlots(AzureRMModuleBase):
    ...
    def create_update_slot(self):
        ...
        try:
            response = self.web_client.web_apps.create_or_update_slot(resource_group_name=self.resource_group, slot=self.name, name=self.webapp_name, site_envelope=self.site)
    ...
    def delete_slot(self):
        ...
        try:
            response = self.web_client.web_apps.delete_slot(resource_group_name=self.resource_group, name=self.webapp_name, slot=self.name)
    ...
    def update_app_settings_slot(self, slot_name=None, app_settings=None):
        ...
        try:
            response = self.web_client.web_apps.update_application_settings_slot(resource_group_name=self.resource_group, name=self.webapp_name, slot=slot_name, kind=None, properties=app_settings)
            self.log('Response : {0}'.format(response))
    ...
    def create_or_update_source_control_slot(self):
        ...
        try:
            response = self.web_client.web_client.create_or_update_source_control_slot(resource_group_name=self.resource_group, name=self.webapp_name, site_source_control=self.deployment_source, slot=self.name)
            self.log('Response : {0}'.format(response))
            return response.as_dict()
        except CloudError as ex:
        ...
        try:
            if self.swap['action'] == 'swap':
                if self.swap['target_slot'] is None:
                    response = self.web_client.web_apps.swap_slot_with_production(resource_group_name=self.resource_group, name=self.webapp_name, target_slot=self.name, preserve_vnet=self.swap['preserve_vnet'])
                else:
                    response = self.web_client.web_apps.swap_slot_slot(resource_group_name=self.resource_group, name=self.webapp_name, slot=self.name, target_slot=self.swap['target_slot'], preserve_vnet=self.swap['preserve_vnet'])
            elif self.swap['action'] == 'preview':
                if self.swap['target_slot'] is None:
                    response = self.web_client.web_apps.apply_slot_config_to_production(resource_group_name=self.resource_group, name=self.webapp_name, target_slot=self.name, preserve_vnet=self.swap['preserve_vnet'])
                else:
                    response = self.web_client.web_apps.apply_slot_configuration_slot(resource_group_name=self.resource_group, name=self.webapp_name, slot=self.name, target_slot=self.swap['target_slot'], preserve_vnet=self.swap['preserve_vnet'])
...
def test_AzureRMWebAppSlots_exec_module():
    ret = AzureRMWebAppSlots().exec_module()

def test_AzureRMWebAppSlots_is_site_config_changed():
    ret = AzureRMWebAppSlots().is_site_config_changed()

def test_AzureRMWebAppSlots_is_app_settings_changed():
    ret = AzureRMWebAppSlots().is_app_settings_changed()

def test_AzureRMWebAppSlots_is_deployment_source_changed():
    ret = AzureRMWebAppSlots().is_deployment_source_changed()

def test_AzureRMWebAppSlots_create_update_slot():
    ret = AzureRMWebAppSlots().create_update_slot()

def test_AzureRMWebAppSlots_delete_slot():
    ret = AzureRMWebAppSlots().delete_slot()

def test_AzureRMWebAppSlots_get_webapp():
    ret = AzureRMWebAppSlots().get_webapp()

def test_AzureRMWebAppSlots_get_slot():
    ret = AzureRMWebAppSlots().get_slot()

def test_AzureRMWebAppSlots_list_app_settings():
    ret = AzureRMWebAppSlots().list_app_settings()

def test_AzureRMWebAppSlots_list_app_settings_slot():
    ret = AzureRMWebAppSlots().list_app_settings_slot()

def test_AzureRMWebAppSlots_update_app_settings_slot():
    ret = AzureRMWebAppSlots().update_app_settings_slot()

def test_AzureRMWebAppSlots_create_or_update_source_control_slot():
    ret = AzureRMWebAppSlots().create_or_update_source_control_slot()

def test_AzureRMWebAppSlots_get_configuration():
    ret = AzureRMWebAppSlots().get_configuration()

def test_AzureRMWebAppSlots_get_configuration_slot():
    ret = AzureRMWebAppSlots().get_configuration_slot()

def test_AzureRMWebAppSlots_update_configuration_slot():
    ret = AzureRMWebAppSlots().update_configuration_slot()

def test_AzureRMWebAppSlots_set_state_slot():
    ret = AzureRMWebAppSlots().set_state_slot()

def test_AzureRMWebAppSlots_swap_slot():
    ret = AzureRMWebAppSlots().swap_slot()

def test_AzureRMWebAppSlots_clone_slot():
    ret = AzureRMWebAppSlots().clone_slot()