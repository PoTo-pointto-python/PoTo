...
import tempfile
import time
from .util import SubprocessError, ApplicationError, cmd_quote, display, ANSIBLE_TEST_DATA_ROOT
from .util_common import intercept_command, get_network_settings, run_command
from .core_ci import AnsibleCoreCI
...
class ManageWindowsCI:
    ...
    def __init__(self, core_ci):
        ...
        ssh_options = dict(BatchMode='yes', StrictHostKeyChecking='no', UserKnownHostsFile='/dev/null', ServerAliveInterval=15, ServerAliveCountMax=4)
    ...
    def wait(self):
        ...
        extra_vars = ['ansible_connection=winrm', 'ansible_host=%s' % self.core_ci.connection.hostname, 'ansible_user=%s' % self.core_ci.connection.username, 'ansible_password=%s' % self.core_ci.connection.password, 'ansible_port=%s' % self.core_ci.connection.port, 'ansible_winrm_server_cert_validation=ignore']
        ...
        for dummy in range(1, 120):
            try:
                ...
                time.sleep(10)
        raise ApplicationError('Timeout waiting for %s/%s instance %s.' % (self.core_ci.platform, self.core_ci.version, self.core_ci.instance_id))
    ...
    def ssh(self, command, options=None, force_pty=True):
        ...
        run_command(self.core_ci.args, ['ssh', '-q'] + self.ssh_args + options + ['-p', '22', '%s@%s' % (self.core_ci.connection.username, self.core_ci.connection.hostname)] + [command])
    ...
    def scp(self, src, dst):
        ...
        for dummy in range(1, 10):
            try:
                run_command(self.core_ci.args, ['scp'] + self.ssh_args + ['-P', '22', '-q', '-r', src, dst])
                return
            except SubprocessError:
                time.sleep(10)
...
class ManageNetworkCI:
    ...
    def wait(self):
        ...
        extra_vars = ['ansible_host=%s' % self.core_ci.connection.hostname, 'ansible_port=%s' % self.core_ci.connection.port, 'ansible_ssh_private_key_file=%s' % self.core_ci.ssh_key.key] + ['%s=%s' % (key, value) for (key, value) in settings.inventory_vars.items()]
        name = '%s-%s' % (self.core_ci.platform, self.core_ci.version.replace('.', '-'))
        env = ansible_environment(self.core_ci.args)
        cmd = ['ansible', '-m', '%s%s_command' % (settings.collection + '.' if settings.collection else '', self.core_ci.platform), '-a', 'commands=?', '-u', self.core_ci.connection.username, '-i', '%s,' % name, '-e', ' '.join(extra_vars), name]
...
class ManagePosixCI:
    ...
    def upload_source(self):
        ...
        with tempfile.NamedTemporaryFile(prefix='ansible-source-', suffix='.tgz') as local_source_fd:
            ...
            self.ssh('rm -rf ~/ansible ~/ansible_collections && cd ~/ && gunzip --stdout %s | tar oxf - && rm %s' % (remote_source_path, remote_source_path))
    ...
    def ssh(self, command, options=None, capture=False):
        ...
        return run_command(self.core_ci.args, ['ssh', '-tt', '-q'] + self.ssh_args + options + ['-p', str(self.core_ci.connection.port), '%s@%s' % (self.core_ci.connection.username, self.core_ci.connection.hostname)] + self.become + [command], capture=capture)
...
def test_ManageWindowsCI_setup():
    ret = ManageWindowsCI().setup()

def test_ManageWindowsCI_wait():
    ret = ManageWindowsCI().wait()

def test_ManageWindowsCI_download():
    ret = ManageWindowsCI().download()

def test_ManageWindowsCI_upload():
    ret = ManageWindowsCI().upload()

def test_ManageWindowsCI_ssh():
    ret = ManageWindowsCI().ssh()

def test_ManageWindowsCI_scp():
    ret = ManageWindowsCI().scp()

def test_ManageNetworkCI_wait():
    ret = ManageNetworkCI().wait()

def test_ManagePosixCI_setup():
    ret = ManagePosixCI().setup()

def test_ManagePosixCI_wait():
    ret = ManagePosixCI().wait()

def test_ManagePosixCI_configure():
    ret = ManagePosixCI().configure()

def test_ManagePosixCI_upload_source():
    ret = ManagePosixCI().upload_source()

def test_ManagePosixCI_download():
    ret = ManagePosixCI().download()

def test_ManagePosixCI_upload():
    ret = ManagePosixCI().upload()

def test_ManagePosixCI_ssh():
    ret = ManagePosixCI().ssh()

def test_ManagePosixCI_scp():
    ret = ManagePosixCI().scp()