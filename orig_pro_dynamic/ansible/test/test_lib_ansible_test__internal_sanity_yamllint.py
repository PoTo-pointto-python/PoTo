...
from ..sanity import SanitySingleVersion, SanityMessage, SanityFailure, SanitySkipped, SanitySuccess, SANITY_ROOT
from ..target import TestTarget
from ..util import SubprocessError, display, is_subdir, find_python
...
class YamllintTest(SanitySingleVersion):
    ...
    def test(self, args, targets, python_version):
        ...
        if not pyyaml_presence['cloader']:
            display.warning("Skipping sanity test '%s' due to missing libyaml support in PyYAML." % self.name)
    ...
    def test_paths(args, paths, python):
        ...
        """
        cmd = [python, os.path.join(SANITY_ROOT, 'yamllint', 'yamllinter.py')]
        data = '\n'.join(paths)
...
def test_YamllintTest_error_code():
    ret = YamllintTest().error_code()

def test_YamllintTest_filter_targets():
    ret = YamllintTest().filter_targets()

def test_YamllintTest_test():
    ret = YamllintTest().test()

def test_YamllintTest_test_paths():
    ret = YamllintTest().test_paths()