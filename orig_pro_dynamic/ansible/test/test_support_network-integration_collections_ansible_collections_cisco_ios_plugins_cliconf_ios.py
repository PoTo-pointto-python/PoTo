...
class Cliconf(CliconfBase):
    ...
    def get_device_operations(self):
        return {'supports_diff_replace': True, 'supports_commit': False, 'supports_rollback': False, 'supports_defaults': True, 'supports_onbox_diff': False, 'supports_commit_comment': False, 'supports_multiline_delimiter': True, 'supports_diff_match': True, 'supports_diff_ignore_lines': True, 'supports_generate_diff': True, 'supports_replace': False}
    ...
    def set_cli_prompt_context(self):
        ...
        if self._connection.connected:
            ...
            if re.search('config.*\\)#', to_text(out, errors='surrogate_then_replace').strip()):
...
def test_Cliconf_get_config():
    ret = Cliconf().get_config()

def test_Cliconf_get_diff():
    ret = Cliconf().get_diff()

def test_Cliconf_edit_config():
    ret = Cliconf().edit_config()

def test_Cliconf_edit_macro():
    ret = Cliconf().edit_macro()

def test_Cliconf_get():
    ret = Cliconf().get()

def test_Cliconf_get_device_info():
    ret = Cliconf().get_device_info()

def test_Cliconf_get_device_operations():
    ret = Cliconf().get_device_operations()

def test_Cliconf_get_option_values():
    ret = Cliconf().get_option_values()

def test_Cliconf_get_capabilities():
    ret = Cliconf().get_capabilities()

def test_Cliconf_edit_banner():
    ret = Cliconf().edit_banner()

def test_Cliconf_run_commands():
    ret = Cliconf().run_commands()

def test_Cliconf_get_defaults_flag():
    ret = Cliconf().get_defaults_flag()

def test_Cliconf_set_cli_prompt_context():
    ret = Cliconf().set_cli_prompt_context()

def test_Cliconf__extract_banners():
    ret = Cliconf()._extract_banners()

def test_Cliconf__diff_banners():
    ret = Cliconf()._diff_banners()