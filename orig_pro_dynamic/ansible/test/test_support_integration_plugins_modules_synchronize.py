...
def main():
    ...
    if rsync_password:
        try:
            ...
            module.fail_json(msg='to use rsync connection with passwords, you must install the sshpass program')
    ...
    if rsync_opts:
        if '' in rsync_opts:
            module.warn('The empty string is present in rsync_opts which will cause rsync to transfer the current working directory. If this is intended, use "." instead to get rid of this warning. If this is unintended, check for problems in your playbook leading to empty string in rsync_opts.')
        ...
        def _write_password_to_pipe(proc):
            ...
            try:
                ...
                if exc.errno != errno.EPIPE or proc.poll() is None:
                    raise
        (rc, out, err) = module.run_command(cmd, pass_fds=_sshpass_pipe, before_communicate_callback=_write_password_to_pipe)
    else:
