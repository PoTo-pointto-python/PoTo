...
TEST_ITEMS_NON_SEQUENCES = ({}, object(), frozenset(), 4, 0.0) + TEST_STRINGS
TEST_ITEMS_SEQUENCES = ([], (), SeqStub())
TEST_ITEMS_SEQUENCES = TEST_ITEMS_SEQUENCES + (TEST_ITEMS_NON_SEQUENCES,)

@pytest.mark.parametrize('sequence_input', TEST_ITEMS_SEQUENCES)
def test_sequence_positive(sequence_input):
    sequence_input = TEST_ITEMS_SEQUENCES[0]
...
def test_sequence_negative(non_sequence_input):
    non_sequence_input = TEST_ITEMS_NON_SEQUENCES[0]
...
class TestImmutableDict:
    ...
    def test_immutable(self):
        imdict = ImmutableDict({1: 2})
        expected_reason = "^'ImmutableDict' object does not support item assignment$"
    ...
    def test_nonhashable(self):
        ...
        expected_reason = "^unhashable type: 'list'$"
        with pytest.raises(TypeError, match=expected_reason):
            hash(imdict)
...
def test_IteratorStub___next__():
    ret = IteratorStub().__next__()

def test_IterableStub___iter__():
    ret = IterableStub().__iter__()

def test_TestImmutableDict_test_scalar():
    ret = TestImmutableDict().test_scalar()

def test_TestImmutableDict_test_string():
    ret = TestImmutableDict().test_string()

def test_TestImmutableDict_test_container():
    ret = TestImmutableDict().test_container()

def test_TestImmutableDict_test_from_tuples():
    ret = TestImmutableDict().test_from_tuples()

def test_TestImmutableDict_test_from_kwargs():
    ret = TestImmutableDict().test_from_kwargs()

def test_TestImmutableDict_test_immutable():
    ret = TestImmutableDict().test_immutable()

def test_TestImmutableDict_test_hashable():
    ret = TestImmutableDict().test_hashable()

def test_TestImmutableDict_test_nonhashable():
    ret = TestImmutableDict().test_nonhashable()

def test_TestImmutableDict_test_len():
    ret = TestImmutableDict().test_len()

def test_TestImmutableDict_test_repr():
    ret = TestImmutableDict().test_repr()