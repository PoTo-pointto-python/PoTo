...
try:
    ...
    YAML_FOUND = True
valid_file_format = re.compile('.*(\\.)(yml|yaml|json)$')
...
def generate_docstring(operation_spec):
    ...
    for parameter in parameters:
        docs += '{0} ({1}:{2}): {3}\n'.format(parameter.get('name'), parameter.get('type', 'No Type'), 'Required' if parameter.get('required', False) else 'Not Required', parameter.get('description'))
    return docs
        ...
        for url in session._spec.get('paths').keys():
            ...
            for method in methods.keys():
                ...
                if not operation_name:
                    ...
                    operation_name += re.sub('{(.*)}', '', url).replace('/', ' ').title().replace(' ', '')
...
def test_RestOperation_restmethod():
    ret = RestOperation().restmethod()

def test_ECSSession_client():
    ret = ECSSession().client()

def test_ECSSession__set_config():
    ret = ECSSession()._set_config()

def test_ECSSession_get_config():
    ret = ECSSession().get_config()

def test_ECSSession__read_config_vars():
    ret = ECSSession()._read_config_vars()