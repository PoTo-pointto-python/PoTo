...
import os
import sys
from ..util import ANSIBLE_TEST_DATA_ROOT, display, get_available_python_versions, is_subdir, SubprocessError, REMOTE_ONLY_PYTHON_VERSIONS
from ..util_common import intercept_command, ResultType, handle_layout_messages
from ..ansible_util import ansible_environment, check_pyyaml
from ..target import walk_internal_targets, walk_units_targets
from ..config import UnitsConfig
from ..coverage_util import coverage_context
from ..data import data_context
from ..executor import AllTargetsSkipped, Delegate, get_changes_filter, install_command_requirements, SUPPORTED_PYTHON_VERSIONS
...
def command_units(args):
    ...
    remote_paths = [path for path in paths if is_subdir(path, data_context().content.unit_module_path) or is_subdir(path, data_context().content.unit_module_utils_path)]
    if not paths:
    ...
    for version in SUPPORTED_PYTHON_VERSIONS:
        ...
        cmd = ['pytest', '--boxed', '-r', 'a', '-n', str(args.num_workers) if args.num_workers else 'auto', '--color', 'yes' if args.color else 'no', '-p', 'no:cacheprovider', '-c', os.path.join(ANSIBLE_TEST_DATA_ROOT, 'pytest.ini'), '--junit-xml', os.path.join(ResultType.JUNIT.path, 'python%s-units.xml' % version)]
