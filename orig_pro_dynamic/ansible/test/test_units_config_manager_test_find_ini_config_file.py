...
class TestFindIniFile:
    ...
    def test_env_has_cfg_file(self, setup_env, setup_existing_files, expected):
        (setup_env,  expected) = (([alt_cfg_file], alt_cfg_file), ([cfg_dir], cfg_file))[0]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_in_homedir, cfg_in_cwd, alt_cfg_file, cfg_file)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
    ...
    def test_env_has_no_cfg_file(self, setup_env, setup_existing_files):
        setup_env = ([alt_cfg_file], [cfg_dir])[0]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_in_homedir, cfg_in_cwd)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
    ...
    def test_ini_in_cwd(self, setup_env, setup_existing_files):
        setup_env = [None]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_in_homedir, cfg_in_cwd, cfg_file, alt_cfg_file)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
    ...
    def test_ini_in_homedir(self, setup_env, setup_existing_files):
        setup_env = [None]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_in_homedir, cfg_file, alt_cfg_file)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
    ...
    def test_ini_in_systemdir(self, setup_env, setup_existing_files):
        setup_env = [None]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_file, alt_cfg_file)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
    ...
    def test_no_warning_on_writable_if_env_used(self, setup_env, setup_existing_files, monkeypatch, expected):
        (setup_env,  expected) = (([alt_cfg_file], alt_cfg_file), ([cfg_in_cwd], cfg_in_cwd))[0]
        setup_existing_files = [('/etc/ansible/ansible.cfg', cfg_in_homedir, cfg_in_cwd, cfg_file, alt_cfg_file)]
        setup_env = setup_env()
        setup_existing_files = setup_existing_files()
...
def test_TestFindIniFile_test_env_has_cfg_file():
    ret = TestFindIniFile().test_env_has_cfg_file()

def test_TestFindIniFile_test_env_has_no_cfg_file():
    ret = TestFindIniFile().test_env_has_no_cfg_file()

def test_TestFindIniFile_test_ini_in_cwd():
    ret = TestFindIniFile().test_ini_in_cwd()

def test_TestFindIniFile_test_ini_in_homedir():
    ret = TestFindIniFile().test_ini_in_homedir()

def test_TestFindIniFile_test_ini_in_systemdir():
    ret = TestFindIniFile().test_ini_in_systemdir()

def test_TestFindIniFile__os_stat():
    ret = TestFindIniFile()._os_stat()

def test_TestFindIniFile_test_cwd_does_not_exist():
    ret = TestFindIniFile().test_cwd_does_not_exist()

def test_TestFindIniFile_test_no_config():
    ret = TestFindIniFile().test_no_config()

def test_TestFindIniFile__os_stat():
    ret = TestFindIniFile()._os_stat()

def test_TestFindIniFile_test_no_cwd_cfg_no_warning_on_writable():
    ret = TestFindIniFile().test_no_cwd_cfg_no_warning_on_writable()

def test_TestFindIniFile__os_stat():
    ret = TestFindIniFile()._os_stat()

def test_TestFindIniFile_test_cwd_warning_on_writable():
    ret = TestFindIniFile().test_cwd_warning_on_writable()

def test_TestFindIniFile__os_stat():
    ret = TestFindIniFile()._os_stat()

def test_TestFindIniFile_test_no_warning_on_writable_if_env_used():
    ret = TestFindIniFile().test_no_warning_on_writable_if_env_used()

def test_TestFindIniFile__os_stat():
    ret = TestFindIniFile()._os_stat()

def test_TestFindIniFile_test_cwd_warning_on_writable_no_warning_set():
    ret = TestFindIniFile().test_cwd_warning_on_writable_no_warning_set()