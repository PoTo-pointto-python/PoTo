...
from ansible.module_utils.ec2 import AWSRetry, camel_dict_to_snake_dict, compare_aws_tags, ansible_dict_to_boto3_tag_list, boto3_tag_list_to_ansible_dict
...
def wait_for_vpc_attribute(connection, module, vpc_id, attribute, expected_value):
    ...
    while time() < start_time + 300:
        current_value = connection.describe_vpc_attribute(Attribute=attribute, VpcId=vpc_id)['{0}{1}'.format(attribute[0].upper(), attribute[1:])]['Value']
...
def get_cidr_network_bits(module, cidr_block):
    ...
    for cidr in cidr_block:
        ...
        if len(split_addr) == 2:
            ...
            if cidr != valid_cidr:
                module.warn('One of your CIDR addresses ({0}) has host bits set. To get rid of this warning, check the network mask and make sure that only network bits are set: {1}.'.format(cidr, valid_cidr))
    ...
    if state == 'present':
        ...
        if ipv6_cidr:
            if 'Ipv6CidrBlockAssociationSet' in vpc_obj.keys():
                module.warn('Only one IPv6 CIDR is permitted per VPC, {0} already has CIDR {1}'.format(vpc_id, vpc_obj['Ipv6CidrBlockAssociationSet'][0]['Ipv6CidrBlock']))
            else:
                try:
        ...
        if to_add or to_remove:
            try:
                connection.get_waiter('vpc_available').wait(VpcIds=[vpc_id], Filters=[{'Name': 'cidr-block-association.cidr-block', 'Values': expected_cidrs}])
            except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
