...
def to_permission(rule):
    ...
    if rule.target_type == 'ipv4':
        ...
        perm['PrefixListIds'] = [{'PrefixListId': rule.target}]
        if rule.description:
...
def rule_from_group_permission(perm):
    ...
    if 'UserIdGroupPairs' in perm and perm['UserIdGroupPairs']:
        for pair in perm['UserIdGroupPairs']:
            target = (pair.get('UserId', None), pair.get('GroupId', None), pair.get('GroupName', None))
            if pair.get('UserId', '').startswith('amazon-'):
                target = (target[0], None, target[2])
...
def validate_rule(module, rule):
    VALID_PARAMS = ('cidr_ip', 'cidr_ipv6', 'ip_prefix', 'group_id', 'group_name', 'group_desc', 'proto', 'from_port', 'to_port', 'rule_desc')
        ...
        if group_name == name:
            ...
            if not rule.get('group_desc', '').strip():
                try:
                    ...
                    module.fail_json(msg='group %s will be automatically created by rule %s but no description was provided' % (group_name, rule))
                ...
                try:
                    ...
                    get_waiter(client, 'security_group_exists').wait(GroupIds=[auto_group['GroupId']])
                    ...
                    try:
                        ...
                        module.fail_json_aws(e, msg="Could not create or use existing group '{0}' in rule. Make sure the group exists".format(group_name))
...
def validate_ip(module, cidr_ip):
    ...
    if len(split_addr) == 2:
        try:
            ...
            if ip != cidr_ip:
                module.warn('One of your CIDR addresses ({0}) has host bits set. To get rid of this warning, check the network mask and make sure that only network bits are set: {1}.'.format(cidr_ip, ip))
        except ValueError:
            try:
                ...
                if ip6 != cidr_ip:
                    module.warn('One of your IPv6 CIDR addresses ({0}) has host bits set. To get rid of this warning, check the network mask and make sure that only network bits are set: {1}.'.format(cidr_ip, ip6))
                return ip6
        return ip
    return cidr_ip
...
def get_diff_final_resource(client, module, security_group):
    ...
    def get_final_rules(client, module, security_group_rules, specified_rules, purge_rules):
        ...
        for rule in specified_rules:
            format_rule = {'from_port': None, 'to_port': None, 'ip_protocol': rule.get('proto', 'tcp'), 'ip_ranges': [], 'ipv6_ranges': [], 'prefix_list_ids': [], 'user_id_group_pairs': []}
