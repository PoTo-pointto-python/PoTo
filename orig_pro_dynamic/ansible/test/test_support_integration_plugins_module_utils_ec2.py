...
from ansible.module_utils.common.dict_transformations import camel_dict_to_snake_dict, snake_dict_to_camel_dict, _camel_to_snake, _snake_to_camel
BOTO_IMP_ERR = None
try:
    import boto
    import boto.ec2
...
def _boto3_conn(conn_type=None, resource=None, region=None, endpoint=None, **params):
    ...
    config = botocore.config.Config(user_agent_extra='Ansible/{0}'.format(__version__))
...
def aws_common_argument_spec():
    return dict(debug_botocore_endpoint_logs=dict(fallback=(env_fallback, ['ANSIBLE_DEBUG_BOTOCORE_LOGS']), default=False, type='bool'), ec2_url=dict(), aws_secret_key=dict(aliases=['ec2_secret_key', 'secret_key'], no_log=True), aws_access_key=dict(aliases=['ec2_access_key', 'access_key']), validate_certs=dict(default=True, type='bool'), security_token=dict(aliases=['access_token'], no_log=True), profile=dict(), aws_config=dict(type='dict'))

def ec2_argument_spec():
    spec = aws_common_argument_spec()
    spec.update(dict(region=dict(aliases=['aws_region', 'ec2_region'])))
    return spec
...
def test_AWSRetry_status_code_from_exception():
    ret = AWSRetry().status_code_from_exception()

def test_AWSRetry_found():
    ret = AWSRetry().found()