    ...
    def gen_runable(self, subsets, valid_subsets, resource_facts=False):
        ...
        for subset in subsets:
            ...
            if subset not in valid_subsets:
                self._module.fail_json(msg='Subset must be one of [%s], got %s' % (', '.join(sorted([item for item in valid_subsets])), subset))
            if exclude:
...
def test_FactsBase_gen_runable():
    ret = FactsBase().gen_runable()

def test_FactsBase_get_network_resources_facts():
    ret = FactsBase().get_network_resources_facts()

def test_FactsBase_get_network_legacy_facts():
    ret = FactsBase().get_network_legacy_facts()