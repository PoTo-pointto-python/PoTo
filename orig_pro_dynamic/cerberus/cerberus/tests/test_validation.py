...
# PT
def test_PT_unknown_field():
    validator = Validator()
    test_unknown_field(validatir)

def test_PT_test_empty_field_definition():
    document = {'property': 'string'}
    test_empty_field_definition(document)

def test_PT_test_success_with_multiple_rules():
    document = [
        {'a_boolean': True},
        {'a_datetime': datetime.now()},
        {'a_float': 3.5},
    ...
    test_success_with_multiple_rules(document)

def test_PT_test_one_of_two_types():
    validator = Validator()
    test_one_of_two_types(validator)

def test_PT_test_unknown_key_dict():
    validator = Validator()
    test_unknown_key_dict(validator)

def test_PT_test_unknown_key_list():
    validator = Validator()
    test_unknown_key_dict(validator)

def test_PT_test_unknown_keys_list_of_dicts():
    validator = Validator()
    test_unknown_keys_list_of_dicts(validator)

def test_PT_test_validated():
    validator = Validator()
    test_validated(validator)

def test_PT_test_issue_107():
    validator = Validator()
    test_issue_107(validator)