...
def load_privatekey(path, passphrase=None, check_passphrase=True, content=None, backend='pyopenssl'):
    ...
    try:
        ...
        if backend == 'pyopenssl':
            try:
                result = crypto.load_privatekey(crypto.FILETYPE_PEM, priv_key_detail, to_bytes(passphrase or ''))
            ...
            if check_passphrase:
                try:
                    crypto.load_privatekey(crypto.FILETYPE_PEM, priv_key_detail, to_bytes('y' if passphrase == 'x' else 'x'))
                    if passphrase is not None:
            ...
            try:
                result = load_pem_private_key(priv_key_detail, None if passphrase is None else to_bytes(passphrase), cryptography_backend())
...
def write_file(module, content, default_mode=None, path=None):
    ...
    try:
        ...
        pass
    module.add_cleanup_file(tmp_name)
    try:
        try:
            file = os.open(tmp_name, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 384)
...
for (dotted, names) in _OID_MAP.items():
    for name in names:
        if name in _NORMALIZE_NAMES and _OID_LOOKUP[name] != dotted:
            raise AssertionError('Name collision during setup: "{0}" for OIDs {1} and {2}'.format(name, dotted, _OID_LOOKUP[name]))
...
for (alias, original) in [('userID', 'userId')]:
    if alias in _NORMALIZE_NAMES:
        raise AssertionError('Name collision during adding aliases: "{0}" (alias for "{1}") is already mapped to OID {2}'.format(alias, original, _OID_LOOKUP[alias]))
...
def cryptography_get_extensions_from_cert(cert):
    ...
    for i in range(backend._lib.X509_get_ext_count(x509_obj)):
        ...
        entry = dict(critical=crit == 1, value=base64.b64encode(der))
...
def pyopenssl_get_extensions_from_cert(cert):
    ...
    for i in range(0, ext_count):
        ...
        entry = dict(critical=bool(ext.get_critical()), value=base64.b64encode(ext.get_data()))
        oid = _obj2txt(OpenSSL._util.lib, OpenSSL._util.ffi, OpenSSL._util.lib.X509_EXTENSION_get_object(ext._extension))
        result[oid] = entry
    return result
...
if HAS_CRYPTOGRAPHY:
    REVOCATION_REASON_MAP = {'unspecified': x509.ReasonFlags.unspecified, 'key_compromise': x509.ReasonFlags.key_compromise, 'ca_compromise': x509.ReasonFlags.ca_compromise, 'affiliation_changed': x509.ReasonFlags.affiliation_changed, 'superseded': x509.ReasonFlags.superseded, 'cessation_of_operation': x509.ReasonFlags.cessation_of_operation, 'certificate_hold': x509.ReasonFlags.certificate_hold, 'privilege_withdrawn': x509.ReasonFlags.privilege_withdrawn, 'aa_compromise': x509.ReasonFlags.aa_compromise, 'remove_from_crl': x509.ReasonFlags.remove_from_crl}
...
def test_OpenSSLObject__check_state():
    ret = OpenSSLObject()._check_state()

def test_OpenSSLObject__check_perms():
    ret = OpenSSLObject()._check_perms()

def test_OpenSSLObject_check():
    ret = OpenSSLObject().check()

def test_OpenSSLObject_dump():
    ret = OpenSSLObject().dump()

def test_OpenSSLObject_generate():
    ret = OpenSSLObject().generate()

def test_OpenSSLObject_remove():
    ret = OpenSSLObject().remove()