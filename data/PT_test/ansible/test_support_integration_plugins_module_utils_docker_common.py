...
class AnsibleDockerClient(Client):
    ...
    def __init__(self, argument_spec=None, supports_check_mode=False, mutually_exclusive=None, required_together=None, required_if=None, min_docker_version=MIN_DOCKER_VERSION, min_docker_api_version=None, option_minimal_versions=None, option_minimal_versions_ignore_params=None, fail_results=None):
    ...
    def inspect_distribution(self, image, **kwargs):
        ...
        if self.docker_py_version < LooseVersion('4.0.0'):
            ...
            if header:
                return self._result(self._get(self._url('/distribution/{0}/json', image), headers={'X-Registry-Auth': header}), json=True)
...
def parse_healthcheck(healthcheck):
    ...
    if not healthcheck or not healthcheck.get('test'):
        return (None, None)
    result = dict()
    options = dict(test='test', interval='interval', timeout='timeout', start_period='start_period', retries='retries')
            ...
            if value in duration_options:
                ...
                if key == 'test':
                    ...
                    try:
                        ...
                        raise ValueError('Cannot parse number of retries for healthcheck. Expected an integer, got "{0}".'.format(result[key]))
    if result['test'] == ['NONE']:
        return (None, True)
    return (result, False)
...
def test_DockerBaseClass_log():
    ret = DockerBaseClass().log()

def test_AnsibleDockerClient_log():
    ret = AnsibleDockerClient().log()

def test_AnsibleDockerClient_fail():
    ret = AnsibleDockerClient().fail()

def test_AnsibleDockerClient__get_value():
    ret = AnsibleDockerClient()._get_value()

def test_AnsibleDockerClient_auth_params():
    ret = AnsibleDockerClient().auth_params()

def test_AnsibleDockerClient__handle_ssl_error():
    ret = AnsibleDockerClient()._handle_ssl_error()

def test_AnsibleDockerClient__get_minimal_versions():
    ret = AnsibleDockerClient()._get_minimal_versions()

def test_AnsibleDockerClient_get_container_by_id():
    ret = AnsibleDockerClient().get_container_by_id()

def test_AnsibleDockerClient_get_container():
    ret = AnsibleDockerClient().get_container()

def test_AnsibleDockerClient_get_network():
    ret = AnsibleDockerClient().get_network()

def test_AnsibleDockerClient_find_image():
    ret = AnsibleDockerClient().find_image()

def test_AnsibleDockerClient_find_image_by_id():
    ret = AnsibleDockerClient().find_image_by_id()

def test_AnsibleDockerClient__image_lookup():
    ret = AnsibleDockerClient()._image_lookup()

def test_AnsibleDockerClient_pull_image():
    ret = AnsibleDockerClient().pull_image()

def test_AnsibleDockerClient_report_warnings():
    ret = AnsibleDockerClient().report_warnings()

def test_AnsibleDockerClient_inspect_distribution():
    ret = AnsibleDockerClient().inspect_distribution()

def test_DifferenceTracker_add():
    ret = DifferenceTracker().add()

def test_DifferenceTracker_merge():
    ret = DifferenceTracker().merge()

def test_DifferenceTracker_empty():
    ret = DifferenceTracker().empty()

def test_DifferenceTracker_get_before_after():
    ret = DifferenceTracker().get_before_after()

def test_DifferenceTracker_has_difference_for():
    ret = DifferenceTracker().has_difference_for()

def test_DifferenceTracker_get_legacy_docker_container_diffs():
    ret = DifferenceTracker().get_legacy_docker_container_diffs()

def test_DifferenceTracker_get_legacy_docker_diffs():
    ret = DifferenceTracker().get_legacy_docker_diffs()