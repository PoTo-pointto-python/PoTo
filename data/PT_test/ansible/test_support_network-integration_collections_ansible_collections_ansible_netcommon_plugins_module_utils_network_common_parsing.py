...
class CommandRunner(object):
    ...
    def add_command(self, command, output=None, prompt=None, response=None, **kwargs):
        ...
        cmd = self.module.cli.to_command(command, output=output, prompt=prompt, response=response, **kwargs)
...
class Conditional(object):
    ...
    """
    OPERATORS = {'eq': ['eq', '=='], 'neq': ['neq', 'ne', '!='], 'gt': ['gt', '>'], 'ge': ['ge', '>='], 'lt': ['lt', '<'], 'le': ['le', '<='], 'contains': ['contains'], 'matches': ['matches']}
    ...
    def get_json(self, result):
        string = re.sub('\\[[\\\'|\\"]', '.', self.key)
        string = re.sub('[\\\'|\\"]\\]', '.', string)
        parts = re.split('\\.(?=[^\\]]*(?:\\[|$))', string)
        for part in parts:
            match = re.findall('\\[(\\S+?)\\]', part)
            if match:
                key = part[:part.find('[')]
                result = result[key]
                for m in match:
...
def test_Cli___call__():
    ret = Cli().__call__()

def test_Cli_to_command():
    ret = Cli().to_command()

def test_Cli_add_commands():
    ret = Cli().add_commands()

def test_Cli_run_commands():
    ret = Cli().run_commands()

def test_Command___str__():
    ret = Command().__str__()

def test_CommandRunner_add_command():
    ret = CommandRunner().add_command()

def test_CommandRunner_get_command():
    ret = CommandRunner().get_command()

def test_CommandRunner_get_responses():
    ret = CommandRunner().get_responses()

def test_CommandRunner_add_conditional():
    ret = CommandRunner().add_conditional()

def test_CommandRunner_run():
    ret = CommandRunner().run()

def test_Conditional___call__():
    ret = Conditional().__call__()

def test_Conditional__cast_value():
    ret = Conditional()._cast_value()

def test_Conditional__func():
    ret = Conditional()._func()

def test_Conditional_get_value():
    ret = Conditional().get_value()

def test_Conditional_get_json():
    ret = Conditional().get_json()

def test_Conditional_number():
    ret = Conditional().number()

def test_Conditional_eq():
    ret = Conditional().eq()

def test_Conditional_neq():
    ret = Conditional().neq()

def test_Conditional_gt():
    ret = Conditional().gt()

def test_Conditional_ge():
    ret = Conditional().ge()

def test_Conditional_lt():
    ret = Conditional().lt()

def test_Conditional_le():
    ret = Conditional().le()

def test_Conditional_contains():
    ret = Conditional().contains()

def test_Conditional_matches():
    ret = Conditional().matches()