...
def main():
    ...
    if collection_full_name:
        ...
        _collection_finder._meta_yml_to_dict = yaml_to_dict
    ...
    def test_python_module(path, name, base_dir, messages):
        ...
        try:
            ...
            if isinstance(ex, SyntaxError):
                if ex.filename == full_path:
                    line = ex.lineno or 0
                    offset = ex.offset or 0
                elif ex.filename.startswith(base_path) and (not ex.filename.startswith(temp_path)):
                    source = (os.path.relpath(ex.filename, base_path), ex.lineno or 0, ex.offset or 0)
                message = re.sub(' \\(.*?, line [0-9]+\\)$', '', message)
...
def test_RestrictedModuleLoader_find_module():
    ret = RestrictedModuleLoader().find_module()

def test_RestrictedModuleLoader_load_module():
    ret = RestrictedModuleLoader().load_module()

def test_RestrictedModuleLoader___load_module():
    ret = RestrictedModuleLoader().__load_module()