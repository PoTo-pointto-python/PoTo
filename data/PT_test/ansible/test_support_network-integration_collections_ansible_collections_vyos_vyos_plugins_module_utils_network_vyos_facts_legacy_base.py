...
class Default(LegacyFactsBase):
    ...
    def parse_serialnum(self, data):
        match = re.search('HW S/N:\\s+(\\S+)', data)
        if match:
    ...
    def populate(self):
        ...
        for line in commits.split('\n'):
            ...
            if match:
                ...
                entry = dict(revision=match.group(1), datetime=match.group(2), by=str(match.group(3)).strip(), via=str(match.group(4)).strip(), comment=None)
            else:
...
def test_LegacyFactsBase_populate():
    ret = LegacyFactsBase().populate()

def test_Default_populate():
    ret = Default().populate()

def test_Default_parse_serialnum():
    ret = Default().parse_serialnum()

def test_Default_platform_facts():
    ret = Default().platform_facts()

def test_Config_populate():
    ret = Config().populate()

def test_Neighbors_populate():
    ret = Neighbors().populate()

def test_Neighbors_parse():
    ret = Neighbors().parse()

def test_Neighbors_parse_neighbors():
    ret = Neighbors().parse_neighbors()

def test_Neighbors_parse_interface():
    ret = Neighbors().parse_interface()

def test_Neighbors_parse_host():
    ret = Neighbors().parse_host()

def test_Neighbors_parse_port():
    ret = Neighbors().parse_port()