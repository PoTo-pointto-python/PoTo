...
class TestAnsibleModuleTmpDir:
    ...
    def test_tmpdir_property(self, monkeypatch, args, expected, stat_exists):
        (args,  expected,  stat_exists) = ((s, e, t) for (s, t, e) in DATA)[0]
        ...
        if not stat_exists:
            ...
            assert makedirs['mode'] == 448

    @pytest.mark.parametrize('stdin', ({'_ansible_tmpdir': None, '_ansible_remote_tmp': '$HOME/.test', '_ansible_keep_remote_files': True},), indirect=['stdin'])
    def test_tmpdir_makedirs_failure(self, am, monkeypatch):
        stdin = ({'_ansible_tmpdir': None, '_ansible_remote_tmp': '$HOME/.test', '_ansible_keep_remote_files': True},)[0]
        ...
        assert mock_makedirs.call_args[1] == {'mode': 448}
        assert mock_mkdtemp.call_args[1]['dir'] is None
        assert mock_mkdtemp.call_args[1]['prefix'].startswith('ansible-moduletmp-')

def test_TestAnsibleModuleTmpDir_mock_mkdtemp():
    ret = TestAnsibleModuleTmpDir().mock_mkdtemp()

def test_TestAnsibleModuleTmpDir_mock_makedirs():
    ret = TestAnsibleModuleTmpDir().mock_makedirs()

def test_TestAnsibleModuleTmpDir_test_tmpdir_property():
    ret = TestAnsibleModuleTmpDir().test_tmpdir_property()

def test_TestAnsibleModuleTmpDir_test_tmpdir_makedirs_failure():
    ret = TestAnsibleModuleTmpDir().test_tmpdir_makedirs_failure()