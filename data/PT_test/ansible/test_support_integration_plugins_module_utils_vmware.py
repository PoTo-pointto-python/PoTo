...
def gather_vm_facts(content, vm):
    ...
    for entry in vm.config.hardware.device:
        ...
        facts[factname] = {'addresstype': entry.addressType, 'label': entry.deviceInfo.label, 'macaddress': mac_addr, 'ipaddresses': net_dict.get(entry.macAddress, None), 'macaddress_dash': mac_addr_dash, 'summary': entry.deviceInfo.summary, 'portgroup_portkey': port_key, 'portgroup_key': port_group_key}
...
def deserialize_snapshot_obj(obj):
    return {'id': obj.id, 'name': obj.name, 'description': obj.description, 'creation_time': obj.createTime, 'state': obj.state}
...
def test_PyVmomi_is_vcenter():
    ret = PyVmomi().is_vcenter()

def test_PyVmomi_get_managed_objects_properties():
    ret = PyVmomi().get_managed_objects_properties()

def test_PyVmomi_get_vm():
    ret = PyVmomi().get_vm()

def test_PyVmomi_gather_facts():
    ret = PyVmomi().gather_facts()

def test_PyVmomi_get_vm_path():
    ret = PyVmomi().get_vm_path()

def test_PyVmomi_get_vm_or_template():
    ret = PyVmomi().get_vm_or_template()

def test_PyVmomi_find_cluster_by_name():
    ret = PyVmomi().find_cluster_by_name()

def test_PyVmomi_get_all_hosts_by_cluster():
    ret = PyVmomi().get_all_hosts_by_cluster()

def test_PyVmomi_find_hostsystem_by_name():
    ret = PyVmomi().find_hostsystem_by_name()

def test_PyVmomi_get_all_host_objs():
    ret = PyVmomi().get_all_host_objs()

def test_PyVmomi_host_version_at_least():
    ret = PyVmomi().host_version_at_least()

def test_PyVmomi_find_host_portgroup_by_name():
    ret = PyVmomi().find_host_portgroup_by_name()

def test_PyVmomi_get_all_port_groups_by_host():
    ret = PyVmomi().get_all_port_groups_by_host()

def test_PyVmomi_find_network_by_name():
    ret = PyVmomi().find_network_by_name()

def test_PyVmomi_network_exists_by_name():
    ret = PyVmomi().network_exists_by_name()

def test_PyVmomi_find_datacenter_by_name():
    ret = PyVmomi().find_datacenter_by_name()

def test_PyVmomi_is_datastore_valid():
    ret = PyVmomi().is_datastore_valid()

def test_PyVmomi_find_datastore_by_name():
    ret = PyVmomi().find_datastore_by_name()

def test_PyVmomi_find_folder_by_name():
    ret = PyVmomi().find_folder_by_name()

def test_PyVmomi_find_datastore_cluster_by_name():
    ret = PyVmomi().find_datastore_cluster_by_name()

def test_PyVmomi_find_resource_pool_by_name():
    ret = PyVmomi().find_resource_pool_by_name()

def test_PyVmomi_find_resource_pool_by_cluster():
    ret = PyVmomi().find_resource_pool_by_cluster()

def test_PyVmomi_vmdk_disk_path_split():
    ret = PyVmomi().vmdk_disk_path_split()

def test_PyVmomi_find_vmdk_file():
    ret = PyVmomi().find_vmdk_file()

def test_PyVmomi__deepmerge():
    ret = PyVmomi()._deepmerge()

def test_PyVmomi__extract():
    ret = PyVmomi()._extract()

def test_PyVmomi__jsonify():
    ret = PyVmomi()._jsonify()

def test_PyVmomi_to_json():
    ret = PyVmomi().to_json()

def test_PyVmomi_get_folder_path():
    ret = PyVmomi().get_folder_path()