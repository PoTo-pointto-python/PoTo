...
def changed(result):
    ...
    if 'changed' not in result:
        changed = False
        if 'results' in result and isinstance(result['results'], MutableSequence) and isinstance(result['results'][0], MutableMapping):
...
def version_compare(value, version, operator='eq', strict=False):
    ...
    op_map = {'==': 'eq', '=': 'eq', 'eq': 'eq', '<': 'lt', 'lt': 'lt', '<=': 'le', 'le': 'le', '>': 'gt', 'gt': 'gt', '>=': 'ge', 'ge': 'ge', '!=': 'ne', '<>': 'ne', 'ne': 'ne'}
    if strict:
...
class TestModule(object):
    ...
    def tests(self):
        return {'failed': failed, 'failure': failed, 'succeeded': success, 'success': success, 'successful': success, 'reachable': reachable, 'unreachable': unreachable, 'changed': changed, 'change': changed, 'skipped': skipped, 'skip': skipped, 'finished': finished, 'started': started, 'match': match, 'search': search, 'regex': regex, 'version_compare': version_compare, 'version': version_compare, 'any': any, 'all': all, 'truthy': truthy, 'falsy': falsy, 'vault_encrypted': vault_encrypted}
