...
from ansible.module_utils.vmware import find_obj, gather_vm_facts, get_all_objs, compile_folder_path_for_object, serialize_spec, vmware_argument_spec, set_vm_power_state, PyVmomi, find_dvs_by_name, find_dvspg_by_name, wait_for_vm_ip, wait_for_task, TaskError, quote_obj_name
...
class PyVmomiDeviceHelper(object):
    ...
    def get_device(self, device_type, name):
        nic_dict = dict(pcnet32=vim.vm.device.VirtualPCNet32(), vmxnet2=vim.vm.device.VirtualVmxnet2(), vmxnet3=vim.vm.device.VirtualVmxnet3(), e1000=vim.vm.device.VirtualE1000(), e1000e=vim.vm.device.VirtualE1000e(), sriov=vim.vm.device.VirtualSriovEthernetCard())
...
class PyVmomiHelper(PyVmomi):
        ...
        if expected_cdrom_spec:
            for cdrom_spec in expected_cdrom_spec:
                ...
                if cdrom_spec['controller_type'] not in ['ide', 'sata']:
                    self.module.fail_json(msg="Invalid cdrom.controller_type: %s, valid value is 'ide' or 'sata'." % cdrom_spec['controller_type'])
                cdrom_spec['state'] = cdrom_spec.get('state', 'present').lower()
                if cdrom_spec['state'] not in ['present', 'absent']:
                    self.module.fail_json(msg="Invalid cdrom.state: %s, valid value is 'present', 'absent'." % cdrom_spec['state'])
                if cdrom_spec['state'] == 'present':
                    if 'type' in cdrom_spec and cdrom_spec.get('type') not in ['none', 'client', 'iso']:
                        self.module.fail_json(msg="Invalid cdrom.type: %s, valid value is 'none', 'client' or 'iso'." % cdrom_spec.get('type'))
                ...
                if cdrom_spec['controller_type'] == 'ide' and (cdrom_spec.get('controller_number') not in [0, 1] or cdrom_spec.get('unit_number') not in [0, 1]):
                ...
                if cdrom_spec['controller_type'] == 'sata' and (cdrom_spec.get('controller_number') not in range(0, 4) or cdrom_spec.get('unit_number') not in range(0, 30)):
    ...
    def configure_cdrom_dict(self, vm_obj):
        ...
        if len(cdrom_devices) == 0:
            ...
            cdrom_spec = self.device_helper.create_cdrom(ide_device=ide_device, cdrom_type=self.params['cdrom']['type'], iso_path=iso_path)
            if vm_obj and vm_obj.runtime.powerState == vim.VirtualMachinePowerState.poweredOn:
                cdrom_spec.device.connectable.connected = self.params['cdrom']['type'] != 'none'
        elif not self.device_helper.is_equal_cdrom(vm_obj=vm_obj, cdrom_device=cdrom_devices[0], cdrom_type=self.params['cdrom']['type'], iso_path=iso_path):
            ...
            for expected_cdrom_spec in configured_cdroms['ide']:
                ...
                for cdrom in expected_cdrom_spec['cdrom']:
                    ...
                    if not cdrom_device and cdrom.get('state') != 'absent':
                        ...
                        cdrom_spec = self.device_helper.create_cdrom(ide_device=ide_device, cdrom_type=cdrom['type'], iso_path=iso_path, unit_number=unit_number)
                        self.change_detected = True
                        self.configspec.deviceChange.append(cdrom_spec)
    ...
    def get_vm_network_interfaces(self, vm=None):
        ...
        if vm is None:
            return device_list
        nw_device_types = (vim.vm.device.VirtualPCNet32, vim.vm.device.VirtualVmxnet2, vim.vm.device.VirtualVmxnet3, vim.vm.device.VirtualE1000, vim.vm.device.VirtualE1000e, vim.vm.device.VirtualSriovEthernetCard)
        for device in vm.config.hardware.device:
    ...
    def select_datastore(self, vm_obj=None):
        ...
        if not datastore:
            if len(self.params['disk']) != 0 or self.params['template'] is None:
                self.module.fail_json(msg='Unable to find the datastore with given parameters. This could mean, %s is a non-existent virtual machine and module tried to deploy it as new virtual machine with no disk. Please specify disks parameter or specify template to clone from.' % self.params['name'])
    ...
    def customize_exist_vm(self):
        ...
        if self.params['wait_for_customization']:
            ...
            if not is_customization_ok:
                return {'changed': self.change_applied, 'failed': True, 'msg': 'Wait for customization failed due to timeout', 'op': 'wait_for_customize_exist'}
        return {'changed': self.change_applied, 'failed': False}
...
def test_PyVmomiDeviceHelper_create_scsi_controller():
    ret = PyVmomiDeviceHelper().create_scsi_controller()

def test_PyVmomiDeviceHelper_is_scsi_controller():
    ret = PyVmomiDeviceHelper().is_scsi_controller()

def test_PyVmomiDeviceHelper_create_ide_controller():
    ret = PyVmomiDeviceHelper().create_ide_controller()

def test_PyVmomiDeviceHelper_create_cdrom():
    ret = PyVmomiDeviceHelper().create_cdrom()

def test_PyVmomiDeviceHelper_is_equal_cdrom():
    ret = PyVmomiDeviceHelper().is_equal_cdrom()

def test_PyVmomiDeviceHelper_update_cdrom_config():
    ret = PyVmomiDeviceHelper().update_cdrom_config()

def test_PyVmomiDeviceHelper_remove_cdrom():
    ret = PyVmomiDeviceHelper().remove_cdrom()

def test_PyVmomiDeviceHelper_create_scsi_disk():
    ret = PyVmomiDeviceHelper().create_scsi_disk()

def test_PyVmomiDeviceHelper_get_device():
    ret = PyVmomiDeviceHelper().get_device()

def test_PyVmomiDeviceHelper_create_nic():
    ret = PyVmomiDeviceHelper().create_nic()

def test_PyVmomiDeviceHelper_integer_value():
    ret = PyVmomiDeviceHelper().integer_value()

def test_PyVmomiCache_find_obj():
    ret = PyVmomiCache().find_obj()

def test_PyVmomiCache_get_all_objs():
    ret = PyVmomiCache().get_all_objs()

def test_PyVmomiCache_get_network():
    ret = PyVmomiCache().get_network()

def test_PyVmomiCache_get_cluster():
    ret = PyVmomiCache().get_cluster()

def test_PyVmomiCache_get_esx_host():
    ret = PyVmomiCache().get_esx_host()

def test_PyVmomiCache_get_parent_datacenter():
    ret = PyVmomiCache().get_parent_datacenter()

def test_PyVmomiHelper_gather_facts():
    ret = PyVmomiHelper().gather_facts()

def test_PyVmomiHelper_remove_vm():
    ret = PyVmomiHelper().remove_vm()

def test_PyVmomiHelper_configure_guestid():
    ret = PyVmomiHelper().configure_guestid()

def test_PyVmomiHelper_configure_resource_alloc_info():
    ret = PyVmomiHelper().configure_resource_alloc_info()

def test_PyVmomiHelper_configure_cpu_and_memory():
    ret = PyVmomiHelper().configure_cpu_and_memory()

def test_PyVmomiHelper_sanitize_cdrom_params():
    ret = PyVmomiHelper().sanitize_cdrom_params()

def test_PyVmomiHelper_configure_cdrom():
    ret = PyVmomiHelper().configure_cdrom()

def test_PyVmomiHelper_configure_cdrom_dict():
    ret = PyVmomiHelper().configure_cdrom_dict()

def test_PyVmomiHelper_configure_cdrom_list():
    ret = PyVmomiHelper().configure_cdrom_list()

def test_PyVmomiHelper_configure_hardware_params():
    ret = PyVmomiHelper().configure_hardware_params()

def test_PyVmomiHelper_get_device_by_type():
    ret = PyVmomiHelper().get_device_by_type()

def test_PyVmomiHelper_get_vm_cdrom_devices():
    ret = PyVmomiHelper().get_vm_cdrom_devices()

def test_PyVmomiHelper_get_vm_ide_devices():
    ret = PyVmomiHelper().get_vm_ide_devices()

def test_PyVmomiHelper_get_vm_network_interfaces():
    ret = PyVmomiHelper().get_vm_network_interfaces()

def test_PyVmomiHelper_sanitize_network_params():
    ret = PyVmomiHelper().sanitize_network_params()

def test_PyVmomiHelper_configure_network():
    ret = PyVmomiHelper().configure_network()

def test_PyVmomiHelper_configure_vapp_properties():
    ret = PyVmomiHelper().configure_vapp_properties()

def test_PyVmomiHelper_customize_customvalues():
    ret = PyVmomiHelper().customize_customvalues()

def test_PyVmomiHelper_customize_vm():
    ret = PyVmomiHelper().customize_vm()

def test_PyVmomiHelper_get_vm_scsi_controller():
    ret = PyVmomiHelper().get_vm_scsi_controller()

def test_PyVmomiHelper_get_configured_disk_size():
    ret = PyVmomiHelper().get_configured_disk_size()

def test_PyVmomiHelper_add_existing_vmdk():
    ret = PyVmomiHelper().add_existing_vmdk()

def test_PyVmomiHelper_configure_disks():
    ret = PyVmomiHelper().configure_disks()

def test_PyVmomiHelper_select_host():
    ret = PyVmomiHelper().select_host()

def test_PyVmomiHelper_autoselect_datastore():
    ret = PyVmomiHelper().autoselect_datastore()

def test_PyVmomiHelper_get_recommended_datastore():
    ret = PyVmomiHelper().get_recommended_datastore()

def test_PyVmomiHelper_select_datastore():
    ret = PyVmomiHelper().select_datastore()

def test_PyVmomiHelper_obj_has_parent():
    ret = PyVmomiHelper().obj_has_parent()

def test_PyVmomiHelper_get_scsi_type():
    ret = PyVmomiHelper().get_scsi_type()

def test_PyVmomiHelper_find_folder():
    ret = PyVmomiHelper().find_folder()

def test_PyVmomiHelper_get_resource_pool():
    ret = PyVmomiHelper().get_resource_pool()

def test_PyVmomiHelper_deploy_vm():
    ret = PyVmomiHelper().deploy_vm()

def test_PyVmomiHelper_get_snapshots_by_name_recursively():
    ret = PyVmomiHelper().get_snapshots_by_name_recursively()

def test_PyVmomiHelper_reconfigure_vm():
    ret = PyVmomiHelper().reconfigure_vm()

def test_PyVmomiHelper_customize_exist_vm():
    ret = PyVmomiHelper().customize_exist_vm()

def test_PyVmomiHelper_wait_for_task():
    ret = PyVmomiHelper().wait_for_task()

def test_PyVmomiHelper_get_vm_events():
    ret = PyVmomiHelper().get_vm_events()

def test_PyVmomiHelper_wait_for_customization():
    ret = PyVmomiHelper().wait_for_customization()