...
from ..sanity import SanityMultipleVersion, SanityMessage, SanityFailure, SanitySuccess, SanitySkipped, SANITY_ROOT
from ..target import TestTarget
from ..util import SubprocessError, remove_tree, display, parse_to_list_of_dict, is_subdir, generate_pip_command, find_python
from ..util_common import intercept_command, run_command, ResultType
from ..ansible_util import ansible_environment
from ..executor import generate_pip_install
from ..config import SanityConfig
from ..coverage_util import coverage_context
...
class ImportTest(SanityMultipleVersion):
    ...
    def filter_targets(self, targets): # type: (t.List[TestTarget]) -> t.List[TestTarget]
        ...
        return [target for target in targets if os.path.splitext(target.path)[1] == '.py' and (is_subdir(target.path, data_context().content.module_path) or is_subdir(target.path, data_context().content.module_utils_path))]
    ...
    def test(self, args, targets, python_version):
        ...
        env.update(SANITY_TEMP_PATH=ResultType.TMP.path)
        if data_context().content.collection:
            env.update(SANITY_COLLECTION_FULL_NAME=data_context().content.collection.full_name, SANITY_EXTERNAL_PYTHON=python)
        ...
        try:
            ...
            results = [SanityMessage(message=r['message'], path=os.path.relpath(r['path'], relative_temp_root) if r['path'].startswith(relative_temp_root) else r['path'], line=int(r['line']), column=int(r['column'])) for r in results]
        results = settings.process_errors(results, paths)
        if results:
            return SanityFailure(self.name, messages=results, python_version=python_version)
        return SanitySuccess(self.name, python_version=python_version)

def test_ImportTest_filter_targets():
    ret = ImportTest().filter_targets()

def test_ImportTest_test():
    ret = ImportTest().test()