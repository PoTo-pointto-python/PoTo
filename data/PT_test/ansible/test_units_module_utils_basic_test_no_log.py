...
class TestReturnValues(unittest.TestCase):
    dataset = (('string', frozenset(['string'])), ('', frozenset()), (1, frozenset(['1'])), (1.0, frozenset(['1.0'])), (False, frozenset()), (['1', '2', '3'], frozenset(['1', '2', '3'])), (('1', '2', '3'), frozenset(['1', '2', '3'])), ({'one': 1, 'two': 'dos'}, frozenset(['1', 'dos'])), ({'one': 1, 'two': 'dos', 'three': ['amigos', 'musketeers', None, {'ping': 'pong', 'base': ('balls', 'raquets')}]}, frozenset(['1', 'dos', 'amigos', 'musketeers', 'pong', 'balls', 'raquets'])), (u'Toshio くらとみ', frozenset(['Toshio くらとみ'])), ('Toshio くらとみ', frozenset(['Toshio くらとみ'])))
...
def test_TestReturnValues_test_return_datastructure_name():
    ret = TestReturnValues().test_return_datastructure_name()

def test_TestReturnValues_test_unknown_type():
    ret = TestReturnValues().test_unknown_type()

def test_TestRemoveValues_test_no_removal():
    ret = TestRemoveValues().test_no_removal()

def test_TestRemoveValues_test_strings_to_remove():
    ret = TestRemoveValues().test_strings_to_remove()

def test_TestRemoveValues_test_unknown_type():
    ret = TestRemoveValues().test_unknown_type()

def test_TestRemoveValues_test_hit_recursion_limit():
    ret = TestRemoveValues().test_hit_recursion_limit()