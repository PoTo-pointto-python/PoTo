...
from .encoding import to_bytes, to_optional_bytes, to_optional_text
...
REMOTE_ONLY_PYTHON_VERSIONS = ('2.6',)
SUPPORTED_PYTHON_VERSIONS = ('2.6', '2.7', '3.5', '3.6', '3.7', '3.8', '3.9')
...
def get_available_python_versions(versions): # type: (t.List[str]) -> t.Dict[str, str]
    ...
    try:
        ...
        pass
    get_available_python_versions.result = dict(((version, path) for (version, path) in ((version, find_python(version, required=False)) for version in versions) if path))
    return get_available_python_versions.result
...
def common_environment():
    ...
    env = dict(LC_ALL='en_US.UTF-8', PATH=os.environ.get('PATH', os.path.defpath))
    required = ('HOME',)
    optional = ('HTTPTESTER', 'LD_LIBRARY_PATH', 'SSH_AUTH_SOCK', 'OBJC_DISABLE_INITIALIZE_FORK_SAFETY', 'ANSIBLE_KEEP_REMOTE_FILES', 'LDFLAGS', 'CFLAGS')
...
def is_binary_file(path):
    ...
    :rtype: bool
    """
    assume_text = set(['.cfg', '.conf', '.crt', '.cs', '.css', '.html', '.ini', '.j2', '.js', '.json', '.md', '.pem', '.ps1', '.psm1', '.py', '.rst', '.sh', '.txt', '.xml', '.yaml', '.yml'])
    assume_binary = set(['.bin', '.eot', '.gz', '.ico', '.iso', '.jpg', '.otf', '.p12', '.png', '.pyc', '.rpm', '.ttf', '.woff', '.woff2', '.zip'])
    ...
    with open_binary_file(path) as path_fd:
        return b'\x00' in path_fd.read(1024)

def generate_password():
    """Generate a random password.
    :rtype: str
    """
    chars = [string.ascii_letters, string.digits, string.ascii_letters, string.digits, '-'] * 4
...
class Display:
    ...
    purple = '\x1b[35m'
    cyan = '\x1b[36m'
    verbosity_colors = {0: None, 1: green, 2: blue, 3: cyan}
    ...
    def print_message(self, message, color=None, fd=sys.stdout, truncate=False):
        """
        :type message: str
        :type color: str | None
        :type fd: file
...
def test_Display___warning():
    ret = Display().__warning()

def test_Display_review_warnings():
    ret = Display().review_warnings()

def test_Display_warning():
    ret = Display().warning()

def test_Display_notice():
    ret = Display().notice()
...
def test_Display_print_message():
    ret = Display().print_message()