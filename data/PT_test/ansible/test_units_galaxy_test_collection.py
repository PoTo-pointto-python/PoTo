...
def collection_input(tmp_path_factory):
    ...
    galaxy_args = ['ansible-galaxy', 'collection', 'init', '%s.%s' % (namespace, collection), '-c', '--init-path', test_dir, '--collection-skeleton', skeleton]
...
def galaxy_server():
    ...
    galaxy_api = api.GalaxyAPI(None, 'test_server', 'https://galaxy.ansible.com', token=token.GalaxyToken(token='key'))
    return galaxy_api
...
def files_manifest_info():
    return {'files': [{'name': '.', 'ftype': 'dir', 'chksum_type': None, 'chksum_sha256': None, 'format': 1}, {'name': 'README.md', 'ftype': 'file', 'chksum_type': 'sha256', 'chksum_sha256': 'individual_file_checksum', 'format': 1}], 'format': 1}
...
def test_build_existing_output_file(collection_input):
    ...
    expected = "The output collection artifact '%s' already exists, but is a directory - aborting" % to_native(existing_output_dir)
    with pytest.raises(AnsibleError, match=expected):
...
def test_build_existing_output_without_force(collection_input):
    ...
    with open(existing_output, 'w+') as out_file:
        ...
        out_file.flush()
    expected = "The file '%s' already exists. You can use --force to re-create the collection artifact." % to_native(existing_output)
    with pytest.raises(AnsibleError, match=expected):
        collection.build_collection(input_dir, output_dir, False)
...
def test_invalid_yaml_galaxy_file(galaxy_yml):
    galaxy_yml = b'namespace: value: broken'
    galaxy_yml = galaxy_yml()
...
def test_missing_required_galaxy_key(galaxy_yml):
    galaxy_yml = b'namespace: test_namespace'
    galaxy_yml = galaxy_yml()
    expected = "The collection galaxy.yml at '%s' is missing the following mandatory keys: authors, name, readme, version" % to_native(galaxy_yml)
    with pytest.raises(AnsibleError, match=expected):
        collection._get_galaxy_yml(galaxy_yml)
...
def test_warning_extra_keys(galaxy_yml, monkeypatch):
    ...
    assert display_mock.call_args[0][0] == "Found unknown keys in collection galaxy.yml at '%s': invalid" % to_text(galaxy_yml)
...
def test_build_ignore_files_and_folders(collection_input, monkeypatch):
    ...
    expected_msgs = ["Skipping '%s/galaxy.yml' for collection build" % to_text(input_dir), "Skipping '%s' for collection build" % to_text(retry_file), "Skipping '%s' for collection build" % to_text(git_folder), "Skipping '%s' for collection build" % to_text(tests_folder)]
...
def test_build_ignore_older_release_in_root(collection_input, monkeypatch):
    ...
    expected_msgs = ["Skipping '%s/galaxy.yml' for collection build" % to_text(input_dir), "Skipping '%s' for collection build" % to_text(release_file)]
...
def test_build_ignore_patterns(collection_input, monkeypatch):
    ...
    mock_display = MagicMock()
    monkeypatch.setattr(Display, 'vvv', mock_display)
    actual = collection._build_files_manifest(to_bytes(input_dir), 'namespace', 'collection', ['*.md', 'plugins/action', 'playbooks/*.j2'])
    assert actual['format'] == 1
    expected_missing = ['README.md', 'docs/My Collection.md', 'plugins/action', 'playbooks/templates/test.conf.j2', 'playbooks/templates/subfolder/test.conf.j2']
    expected_present = ['docs', 'roles/common/templates/test.conf.j2', 'roles/common/templates/subfolder/test.conf.j2']
...
def test_build_ignore_symlink_target_outside_collection(collection_input, monkeypatch):
    ...
    assert mock_display.mock_calls[0][1][0] == "Skipping '%s' as it is a symbolic link to a directory outside the collection" % to_text(link_path)
...
def test_publish_no_wait(galaxy_server, collection_artifact, monkeypatch):
    ...
    assert mock_display.call_count == 1
    assert mock_display.mock_calls[0][1][0] == 'Collection has been pushed to the Galaxy server %s %s, not waiting until import has completed due to --no-wait being set. Import task results can be found at %s' % (galaxy_server.name, galaxy_server.api_server, fake_import_uri)
...
def test_find_existing_collections(tmp_path_factory, monkeypatch):
    ...
    collection1_manifest = json.dumps({'collection_info': {'namespace': 'namespace1', 'name': 'collection1', 'version': '1.2.3', 'authors': ['Jordan Borean'], 'readme': 'README.md', 'dependencies': {}}, 'format': 1})
    ...
    assert mock_warning.mock_calls[0][1][0] == "Collection at '%s' does not have a MANIFEST.json file, cannot detect version." % to_text(collection2)
...
def test_extract_tar_file_outside_dir(tmp_path_factory):
    ...
    with tarfile.open(tar_file, 'w:gz') as tfile:
        ...
        tar_info.mode = 420
        tfile.addfile(tarinfo=tar_info, fileobj=b_io)
    expected = re.escape("Cannot extract tar entry '%s' as it will be placed outside the collection directory" % to_native(tar_filename))
    with tarfile.open(tar_file, 'r') as tfile:
        with pytest.raises(AnsibleError, match=expected):
