...
class RabbitClient:
    ...
    def basic_publish(self):
        ...
        if self.params.get('body') is not None:
            args = dict(body=self.params.get('body'), exchange=self.params.get('exchange'), routing_key=self.params.get('routing_key'), properties=pika.BasicProperties(content_type=self.content_type, delivery_mode=1, headers=self.headers))
        if self.params.get('src') is not None and self.content_type == 'text/plain':
            ...
            args = dict(body=self._read_file(self.params.get('src')), exchange=self.params.get('exchange'), routing_key=self.params.get('routing_key'), properties=pika.BasicProperties(content_type=self.content_type, delivery_mode=1, headers=self.headers))
        elif self.params.get('src') is not None:
            args = dict(body=self._read_file(self.params.get('src')), exchange=self.params.get('exchange'), routing_key=self.params.get('routing_key'), properties=pika.BasicProperties(content_type=self.content_type, delivery_mode=1, headers=self.headers))
        try:
            if self.queue is None:
                result = self.conn_channel.queue_declare(durable=self.params.get('durable'), exclusive=self.params.get('exclusive'), auto_delete=self.params.get('auto_delete'))
...
def test_RabbitClient_check_required_library():
    ret = RabbitClient().check_required_library()

def test_RabbitClient_check_host_params():
    ret = RabbitClient().check_host_params()

def test_RabbitClient_append_ssl_certs():
    ret = RabbitClient().append_ssl_certs()

def test_RabbitClient_rabbitmq_argument_spec():
    ret = RabbitClient().rabbitmq_argument_spec()

def test_RabbitClient__read_file():
    ret = RabbitClient()._read_file()

def test_RabbitClient__check_file_mime_type():
    ret = RabbitClient()._check_file_mime_type()

def test_RabbitClient_build_url():
    ret = RabbitClient().build_url()

def test_RabbitClient_connect_to_rabbitmq():
    ret = RabbitClient().connect_to_rabbitmq()

def test_RabbitClient_close_connection():
    ret = RabbitClient().close_connection()

def test_RabbitClient_basic_publish():
    ret = RabbitClient().basic_publish()