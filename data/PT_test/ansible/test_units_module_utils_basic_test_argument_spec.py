...
def options_argspec_dict(options_argspec_list):
    options_argspec_list = options_argspec_list()
...
def test_validator_basic_types(argspec, expected, stdin):
    (argspec,  expected,  stdin) = [(s[0], s[2], s[1]) for s in VALID_SPECS][0]
...
def test_validator_fail(stdin, capfd, argspec, expected):
    (argspec,  expected,  stdin) = [(s[0], s[2], s[1]) for s in INVALID_SPECS][0]
...
class TestComplexArgSpecs:
    ...
    def test_complex_required(self, stdin, complex_argspec):
        stdin = {'foo': 'hello'}
        complex_argspec = complex_argspec()
    ...
    def test_fail_mutually_exclusive(self, capfd, stdin, complex_argspec):
        stdin = {'foo': 'hello', 'bar': 'bad', 'bam': 'bad2', 'bing': 'a', 'bang': 'b', 'bong': 'c'}
        complex_argspec = complex_argspec()
    ...
    def test_fail_list_with_choices(self, capfd, mocker, stdin, complex_argspec):
        stdin = {'foo': 'hello', 'zardoz2': ['one', 'four', 'five']}
        complex_argspec = complex_argspec()
    ...
    def test_list_with_choices(self, capfd, mocker, stdin, complex_argspec):
        stdin = {'foo': 'hello', 'zardoz2': ['one', 'three']}
        complex_argspec = complex_argspec()
    ...
    def test_list_with_elements_path(self, capfd, mocker, stdin, complex_argspec):
        stdin = {'foo': 'hello', 'bar3': ['~/test', 'test/']}
        complex_argspec = complex_argspec()
        'Test choices with list'
        am = basic.AnsibleModule(**complex_argspec)
    ...
    def test_list_with_elements_callable_str(self, capfd, mocker, stdin, complex_argspec):
        stdin = {'foo': 'hello', 'bar_str': [867, '5309']}
        complex_argspec = complex_argspec()
        'Test choices with list'
        am = basic.AnsibleModule(**complex_argspec)
...
class TestComplexOptions:
    ...
    def test_options_type_dict(self, stdin, options_argspec_dict, expected):
        (stdin,  expected) = OPTIONS_PARAMS_DICT[0]
        options_argspec_dict = options_argspec_dict()
    ...
    def test_options_type_list(self, stdin, options_argspec_list, expected):
        (stdin,  expected) = OPTIONS_PARAMS_LIST[0]
        options_argspec_list = options_argspec_list()
        'Test that a basic creation with required and required_if works'
    ...
    def test_fail_validate_options_dict(self, capfd, stdin, options_argspec_dict, expected):
        (stdin,  expected) = FAILING_PARAMS_DICT[0]
        options_argspec_dict = options_argspec_dict()
        'Fail because one of a required_together pair of parameters has a default and the other was not specified'
        with pytest.raises(SystemExit):
            am = basic.AnsibleModule(**options_argspec_dict)
    ...
    def test_fail_validate_options_list(self, capfd, stdin, options_argspec_list, expected):
        (stdin,  expected) = FAILING_PARAMS_LIST[0]
        options_argspec_list = options_argspec_list()
        'Fail because one of a required_together pair of parameters has a default and the other was not specified'
        with pytest.raises(SystemExit):
            am = basic.AnsibleModule(**options_argspec_list)
    ...
    def test_fallback_in_option(self, mocker, stdin, options_argspec_dict):
        stdin = {'foobar': {'foo': 'required', 'bam1': 'test', 'bar': 'case'}}
        options_argspec_dict = options_argspec_dict()
        'Test that the complex argspec works if we get a required parameter via fallback'
        environ = os.environ.copy()
        environ['BAZ'] = 'test data'
    ...
    def test_elements_path_in_option(self, mocker, stdin, options_argspec_dict):
        stdin = {'foobar': {'foo': 'required', 'bam1': 'test', 'baz': 'data', 'bar': 'case', 'bar4': '~/test'}}
        options_argspec_dict = options_argspec_dict()
    ...
    def test_subspec_not_required_defaults(self, stdin, spec, expected):
        (stdin, spec, expected) = ({}, {'one': {'type': 'dict', 'apply_defaults': True, 'options': {'two': {'default': True, 'type': 'bool'}}}}, {'two': True})
...
def test_TestComplexArgSpecs_test_complex_required():
    ret = TestComplexArgSpecs().test_complex_required()

def test_TestComplexArgSpecs_test_complex_duplicate_warning():
    ret = TestComplexArgSpecs().test_complex_duplicate_warning()

def test_TestComplexArgSpecs_test_complex_type_fallback():
    ret = TestComplexArgSpecs().test_complex_type_fallback()

def test_TestComplexArgSpecs_test_fail_mutually_exclusive():
    ret = TestComplexArgSpecs().test_fail_mutually_exclusive()

def test_TestComplexArgSpecs_test_fail_required_together():
    ret = TestComplexArgSpecs().test_fail_required_together()

def test_TestComplexArgSpecs_test_fail_required_together_and_default():
    ret = TestComplexArgSpecs().test_fail_required_together_and_default()

def test_TestComplexArgSpecs_test_fail_required_together_and_fallback():
    ret = TestComplexArgSpecs().test_fail_required_together_and_fallback()

def test_TestComplexArgSpecs_test_fail_list_with_choices():
    ret = TestComplexArgSpecs().test_fail_list_with_choices()

def test_TestComplexArgSpecs_test_list_with_choices():
    ret = TestComplexArgSpecs().test_list_with_choices()

def test_TestComplexArgSpecs_test_list_with_elements_path():
    ret = TestComplexArgSpecs().test_list_with_elements_path()

def test_TestComplexArgSpecs_test_deprecated_alias():
    ret = TestComplexArgSpecs().test_deprecated_alias()

def test_TestComplexArgSpecs_test_list_with_elements_callable_str():
    ret = TestComplexArgSpecs().test_list_with_elements_callable_str()

def test_TestComplexOptions_test_options_type_dict():
    ret = TestComplexOptions().test_options_type_dict()

def test_TestComplexOptions_test_options_type_list():
    ret = TestComplexOptions().test_options_type_list()

def test_TestComplexOptions_test_fail_validate_options_dict():
    ret = TestComplexOptions().test_fail_validate_options_dict()

def test_TestComplexOptions_test_fail_validate_options_list():
    ret = TestComplexOptions().test_fail_validate_options_list()

def test_TestComplexOptions_test_fallback_in_option():
    ret = TestComplexOptions().test_fallback_in_option()

def test_TestComplexOptions_test_elements_path_in_option():
    ret = TestComplexOptions().test_elements_path_in_option()

def test_TestComplexOptions_test_subspec_not_required_defaults():
    ret = TestComplexOptions().test_subspec_not_required_defaults()

def test_TestLoadFileCommonArguments_test_smoketest_load_file_common_args():
    ret = TestLoadFileCommonArguments().test_smoketest_load_file_common_args()

def test_TestLoadFileCommonArguments_test_load_file_common_args():
    ret = TestLoadFileCommonArguments().test_load_file_common_args()