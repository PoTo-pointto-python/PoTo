...
from ..util import SubprocessError, display, find_python
...
class ValidateModulesTest(SanitySingleVersion):
    ...
    def test(self, args, targets, python_version):
        ...
        cmd = [python, os.path.join(SANITY_ROOT, 'validate-modules', 'validate-modules'), '--format', 'json', '--arg-spec'] + paths
        ...
        for filename in messages:
            ...
            for item in output['errors']:
                errors.append(SanityMessage(path=filename, line=int(item['line']) if 'line' in item else 0, column=int(item['column']) if 'column' in item else 0, level='error', code='%s' % item['code'], message=item['msg']))
        errors = settings.process_errors(errors, paths)
        if errors:
            return SanityFailure(self.name, messages=errors)
        return SanitySuccess(self.name)

def test_ValidateModulesTest_error_code():
    ret = ValidateModulesTest().error_code()

def test_ValidateModulesTest_filter_targets():
    ret = ValidateModulesTest().filter_targets()

def test_ValidateModulesTest_test():
    ret = ValidateModulesTest().test()