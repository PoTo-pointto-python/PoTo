...
class Certificate(crypto_utils.OpenSSLObject):
    ...
    def __init__(self, module, backend):
        super(Certificate, self).__init__(module.params['path'], module.params['state'], module.params['force'], module.check_mode)
    ...
    def check(self, module, perms_required=True):
        ...
        if self.privatekey_path or self.privatekey_content:
            try:
                self.privatekey = crypto_utils.load_privatekey(path=self.privatekey_path, content=self.privatekey_content, passphrase=self.privatekey_passphrase, backend=self.backend)
            except crypto_utils.OpenSSLBadPassphraseError as exc:
                raise CertificateError(exc)
            if not self._validate_privatekey():
                return False
        if self.csr_path or self.csr_content:
            self.csr = crypto_utils.load_certificate_request(path=self.csr_path, content=self.csr_content, backend=self.backend)
...
class CertificateAbsent(Certificate):
    ...
    def __init__(self, module):
        super(CertificateAbsent, self).__init__(module, 'cryptography')

    def generate(self, module):
        pass

    def dump(self, check_mode=False):
        result = {'changed': self.changed, 'filename': self.path, 'privatekey': self.privatekey_path, 'csr': self.csr_path}
        if self.backup_file:
...
class SelfSignedCertificateCryptography(Certificate):
    ...
    def generate(self, module):
        if self.privatekey_content is None and (not os.path.exists(self.privatekey_path)):
            raise CertificateError('The private key %s does not exist' % self.privatekey_path)
        if self.csr_content is None and (not os.path.exists(self.csr_path)):
    ...
    def dump(self, check_mode=False):
        ...
        if check_mode:
            ...
            result.update({'notBefore': self.cert.not_valid_before.strftime('%Y%m%d%H%M%SZ'), 'notAfter': self.cert.not_valid_after.strftime('%Y%m%d%H%M%SZ'), 'serial_number': self.cert.serial_number})
        return result
...
class SelfSignedCertificate(Certificate):
    ...
    def __init__(self, module):
        ...
        if self.privatekey_content is None and (not os.path.exists(self.privatekey_path)):
            raise CertificateError('The private key file {0} does not exist'.format(self.privatekey_path))
        self.csr = crypto_utils.load_certificate_request(path=self.csr_path, content=self.csr_content)
        try:
    ...
    def dump(self, check_mode=False):
        ...
        if self.return_content:
            ...
            result['certificate'] = content.decode('utf-8') if content else None
        if check_mode:
            result.update({'notBefore': self.notBefore, 'notAfter': self.notAfter, 'serial_number': self.serial_number})
        else:
            result.update({'notBefore': self.cert.get_notBefore(), 'notAfter': self.cert.get_notAfter(), 'serial_number': self.cert.get_serial_number()})
        return result
...
class OwnCACertificateCryptography(Certificate):
    ...
    def __init__(self, module):
        ...
        self.csr = crypto_utils.load_certificate_request(path=self.csr_path, content=self.csr_content, backend=self.backend)
        self.ca_cert = crypto_utils.load_certificate(path=self.ca_cert_path, content=self.ca_cert_content, backend=self.backend)
        try:
            self.ca_private_key = crypto_utils.load_privatekey(path=self.ca_privatekey_path, content=self.ca_privatekey_content, passphrase=self.ca_privatekey_passphrase, backend=self.backend)
        except crypto_utils.OpenSSLBadPassphraseError as exc:
            module.fail_json(msg=str(exc))
    ...
    def generate(self, module):
        if self.ca_cert_content is None and (not os.path.exists(self.ca_cert_path)):
            raise CertificateError('The CA certificate %s does not exist' % self.ca_cert_path)
        if self.ca_privatekey_content is None and (not os.path.exists(self.ca_privatekey_path)):
            raise CertificateError('The CA private key %s does not exist' % self.ca_privatekey_path)
        if self.csr_content is None and (not os.path.exists(self.csr_path)):
            raise CertificateError('The certificate signing request file %s does not exist' % self.csr_path)
        if not self.check(module, perms_required=False) or self.force:
            cert_builder = x509.CertificateBuilder()
    ...
    def dump(self, check_mode=False):
        result = {'changed': self.changed, 'filename': self.path, 'privatekey': self.privatekey_path, 'csr': self.csr_path, 'ca_cert': self.ca_cert_path, 'ca_privatekey': self.ca_privatekey_path}
        if self.backup_file:
            result['backup_file'] = self.backup_file
        if self.return_content:
            content = crypto_utils.load_file_if_exists(self.path, ignore_errors=True)
...
class OwnCACertificate(Certificate):
    ...
    def __init__(self, module):
        ...
        if self.ca_privatekey_content is None and (not os.path.exists(self.ca_privatekey_path)):
            raise CertificateError('The CA private key file {0} does not exist'.format(self.ca_privatekey_path))
        self.csr = crypto_utils.load_certificate_request(path=self.csr_path, content=self.csr_content)
        self.ca_cert = crypto_utils.load_certificate(path=self.ca_cert_path, content=self.ca_cert_content)
        try:
            self.ca_privatekey = crypto_utils.load_privatekey(path=self.ca_privatekey_path, content=self.ca_privatekey_content, passphrase=self.ca_privatekey_passphrase)
        except crypto_utils.OpenSSLBadPassphraseError as exc:
            module.fail_json(msg=str(exc))
...
class AssertOnlyCertificateBase(Certificate):
    ...
    def assertonly(self, module):
        ...
        if self.privatekey_path is not None or self.privatekey_content is not None:
            if not self._validate_privatekey():
                messages.append('Certificate %s and private key %s do not match' % (self.path, self.privatekey_path or '(provided in module options)'))
        if self.csr_path is not None or self.csr_content is not None:
            if not self._validate_csr_signature():
                messages.append('Certificate %s and CSR %s do not match: private key mismatch' % (self.path, self.csr_path or '(provided in module options)'))
            if not self._validate_csr_subject():
                messages.append('Certificate %s and CSR %s do not match: subject mismatch' % (self.path, self.csr_path or '(provided in module options)'))
            if not self._validate_csr_extensions():
                messages.append('Certificate %s and CSR %s do not match: extensions mismatch' % (self.path, self.csr_path or '(provided in module options)'))
        if self.signature_algorithms is not None:
            wrong_alg = self._validate_signature_algorithms()
            if wrong_alg:
                messages.append('Invalid signature algorithm (got %s, expected one of %s)' % (wrong_alg, self.signature_algorithms))
        if self.subject is not None:
            failure = self._validate_subject()
            if failure:
                (dummy, cert_subject) = failure
                messages.append('Invalid subject component (got %s, expected all of %s to be present)' % (cert_subject, self.subject))
        if self.issuer is not None:
            failure = self._validate_issuer()
            if failure:
                (dummy, cert_issuer) = failure
                messages.append('Invalid issuer component (got %s, expected all of %s to be present)' % (cert_issuer, self.issuer))
        if self.has_expired is not None:
            cert_expired = self._validate_has_expired()
            if cert_expired != self.has_expired:
                messages.append('Certificate expiration check failed (certificate expiration is %s, expected %s)' % (cert_expired, self.has_expired))
        if self.version is not None:
            cert_version = self._validate_version()
            if cert_version != self.version:
                messages.append('Invalid certificate version number (got %s, expected %s)' % (cert_version, self.version))
        if self.key_usage is not None:
            ...
            if failure == NO_EXTENSION:
                ...
                messages.append('Invalid keyUsage components (got %s, expected all of %s to be present)' % (cert_key_usage, self.key_usage))
        if self.extended_key_usage is not None:
            ...
            if failure == NO_EXTENSION:
                ...
                messages.append('Invalid extendedKeyUsage component (got %s, expected all of %s to be present)' % (ext_cert_key_usage, self.extended_key_usage))
        if self.subject_alt_name is not None:
            ...
            if failure == NO_EXTENSION:
                ...
                messages.append('Invalid subjectAltName component (got %s, expected all of %s to be present)' % (cert_san, self.subject_alt_name))
        if self.not_before is not None:
            ...
            if cert_not_valid_before != crypto_utils.get_relative_time_option(self.not_before, 'not_before', backend=self.backend):
                messages.append('Invalid not_before component (got %s, expected %s to be present)' % (cert_not_valid_before, self.not_before))
        if self.not_after is not None:
            ...
            if cert_not_valid_after != crypto_utils.get_relative_time_option(self.not_after, 'not_after', backend=self.backend):
                messages.append('Invalid not_after component (got %s, expected %s to be present)' % (cert_not_valid_after, self.not_after))
        if self.valid_at is not None:
            (not_before, valid_at, not_after) = self._validate_valid_at()
            if not not_before <= valid_at <= not_after:
                messages.append('Certificate is not valid for the specified date (%s) - not_before: %s - not_after: %s' % (self.valid_at, not_before, not_after))
        if self.invalid_at is not None:
            (not_before, invalid_at, not_after) = self._validate_invalid_at()
            if not_before <= invalid_at <= not_after:
                messages.append('Certificate is not invalid for the specified date (%s) - not_before: %s - not_after: %s' % (self.invalid_at, not_before, not_after))
        if self.valid_in is not None:
            (not_before, valid_in, not_after) = self._validate_valid_in()
            if not not_before <= valid_in <= not_after:
                messages.append('Certificate is not valid in %s from now (that would be %s) - not_before: %s - not_after: %s' % (self.valid_in, valid_in, not_before, not_after))
        return messages
...
class AssertOnlyCertificateCryptography(AssertOnlyCertificateBase):
    ...
    def _validate_subject(self):
        expected_subject = Name([NameAttribute(oid=crypto_utils.cryptography_name_to_oid(sub[0]), value=to_text(sub[1])) for sub in self.subject])
    ...
    def _validate_issuer(self):
        expected_issuer = Name([NameAttribute(oid=crypto_utils.cryptography_name_to_oid(iss[0]), value=to_text(iss[1])) for iss in self.issuer])
    ...
    def _validate_key_usage(self):
        try:
            ...
            test_key_usage = dict(digital_signature=current_key_usage.digital_signature, content_commitment=current_key_usage.content_commitment, key_encipherment=current_key_usage.key_encipherment, data_encipherment=current_key_usage.data_encipherment, key_agreement=current_key_usage.key_agreement, key_cert_sign=current_key_usage.key_cert_sign, crl_sign=current_key_usage.crl_sign, encipher_only=False, decipher_only=False)
            if test_key_usage['key_agreement']:
                test_key_usage.update(dict(encipher_only=current_key_usage.encipher_only, decipher_only=current_key_usage.decipher_only))
...
class AssertOnlyCertificate(AssertOnlyCertificateBase):
    ...
    def __init__(self, module):
        ...
        for param in ['signature_algorithms', 'key_usage', 'extended_key_usage', 'subject_alt_name', 'subject', 'issuer', 'not_before', 'not_after', 'valid_at', 'invalid_at']:
    ...
    def _validate_extended_key_usage(self):
        ...
        for extension_idx in range(0, self.cert.get_extension_count()):
            ...
            if extension.get_short_name() == b'extendedKeyUsage':
                ...
                current_xku = [OpenSSL._util.lib.OBJ_txt2nid(usage.strip()) for usage in to_bytes(extension, errors='surrogate_or_strict').split(b',')]
    ...
    def _validate_subject_alt_name(self):
        ...
        for extension_idx in range(0, self.cert.get_extension_count()):
            ...
            if extension.get_short_name() == b'subjectAltName':
                found = True
                l_altnames = [self._normalize_san(altname.strip()) for altname in to_text(extension, errors='surrogate_or_strict').split(', ')]
...
class EntrustCertificate(Certificate):
    ...
    def __init__(self, module, backend):
        ...
        if self.backend == 'pyopenssl':
            ...
            for (k, v) in csr_subject_components:
                if k.upper() == 'O':
                    if self.csr_org is not None:
                        module.fail_json(msg="Entrust provider does not currently support multiple validated organizations. Multiple organizations found in Subject DN: '{0}'. ".format(csr_subject))
                    else:
                        self.csr_org = v
        elif self.backend == 'cryptography':
            ...
            if len(csr_subject_orgs) == 1:
                ...
                module.fail_json(msg="Entrust provider does not currently support multiple validated organizations. Multiple organizations found in Subject DN: '{0}'. ".format(self.csr.subject))
    ...
    def generate(self, module):
        if not self.check(module, perms_required=False) or self.force:
            ...
            body['tracking'] = {'requesterName': module.params['entrust_requester_name'], 'requesterEmail': module.params['entrust_requester_email'], 'requesterPhone': module.params['entrust_requester_phone']}
            try:
...
class AcmeCertificate(Certificate):
            ...
            if self.csr_content is not None:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    module.fail_json(msg='failed to create temporary CSR file: %s' % to_native(err), exception=traceback.format_exc())
                f.close()
    ...
    def dump(self, check_mode=False):
        result = {'changed': self.changed, 'filename': self.path, 'privatekey': self.privatekey_path, 'accountkey': self.accountkey_path, 'csr': self.csr_path}
        if self.backup_file:
            result['backup_file'] = self.backup_file
        if self.return_content:
            content = crypto_utils.load_file_if_exists(self.path, ignore_errors=True)
    ...
    try:
        if module.params['state'] == 'absent':
            ...
            if provider == 'assertonly':
                module.deprecate("The 'assertonly' provider is deprecated; please see the examples of the 'openssl_certificate' module on how to replace it with other modules", version='2.13', collection_name='ansible.builtin')
            ...
            if backend == 'pyopenssl':
                ...
                if module.params['selfsigned_version'] == 2 or module.params['ownca_version'] == 2:
                    module.fail_json(msg='The cryptography backend does not support v2 certificates, use select_crypto_backend=pyopenssl for v2 certificates')
                if provider == 'selfsigned':
...
def test_Certificate__validate_privatekey():
    ret = Certificate()._validate_privatekey()

def test_Certificate__validate_csr():
    ret = Certificate()._validate_csr()

def test_Certificate_remove():
    ret = Certificate().remove()

def test_Certificate_check():
    ret = Certificate().check()

def test_CertificateAbsent_generate():
    ret = CertificateAbsent().generate()

def test_CertificateAbsent_dump():
    ret = CertificateAbsent().dump()

def test_SelfSignedCertificateCryptography_generate():
    ret = SelfSignedCertificateCryptography().generate()

def test_SelfSignedCertificateCryptography_dump():
    ret = SelfSignedCertificateCryptography().dump()

def test_SelfSignedCertificate_generate():
    ret = SelfSignedCertificate().generate()

def test_SelfSignedCertificate_dump():
    ret = SelfSignedCertificate().dump()

def test_OwnCACertificateCryptography_generate():
    ret = OwnCACertificateCryptography().generate()

def test_OwnCACertificateCryptography_check():
    ret = OwnCACertificateCryptography().check()

def test_OwnCACertificateCryptography_dump():
    ret = OwnCACertificateCryptography().dump()

def test_OwnCACertificate_generate():
    ret = OwnCACertificate().generate()

def test_OwnCACertificate_dump():
    ret = OwnCACertificate().dump()

def test_AssertOnlyCertificateBase__validate_privatekey():
    ret = AssertOnlyCertificateBase()._validate_privatekey()

def test_AssertOnlyCertificateBase__validate_csr_signature():
    ret = AssertOnlyCertificateBase()._validate_csr_signature()

def test_AssertOnlyCertificateBase__validate_csr_subject():
    ret = AssertOnlyCertificateBase()._validate_csr_subject()

def test_AssertOnlyCertificateBase__validate_csr_extensions():
    ret = AssertOnlyCertificateBase()._validate_csr_extensions()

def test_AssertOnlyCertificateBase__validate_signature_algorithms():
    ret = AssertOnlyCertificateBase()._validate_signature_algorithms()

def test_AssertOnlyCertificateBase__validate_subject():
    ret = AssertOnlyCertificateBase()._validate_subject()

def test_AssertOnlyCertificateBase__validate_issuer():
    ret = AssertOnlyCertificateBase()._validate_issuer()

def test_AssertOnlyCertificateBase__validate_has_expired():
    ret = AssertOnlyCertificateBase()._validate_has_expired()

def test_AssertOnlyCertificateBase__validate_version():
    ret = AssertOnlyCertificateBase()._validate_version()

def test_AssertOnlyCertificateBase__validate_key_usage():
    ret = AssertOnlyCertificateBase()._validate_key_usage()

def test_AssertOnlyCertificateBase__validate_extended_key_usage():
    ret = AssertOnlyCertificateBase()._validate_extended_key_usage()

def test_AssertOnlyCertificateBase__validate_subject_alt_name():
    ret = AssertOnlyCertificateBase()._validate_subject_alt_name()

def test_AssertOnlyCertificateBase__validate_not_before():
    ret = AssertOnlyCertificateBase()._validate_not_before()

def test_AssertOnlyCertificateBase__validate_not_after():
    ret = AssertOnlyCertificateBase()._validate_not_after()

def test_AssertOnlyCertificateBase__validate_valid_at():
    ret = AssertOnlyCertificateBase()._validate_valid_at()

def test_AssertOnlyCertificateBase__validate_invalid_at():
    ret = AssertOnlyCertificateBase()._validate_invalid_at()

def test_AssertOnlyCertificateBase__validate_valid_in():
    ret = AssertOnlyCertificateBase()._validate_valid_in()

def test_AssertOnlyCertificateBase_assertonly():
    ret = AssertOnlyCertificateBase().assertonly()

def test_AssertOnlyCertificateBase_generate():
    ret = AssertOnlyCertificateBase().generate()

def test_AssertOnlyCertificateBase_check():
    ret = AssertOnlyCertificateBase().check()

def test_AssertOnlyCertificateBase_dump():
    ret = AssertOnlyCertificateBase().dump()

def test_AssertOnlyCertificateCryptography__validate_privatekey():
    ret = AssertOnlyCertificateCryptography()._validate_privatekey()

def test_AssertOnlyCertificateCryptography__validate_csr_signature():
    ret = AssertOnlyCertificateCryptography()._validate_csr_signature()

def test_AssertOnlyCertificateCryptography__validate_csr_subject():
    ret = AssertOnlyCertificateCryptography()._validate_csr_subject()

def test_AssertOnlyCertificateCryptography__validate_csr_extensions():
    ret = AssertOnlyCertificateCryptography()._validate_csr_extensions()

def test_AssertOnlyCertificateCryptography__validate_signature_algorithms():
    ret = AssertOnlyCertificateCryptography()._validate_signature_algorithms()

def test_AssertOnlyCertificateCryptography__validate_subject():
    ret = AssertOnlyCertificateCryptography()._validate_subject()

def test_AssertOnlyCertificateCryptography__validate_issuer():
    ret = AssertOnlyCertificateCryptography()._validate_issuer()

def test_AssertOnlyCertificateCryptography__validate_has_expired():
    ret = AssertOnlyCertificateCryptography()._validate_has_expired()

def test_AssertOnlyCertificateCryptography__validate_version():
    ret = AssertOnlyCertificateCryptography()._validate_version()

def test_AssertOnlyCertificateCryptography__validate_key_usage():
    ret = AssertOnlyCertificateCryptography()._validate_key_usage()

def test_AssertOnlyCertificateCryptography__validate_extended_key_usage():
    ret = AssertOnlyCertificateCryptography()._validate_extended_key_usage()

def test_AssertOnlyCertificateCryptography__validate_subject_alt_name():
    ret = AssertOnlyCertificateCryptography()._validate_subject_alt_name()

def test_AssertOnlyCertificateCryptography__validate_not_before():
    ret = AssertOnlyCertificateCryptography()._validate_not_before()

def test_AssertOnlyCertificateCryptography__validate_not_after():
    ret = AssertOnlyCertificateCryptography()._validate_not_after()

def test_AssertOnlyCertificateCryptography__validate_valid_at():
    ret = AssertOnlyCertificateCryptography()._validate_valid_at()

def test_AssertOnlyCertificateCryptography__validate_invalid_at():
    ret = AssertOnlyCertificateCryptography()._validate_invalid_at()

def test_AssertOnlyCertificateCryptography__validate_valid_in():
    ret = AssertOnlyCertificateCryptography()._validate_valid_in()

def test_AssertOnlyCertificate__validate_privatekey():
    ret = AssertOnlyCertificate()._validate_privatekey()

def test_AssertOnlyCertificate__validate_csr_signature():
    ret = AssertOnlyCertificate()._validate_csr_signature()

def test_AssertOnlyCertificate__validate_csr_subject():
    ret = AssertOnlyCertificate()._validate_csr_subject()

def test_AssertOnlyCertificate__validate_csr_extensions():
    ret = AssertOnlyCertificate()._validate_csr_extensions()

def test_AssertOnlyCertificate__validate_signature_algorithms():
    ret = AssertOnlyCertificate()._validate_signature_algorithms()

def test_AssertOnlyCertificate__validate_subject():
    ret = AssertOnlyCertificate()._validate_subject()

def test_AssertOnlyCertificate__validate_issuer():
    ret = AssertOnlyCertificate()._validate_issuer()

def test_AssertOnlyCertificate__validate_has_expired():
    ret = AssertOnlyCertificate()._validate_has_expired()

def test_AssertOnlyCertificate__validate_version():
    ret = AssertOnlyCertificate()._validate_version()

def test_AssertOnlyCertificate__validate_key_usage():
    ret = AssertOnlyCertificate()._validate_key_usage()

def test_AssertOnlyCertificate__validate_extended_key_usage():
    ret = AssertOnlyCertificate()._validate_extended_key_usage()

def test_AssertOnlyCertificate__normalize_san():
    ret = AssertOnlyCertificate()._normalize_san()

def test_AssertOnlyCertificate__validate_subject_alt_name():
    ret = AssertOnlyCertificate()._validate_subject_alt_name()

def test_AssertOnlyCertificate__validate_not_before():
    ret = AssertOnlyCertificate()._validate_not_before()

def test_AssertOnlyCertificate__validate_not_after():
    ret = AssertOnlyCertificate()._validate_not_after()

def test_AssertOnlyCertificate__validate_valid_at():
    ret = AssertOnlyCertificate()._validate_valid_at()

def test_AssertOnlyCertificate__validate_invalid_at():
    ret = AssertOnlyCertificate()._validate_invalid_at()

def test_AssertOnlyCertificate__validate_valid_in():
    ret = AssertOnlyCertificate()._validate_valid_in()

def test_EntrustCertificate_generate():
    ret = EntrustCertificate().generate()

def test_EntrustCertificate_check():
    ret = EntrustCertificate().check()

def test_EntrustCertificate__get_cert_details():
    ret = EntrustCertificate()._get_cert_details()

def test_EntrustCertificate_dump():
    ret = EntrustCertificate().dump()

def test_AcmeCertificate_generate():
    ret = AcmeCertificate().generate()

def test_AcmeCertificate_dump():
    ret = AcmeCertificate().dump()