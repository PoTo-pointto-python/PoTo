...
from ..sanity import SanitySingleVersion, SanityMessage, SanityFailure, SanitySuccess, SANITY_ROOT
from ..target import TestTarget
from ..util import SubprocessError, display, ConfigParser, is_subdir, find_python
from ..util_common import run_command
from ..ansible_util import ansible_environment, get_collection_detail, CollectionDetail, CollectionDetailError
...
class PylintTest(SanitySingleVersion):
    ...
    @staticmethod
    def pylint(args, context, paths, plugin_dir, plugin_names, python, collection_detail): # type: (...) -> t.List[t.Dict[str, str]]
        ...
        cmd = [python, '-m', 'pylint', '--jobs', '0', '--reports', 'n', '--max-line-length', '160', '--max-complexity', '20', '--rcfile', rcfile, '--output-format', 'json', '--load-plugins', ','.join(load_plugins)] + paths
        if data_context().content.collection:
...
def test_PylintTest_error_code():
    ret = PylintTest().error_code()

def test_PylintTest_filter_targets():
    ret = PylintTest().filter_targets()

def test_PylintTest_add_context():
    ret = PylintTest().add_context()

def test_PylintTest_context_filter():
    ret = PylintTest().context_filter()

def test_PylintTest_filter_path():
    ret = PylintTest().filter_path()

def test_PylintTest_test():
    ret = PylintTest().test()

def test_PylintTest_pylint():
    ret = PylintTest().pylint()