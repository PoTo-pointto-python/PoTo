...
class AnsibleAWSModule(object):
    ...
    def __init__(self, **kwargs):
        ...
        if local_settings['check_boto3'] and (not HAS_BOTO3):
            self._module.fail_json(msg=missing_required_lib('botocore or boto3'))
    ...
    def client(self, service, retry_decorator=None):
        ...
        conn = boto3_conn(self, conn_type='client', resource=service, region=region, endpoint=ec2_url, **aws_connect_kwargs)
        return conn if retry_decorator is None else _RetryingBotoClientWrapper(conn, retry_decorator)

    def resource(self, service):
        (region, ec2_url, aws_connect_kwargs) = get_aws_connection_info(self, boto3=True)
        return boto3_conn(self, conn_type='resource', resource=service, region=region, endpoint=ec2_url, **aws_connect_kwargs)
...
def test_AnsibleAWSModule_params():
    ret = AnsibleAWSModule().params()

def test_AnsibleAWSModule__get_resource_action_list():
    ret = AnsibleAWSModule()._get_resource_action_list()

def test_AnsibleAWSModule_exit_json():
    ret = AnsibleAWSModule().exit_json()

def test_AnsibleAWSModule_fail_json():
    ret = AnsibleAWSModule().fail_json()

def test_AnsibleAWSModule_debug():
    ret = AnsibleAWSModule().debug()

def test_AnsibleAWSModule_warn():
    ret = AnsibleAWSModule().warn()

def test_AnsibleAWSModule_deprecate():
    ret = AnsibleAWSModule().deprecate()

def test_AnsibleAWSModule_boolean():
    ret = AnsibleAWSModule().boolean()

def test_AnsibleAWSModule_md5():
    ret = AnsibleAWSModule().md5()

def test_AnsibleAWSModule_client():
    ret = AnsibleAWSModule().client()

def test_AnsibleAWSModule_resource():
    ret = AnsibleAWSModule().resource()

def test_AnsibleAWSModule_region():
    ret = AnsibleAWSModule().region()

def test_AnsibleAWSModule_fail_json_aws():
    ret = AnsibleAWSModule().fail_json_aws()

def test_AnsibleAWSModule__gather_versions():
    ret = AnsibleAWSModule()._gather_versions()

def test_AnsibleAWSModule_boto3_at_least():
    ret = AnsibleAWSModule().boto3_at_least()

def test_AnsibleAWSModule_botocore_at_least():
    ret = AnsibleAWSModule().botocore_at_least()

def test__RetryingBotoClientWrapper_deciding_wrapper():
    ret = _RetryingBotoClientWrapper().deciding_wrapper()

def test__RetryingBotoClientWrapper__create_optional_retry_wrapper_function():
    ret = _RetryingBotoClientWrapper()._create_optional_retry_wrapper_function()

def test__RetryingBotoClientWrapper___getattr__():
    ret = _RetryingBotoClientWrapper().__getattr__()