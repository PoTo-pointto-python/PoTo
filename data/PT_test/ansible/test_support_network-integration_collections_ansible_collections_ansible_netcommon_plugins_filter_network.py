...
def type5_pw(password, salt=None):
    ...
    salt_chars = u''.join((to_text(string.ascii_letters), to_text(string.digits), u'./'))
    ...
    for vlans in parse_list:
        if line_count == 0:
            if len(result[line_count] + vlans) > first_line_len:
                ...
                result[line_count] += vlans + ','
        elif len(result[line_count] + vlans) > other_line_len:
            result.append('')
            line_count += 1
            result[line_count] += vlans + ','
        else:
...
class FilterModule(object):
    ...
    filter_map = {'parse_cli': parse_cli, 'parse_cli_textfsm': parse_cli_textfsm, 'parse_xml': parse_xml, 'type5_pw': type5_pw, 'hash_salt': hash_salt, 'comp_type5': comp_type5, 'vlan_parser': vlan_parser}
