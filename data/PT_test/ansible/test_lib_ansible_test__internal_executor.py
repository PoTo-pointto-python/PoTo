...
from .core_ci import AnsibleCoreCI, SshKey
from .manage_ci import ManageWindowsCI, ManageNetworkCI
from .cloud import cloud_filter, cloud_init, get_cloud_environment, get_cloud_platforms, CloudEnvironmentConfig
from .io import make_dirs, open_text_file, read_binary_file, read_text_file, write_text_file
from .util import ApplicationWarning, ApplicationError, SubprocessError, display, remove_tree, find_executable, raw_command, get_available_port, generate_pip_command, find_python, cmd_quote, ANSIBLE_LIB_ROOT, ANSIBLE_TEST_DATA_ROOT, ANSIBLE_TEST_CONFIG_ROOT, get_ansible_version, tempdir, open_zipfile, SUPPORTED_PYTHON_VERSIONS, str_to_version
from .util_common import get_docker_completion, get_network_settings, get_remote_completion, get_python_path, intercept_command, named_temporary_file, run_command, write_json_test_results, ResultType, handle_layout_messages
from .docker_util import docker_pull, docker_run, docker_available, docker_rm, get_docker_container_id, get_docker_container_ip
from .ansible_util import ansible_environment, check_pyyaml
from .target import IntegrationTarget, walk_internal_targets, walk_posix_integration_targets, walk_network_integration_targets, walk_windows_integration_targets, TIntegrationTarget
from .ci import get_ci_provider
from .classification import categorize_changes
from .config import TestConfig, EnvironmentConfig, IntegrationConfig, NetworkIntegrationConfig, PosixIntegrationConfig, ShellConfig, WindowsIntegrationConfig, TIntegrationConfig
from .metadata import ChangeDescription
from .integration import integration_test_environment, integration_test_config_file, setup_common_temp_dir, get_inventory_relative_path, check_inventory, delegate_inventory
from .data import data_context
HTTPTESTER_HOSTS = ('ansible.http.tests', 'sni1.ansible.http.tests', 'fail.ansible.http.tests')
...
def check_legacy_modules():
    ...
    for directory in ('core', 'extras'):
        ...
        for (root, _dir_names, file_names) in os.walk(path):
            if file_names:
                raise ApplicationError('Files prohibited in "%s". These are most likely legacy modules from version 2.2 or earlier.' % root)
...
def install_command_requirements(args, python_version=None, context=None, enable_pyyaml_check=False):
    ...
    if args.command != 'sanity':
        ...
        run_command(args, generate_pip_install(pip, '', packages=[get_cryptography_requirement(args, python_version)], constraints=os.path.join(ANSIBLE_TEST_DATA_ROOT, 'cryptography-constraints.txt')))
...
def network_inventory(remotes):
    ...
    for remote in remotes:
        options = dict(ansible_host=remote.connection.hostname, ansible_user=remote.connection.username, ansible_ssh_private_key_file=os.path.abspath(remote.ssh_key.key))
        settings = get_network_settings(remote.args, remote.platform, remote.version)
        options.update(settings.inventory_vars)
        groups[remote.platform].append('%s %s' % (remote.name.replace('.', '-'), ' '.join(('%s="%s"' % (k, options[k]) for k in sorted(options)))))
...
def command_windows_integration(args):
        ...
        if use_httptester and (not docker_available()) and (not docker_httptester):
            ...
            def forward_ssh_ports(target):
                ...
                for remote in [r for r in remotes if r.version != '2008']:
                    ...
                    script = 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\\%s -Hosts "%s"' % (watcher_path, '|'.join(HTTPTESTER_HOSTS))
                    if args.verbosity > 3:
                        script += ' -Verbose'
    ...
    success = False
    try:
        command_integration_filtered(args, internal_targets, all_targets, inventory_path, pre_target=pre_target, post_target=post_target, remote_temp_path=remote_temp_path)
        success = True
    finally:
        if httptester_id:
...
def windows_init(args, internal_targets):
    """
    :type args: WindowsIntegrationConfig
...
def windows_inventory(remotes):
    ...
    for remote in remotes:
        ...
        if remote.name == 'windows-2008':
            options.update(ansible_connection='psrp', ansible_psrp_auth='basic', ansible_psrp_cert_validation='ignore')
        elif remote.name == 'windows-2016':
            options.update(ansible_connection='winrm', ansible_winrm_server_cert_validation='ignore', ansible_winrm_transport='ntlm', ansible_winrm_scheme='http', ansible_port='5985')
        else:
            options.update(ansible_connection='winrm', ansible_winrm_server_cert_validation='ignore')
...
def command_integration_filter(args, targets, init_callback=None): # type: (...) -> t.Tuple[TIntegrationTarget, ...]
...
def command_integration_filtered(args, targets, all_targets, inventory_path, pre_target=None, post_target=None, remote_temp_path=None):
    """
    ...
    try:
                    ...
                    try:
                        ...
                        try:
                            if target.script_path:
                                command_integration_script(args, target, test_dir, inventory_path, common_temp_path, remote_temp_path=remote_temp_path)
                            else:
                                command_integration_role(args, target, start_at_task, test_dir, inventory_path, common_temp_path, remote_temp_path=remote_temp_path)
                                start_at_task = None
                        finally:
                            if post_target:
                                post_target(target)
        ...
        if not args.explain:
            ...
            result_name = '%s-%s.json' % (args.command, re.sub('[^0-9]', '-', str(datetime.datetime.utcnow().replace(microsecond=0))))
...
def start_httptester(args):
    ...
    :rtype: str, list[str]
    """
    ports = [dict(remote=8080, container=80), dict(remote=8443, container=443)]
...
def integration_environment(args, target, test_dir, inventory_path, ansible_config, env_config):
    ...
    integration = dict(JUNIT_OUTPUT_DIR=ResultType.JUNIT.path, ANSIBLE_CALLBACK_WHITELIST=','.join(sorted(set(callback_plugins))), ANSIBLE_TEST_CI=args.metadata.ci_provider or get_ci_provider().code, ANSIBLE_TEST_COVERAGE='check' if args.coverage_check else 'yes' if args.coverage else '', OUTPUT_DIR=test_dir, INVENTORY_PATH=os.path.abspath(inventory_path))
...
def command_integration_script(args, target, test_dir, inventory_path, temp_path, remote_temp_path=None):
    ...
    with integration_test_environment(args, target, inventory_path) as test_env:
        ...
        env.update(dict(ANSIBLE_PLAYBOOK_DIR=cwd))
        ...
        with integration_test_config_file(args, env_config, test_env.integration_dir) as config_path:
            ...
            intercept_command(args, cmd, target_name=target.name, env=env, cwd=cwd, temp_path=temp_path, remote_temp_path=remote_temp_path, module_coverage=module_coverage)
...
def common_integration_filter(args, targets, exclude):
    ...
    if not args.allow_disabled:
        ...
        if skipped:
            ...
            display.warning('Excluding tests marked "%s" which require --allow-disabled or prefixing with "disabled/": %s' % (skip.rstrip('/'), ', '.join(skipped)))
    ...
    if not args.allow_unsupported:
        ...
        if skipped:
            exclude.extend(skipped)
            display.warning('Excluding tests marked "%s" which require --allow-unsupported or prefixing with "unsupported/": %s' % (skip.rstrip('/'), ', '.join(skipped)))
    ...
    if not args.allow_unstable:
        ...
        if skipped:
            exclude.extend(skipped)
            display.warning('Excluding tests marked "%s" which require --allow-unstable or prefixing with "unstable/": %s' % (skip.rstrip('/'), ', '.join(skipped)))
    if isinstance(args, WindowsIntegrationConfig) and args.windows:
        ...
        if all_skipped:
            ...
            display.warning('Excluding tests marked "%s" which are set to skip with --windows %s: %s' % ('", "'.join(skip_aliases), ', '.join(args.windows), ', '.join(all_skipped)))
        if not_skipped:
            for (target, skip_valid, skip_missing) in not_skipped:
                display.warning('Including test "%s" which was marked to skip for --windows %s but not %s.' % (target, ', '.join(skip_valid), ', '.join(skip_missing)))
...
def get_integration_local_filter(args, targets):
    ...
    if not args.allow_root and os.getuid() != 0:
        ...
        if skipped:
            exclude.append(skip)
            display.warning('Excluding tests marked "%s" which require --allow-root or running as root: %s' % (skip.rstrip('/'), ', '.join(skipped)))
    ...
    if not args.allow_destructive:
        ...
        if skipped:
            exclude.extend(skipped)
            display.warning('Excluding tests marked "%s" which require --allow-destructive or prefixing with "destructive/" to run locally: %s' % (skip.rstrip('/'), ', '.join(skipped)))
...
def get_integration_remote_filter(args, targets):
    ...
    exclude = []
    common_integration_filter(args, targets, exclude)
    skips = {'skip/%s' % remote.platform: remote.platform, 'skip/%s/%s' % (remote.platform, remote.version): '%s %s' % (remote.platform, remote.version), 'skip/%s%s' % (remote.platform, remote.version): '%s %s' % (remote.platform, remote.version)}
    if remote.arch:
        skips.update({'skip/%s/%s' % (remote.arch, remote.platform): '%s on %s' % (remote.platform, remote.arch), 'skip/%s/%s/%s' % (remote.arch, remote.platform, remote.version): '%s %s on %s' % (remote.platform, remote.version, remote.arch)})
...
def exclude_targets_by_python_version(targets, python_version, exclude):
    ...
    if skipped:
        exclude.append(skip)
        display.warning('Excluding tests marked "%s" which are not supported on python %s: %s' % (skip.rstrip('/'), python_version, ', '.join(skipped)))
...
def get_python_version(args, configs, name):
    ...
    if not config or not config_python:
        if args.python:
            return args.python
        display.warning('No Python version specified. Use completion config or the --python option to specify one.', unique=True)
        return ''
    ...
    if args.python and args.python not in supported_python_versions:
        raise ApplicationError('Python %s is not supported by %s. Supported Python version(s) are: %s' % (args.python, name, ', '.join(sorted(supported_python_versions))))
...
def get_python_interpreter(args, configs, name):
    ...
    if not config:
        if args.python:
            ...
            guess = 'python'
        display.warning('Using "%s" as the Python interpreter. Use completion config or the --python-interpreter option to specify the path.' % guess, unique=True)
        return guess
...
class EnvironmentDescription:
    ...
    def __init__(self, args):
        ...
        self.data = dict(python_paths=python_paths, pip_paths=pip_paths, program_versions=program_versions, pip_interpreters=pip_interpreters, known_hosts_hash=known_hosts_hash, warnings=warnings)
    ...
    def check_python_pip_association(version, python_paths, pip_paths, pip_interpreters, warnings):
        ...
        match = re.search('#!\\s*(?P<command>[^\\s]+)', pip_shebang)
        if not match:
        ...
        if identical:
            return
        warnings.append('A %s pip was found at "%s", but it uses interpreter "%s" instead of "%s".' % (python_label, pip_path, pip_interpreter, python_interpreter))
    ...
    def check(original, current, target_name, throw):
        ...
        if original_json == current_json:
            return True
        unified_diff = '\n'.join(difflib.unified_diff(a=original_json.splitlines(), b=current_json.splitlines(), fromfile='original.json', tofile='current.json', lineterm=''))
        message = 'Test target "%s" has changed the test environment!\nIf these changes are necessary, they must be reverted before the test finishes.\n>>> Original Environment\n%s\n>>> Current Environment\n%s\n>>> Environment Diff\n%s' % (target_name, original_json, current_json, unified_diff)
        if throw:
...
def test_EnvironmentDescription_check_python_pip_association():
    ret = EnvironmentDescription().check_python_pip_association()

def test_EnvironmentDescription___str__():
    ret = EnvironmentDescription().__str__()

def test_EnvironmentDescription_validate():
    ret = EnvironmentDescription().validate()

def test_EnvironmentDescription_check():
    ret = EnvironmentDescription().check()

def test_EnvironmentDescription_get_version():
    ret = EnvironmentDescription().get_version()

def test_EnvironmentDescription_get_shebang():
    ret = EnvironmentDescription().get_shebang()

def test_EnvironmentDescription_get_hash():
    ret = EnvironmentDescription().get_hash()