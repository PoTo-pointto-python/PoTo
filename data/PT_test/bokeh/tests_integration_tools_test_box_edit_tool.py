...
def Test_BoxEditTool_test_selected_by_default(self, single_plot_page) -> None:
...
def Test_BoxEditTool_test_can_be_deselected_and_selected(self, single_plot_page) -> None:
    plot = _make_plot('both')
    page = single_plot_page(plot)
    button = page.get_toolbar_button('box-edit')
    assert 'active' in button.get_attribute('class')
...
def Test_BoxEditTool_test_double_click_triggers_draw(self, single_plot_page) -> None:
    plot = _make_plot('both')
    page = single_plot_page(plot)
...
def Test_BoxEditTool_test_shift_drag_triggers_draw(self, single_plot_page) -> None:
    plot = _make_plot('both')
    page = single_plot_page(plot)
...
def Test_BoxEditTool_test_drag_moves_box(self, single_plot_page) -> None:
    plot = _make_plot('both')
    page = single_plot_page(plot)
    page.double_click_canvas_at_position(100, 100)
    time.sleep(0.5)
...
def Test_BoxEditTool_test_backspace_deletes_drawn_box(self, single_plot_page) -> None:
...
def Test_BoxEditTool_test_num_objects_limits_drawn_boxes(self, single_plot_page) -> None:
    plot = _make_plot('both', num_objects=2)
    page = single_plot_page(plot)
    page.drag_canvas_at_position(100, 100, 50, 50, mod='\ue008')
    time.sleep(0.5)
...
@flaky(max_runs=10)
def Test_BoxEditTool_test_box_draw_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [1, 2, 1.2162162162162162], 'y': [1, 1, 1.875], 'width': [0.5, 0.5, 0.8108108108108109], 'height': [0.5, 0.5, 0.75]}
...
@flaky(max_runs=10)
def Test_BoxEditTool_test_box_drag_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [1, 2, 1.6216216216216217], 'y': [1, 1, 1.5000000000000002], 'width': [0.5, 0.5, 0.8108108108108109], 'height': [0.5, 0.5, 0.75]}
    page = bokeh_server_page(_make_server_plot(expected))
    page.double_click_canvas_at_position(100, 100)
    time.sleep(0.5)
...
@flaky(max_runs=10)
def Test_BoxEditTool_test_box_delete_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [2], 'y': [1], 'width': [0.5], 'height': [0.5]}
