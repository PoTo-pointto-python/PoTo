...
def test_common_decode_json() -> None:
    for (event_name, event_cls) in events._CONCRETE_EVENT_CLASSES.items():
        if event_name is None:
            continue
        event = events.Event.decode_json({'event_name': event_cls.event_name, 'event_values': {'model': {'id': 'test-model-id'}}})
...
def test_panevent_decode_json() -> None:
    event_values = dict(model={'id': 'test-model-id'}, delta_x=0.1, delta_y=0.3, sx=3, sy=-2, x=10, y=100)
...
def EventCallback___init__(self, attributes=[]):
    self.event_name = None
    self.attributes = attributes
    self.payload = {}

def EventCallback___call__(self, event):
