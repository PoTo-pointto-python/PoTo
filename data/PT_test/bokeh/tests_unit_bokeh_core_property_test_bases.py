...
def TestProperty_test_is_valid_supresses_validation_detail(self, mock_validate) -> None:
    p = bcpb.Property()
    p.is_valid(None)
    assert mock_validate.called
    assert mock_validate.call_args[0] == (None, False)

def TestProperty_test_serialized_default(self) -> None:
    p = bcpb.Property()
...
def TestProperty_test_assert_bools(self) -> None:
    hp = HasProps()
    p = bcpb.Property()
    p.asserts(True, 'true')
...
def TestProperty_test_assert_functions(self) -> None:
    hp = HasProps()
    p = bcpb.Property()
...
def TestProperty_test_assert_msg_funcs(self) -> None:
    hp = HasProps()
    p = bcpb.Property()
...
def TestProperty_test_matches_basic_types(self, capsys) -> None:
    p = bcpb.Property()
...
def TestProperty_test_matches_compatible_arrays(self, capsys) -> None:
    p = bcpb.Property()
    a = np.arange(5)
    b = np.arange(5)
...
def TestProperty_test_matches_incompatible_arrays(self, capsys) -> None:
    p = bcpb.Property()
    a = np.arange(5)
...
def TestProperty_test_matches_dicts_with_array_values(self, capsys) -> None:
    p = bcpb.Property()
...
def TestProperty_test_matches_non_dict_containers_with_array_false(self, capsys) -> None:
    p = bcpb.Property()
    d1 = [np.arange(10)]
    d2 = [np.arange(10)]
    assert p.matches(d1, d1) is True
    ...
    t1 = (np.arange(10),)
    t2 = (np.arange(10),)
    assert p.matches(t1, t1) is True
    assert p.matches(t1, t2) is False
    (out, err) = capsys.readouterr()
    assert err == ''

def TestProperty_test_matches_dicts_with_series_values(self, capsys, pd) -> None:
    p = bcpb.Property()
...
def TestProperty_test_matches_dicts_with_index_values(self, capsys, pd) -> None:
    p = bcpb.Property()
    d1 = pd.DataFrame(dict(foo=np.arange(10)))
    d2 = pd.DataFrame(dict(foo=np.arange(10)))
...
def TestProperty_test_validation_on(self) -> None:
