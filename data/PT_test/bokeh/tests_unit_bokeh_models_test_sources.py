...
def TestColumnDataSource_test_data_accepts_dataframe_arg(self, pd) -> None:
    data = dict(a=[1, 2], b=[2, 3])
    df = pd.DataFrame(data)
    ds = bms.ColumnDataSource()
    assert ds.data == {}
...
def TestColumnDataSource_test_init_dataframe_data_kwarg(self, pd) -> None:
    data = dict(a=[1, 2], b=[2, 3])
    df = pd.DataFrame(data)
...
def TestColumnDataSource_test_init_dataframe_index_named_column(self, pd) -> None:
...
def TestColumnDataSource_test_data_accepts_dataframe_index_named_column(self, pd) -> None:
    data = dict(a=[1, 2], b=[2, 3], index=[4, 5])
    df = pd.DataFrame(data)
    ds = bms.ColumnDataSource()
    assert ds.data == {}
...
def TestColumnDataSource_test_init_dataframe_column_categoricalindex(self, pd) -> None:
...
def TestColumnDataSource_test_data_accepts_dataframe_column_categoricalindex(self, pd) -> None:
    columns = pd.CategoricalIndex(['a', 'b'])
    data = [[0, 2], [1, 3]]
    df = pd.DataFrame(columns=columns, data=data)
    ds = bms.ColumnDataSource()
...
def TestColumnDataSource_test_init_dataframe_nonstring_named_column(self, pd) -> None:
    data = {1: [1, 2], 2: [2, 3]}
    df = pd.DataFrame(data)
    with pytest.raises(ValueError, match='expected an element of.*'):
        bms.ColumnDataSource(data=df)

def TestColumnDataSource_test_init_dataframe_nonstring_named_multicolumn(self, pd) -> None:
    data = {(1, 2): [1, 2], (2, 3): [2, 3]}
    df = pd.DataFrame(data)
    with pytest.raises(TypeError, match='Could not flatten.*'):
        bms.ColumnDataSource(data=df)

def TestColumnDataSource_test_init_groupby_arg(self, pd) -> None:
...
def TestColumnDataSource_test_data_accepts_groupby_arg(self, pd) -> None:
    from bokeh.sampledata.autompg import autompg as df
    group = df.groupby(by=['origin', 'cyl'])
    ds = bms.ColumnDataSource()
    assert ds.data == {}
...
def TestColumnDataSource_test_init_groupby_data_kwarg(self, pd) -> None:
    from bokeh.sampledata.autompg import autompg as df
    group = df.groupby(by=['origin', 'cyl'])
...
def TestColumnDataSource_test_init_groupby_with_None_subindex_name(self, pd) -> None:
...
def TestColumnDataSource_test_data_accepts_groupby_with_None_subindex_name(self, pd) -> None:
    df = pd.DataFrame({'A': [1, 2, 3, 4] * 2, 'B': [10, 20, 30, 40] * 2, 'C': range(8)})
    group = df.groupby(['A', [10, 20, 30, 40] * 2])
    ds = bms.ColumnDataSource()
    assert ds.data == {}
...
def TestColumnDataSource_test_init_propertyvaluecolumndata_copy(self) -> None:
    data = dict(a=[1], b=[2])
...
def TestColumnDataSource_test_add_with_name(self) -> None:
    ds = bms.ColumnDataSource()
...
def TestColumnDataSource_test_add_without_name(self) -> None:
    ds = bms.ColumnDataSource()
    name = ds.add([1, 2, 3])
...
def TestColumnDataSource_test_add_with_and_without_name(self) -> None:
    ds = bms.ColumnDataSource()
...
def TestColumnDataSource_test_remove_exists(self) -> None:
    ds = bms.ColumnDataSource()
    name = ds.add([1, 2, 3], 'foo')
    assert name
    ds.remove('foo')
...
def TestColumnDataSource_test_remove_exists2(self) -> None:
...
def TestColumnDataSource_test_stream_bad_data(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10], b=[20]))
    with pytest.raises(ValueError, match='Must stream updates to all existing columns \\(missing: a, b\\)'):
        ds.stream(dict())
    with pytest.raises(ValueError, match='Must stream updates to all existing columns \\(missing: b\\)'):
        ds.stream(dict(a=[10]))
    with pytest.raises(ValueError, match='Must stream updates to all existing columns \\(extra: x\\)'):
        ds.stream(dict(a=[10], b=[10], x=[10]))
    with pytest.raises(ValueError, match='Must stream updates to all existing columns \\(missing: b, extra: x\\)'):
        ds.stream(dict(a=[10], x=[10]))
    with pytest.raises(ValueError, match='All streaming column updates must be the same length'):
        ds.stream(dict(a=[10], b=[10, 20]))
    with pytest.raises(ValueError, match='stream\\(...\\) only supports 1d sequences, got ndarray with size \\(.*'):
        ds.stream(dict(a=[10], b=np.ones((1, 1))))

def TestColumnDataSource_test__df_index_name_with_named_index(self, pd) -> None:
    df = pd.DataFrame(dict(a=[10], b=[20], c=[30])).set_index('c')
    assert bms.ColumnDataSource._df_index_name(df) == 'c'

def TestColumnDataSource_test__df_index_name_with_unnamed_index(self, pd) -> None:
    df = pd.DataFrame(dict(a=[10], b=[20], c=[30]))
    assert bms.ColumnDataSource._df_index_name(df) == 'index'

def TestColumnDataSource_test__df_index_name_with_named_multi_index(self, pd) -> None:
...
def TestColumnDataSource_test__df_index_name_with_unnamed_multi_index(self, pd) -> None:
...
def TestColumnDataSource_test__stream_good_data(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10], b=[20]))
    ds._document = 'doc'
    stuff = {}
    mock_setter = object()
...
def TestColumnDataSource_test_stream_good_data(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10], b=[20]))
    ds._document = 'doc'
    stuff = {}
...
def TestColumnDataSource_test__stream_good_datetime64_data(self) -> None:
    now = dt.datetime.now()
...
def TestColumnDataSource_test__stream_good_datetime64_data_transformed(self) -> None:
    now = dt.datetime.now()
    dates = np.array([now + dt.timedelta(i) for i in range(1, 10)], dtype='datetime64')
...
def TestColumnDataSource_test__stream_good_df_with_date_index_data(self, pd) -> None:
    df = pd.DataFrame(index=pd.date_range('now', periods=30, freq='T'), columns=['A'], data=np.cumsum(np.random.standard_normal(30), axis=0))
    ds = bms.ColumnDataSource(data=df)
    ds._document = 'doc'
    stuff = {}
    mock_setter = object()
    ...
    def mock(*args, **kw):
        ...
        stuff['kw'] = kw
    ds.data._stream = mock
    new_df = pd.DataFrame(index=df.index + pd.to_timedelta('30m'), columns=df.columns, data=np.random.standard_normal(30))
...
def TestColumnDataSource_test__stream_good_dict_of_index_and_series_data(self, pd) -> None:
    df = pd.DataFrame(index=pd.date_range('now', periods=30, freq='T'), columns=['A'], data=np.cumsum(np.random.standard_normal(30), axis=0))
    ds = bms.ColumnDataSource(data=df)
    ds._document = 'doc'
    stuff = {}
...
def TestColumnDataSource_test__stream_good_dict_of_index_and_series_data_transformed(self, pd) -> None:
    df = pd.DataFrame(index=pd.date_range('now', periods=30, freq='T'), columns=['A'], data=np.cumsum(np.random.standard_normal(30), axis=0))
...
def TestColumnDataSource__assert_equal_dicts_of_arrays(self, d1, d2):
    assert d1.keys() == d2.keys()
    for (k, v) in d1.items():
        assert type(v) == np.ndarray
        assert np.array_equal(v, d2[k])

def TestColumnDataSource_test_stream_dict_to_ds_created_from_df(self, pd) -> None:
    ...
    self._assert_equal_dicts_of_arrays(notify_owners_stuff['args'][0], dict(a=np.array([10]), b=np.array([20]), c=np.array([30])))
    self._assert_equal_dicts_of_arrays(dict(ds.data), dict(a=np.array([10, 11, 12]), b=np.array([20, 21, 22]), c=np.array([30, 31, 32])))

def TestColumnDataSource_test_stream_series_to_ds_created_from_df(self, pd) -> None:
    ...
    assert stream_stuff['kwargs'] == {}
    assert len(notify_owners_stuff['args']) == 1
    self._assert_equal_dicts_of_arrays(notify_owners_stuff['args'][0], dict(a=np.array([10]), b=np.array([20]), c=np.array([30]), index=np.array([0])))
    self._assert_equal_dicts_of_arrays(dict(ds.data), dict(a=np.array([10, 11]), b=np.array([20, 21]), c=np.array([30, 31]), index=np.array([0, 0])))

def TestColumnDataSource_test_stream_df_to_ds_created_from_df_named_index(self, pd) -> None:
    data = pd.DataFrame(dict(a=[10], b=[20], c=[30])).set_index('c')
    ds = bms.ColumnDataSource(data)
    ds._document = 'doc'
    notify_owners_stuff = {}
...
def TestColumnDataSource_test_stream_df_to_ds_created_from_df_default_index(self, pd) -> None:
    data = pd.DataFrame(dict(a=[10], b=[20], c=[30]))
    ds = bms.ColumnDataSource(data)
    ds._document = 'doc'
    notify_owners_stuff = {}
    ...
    assert len(notify_owners_stuff['args']) == 1
    self._assert_equal_dicts_of_arrays(notify_owners_stuff['args'][0], dict(a=np.array([10]), b=np.array([20]), c=np.array([30]), index=np.array([0])))
    self._assert_equal_dicts_of_arrays(dict(ds.data), dict(a=np.array([10, 11, 12]), b=np.array([20, 21, 22]), c=np.array([30, 31, 32]), index=np.array([0, 0, 1])))

def TestColumnDataSource_test_patch_bad_columns(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10, 11], b=[20, 21]))
    with pytest.raises(ValueError, match='Can only patch existing columns \\(extra: c\\)'):
        ds.patch(dict(c=[(0, 100)]))
    with pytest.raises(ValueError, match='Can only patch existing columns \\(extra: c, d\\)'):
        ds.patch(dict(a=[(0, 100)], c=[(0, 100)], d=[(0, 100)]))

def TestColumnDataSource_test_patch_bad_simple_indices(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10, 11], b=[20, 21]))
    with pytest.raises(ValueError, match='Out-of bounds index \\(3\\) in patch for column: a'):
        ds.patch(dict(a=[(3, 100)]))

def TestColumnDataSource_test_patch_good_simple_indices(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10, 11], b=[20, 21]))
    ds._document = 'doc'
    stuff = {}
    mock_setter = object()
...
def TestColumnDataSource_test_patch_bad_slice_indices(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10, 11, 12, 13, 14, 15], b=[20, 21, 22, 23, 24, 25]))
    with pytest.raises(ValueError, match='Out-of bounds slice index stop \\(10\\) in patch for column: a'):
        ds.patch(dict(a=[(slice(10), list(range(10)))]))
    with pytest.raises(ValueError, match='Patch slices must have start < end, got slice\\(10, 1, None\\)'):
        ds.patch(dict(a=[(slice(10, 1), list(range(10)))]))
    with pytest.raises(ValueError, match='Patch slices must have non-negative \\(start, stop, step\\) values, got slice\\(None, 10, -1\\)'):
        ds.patch(dict(a=[(slice(None, 10, -1), list(range(10)))]))
    with pytest.raises(ValueError, match='Patch slices must have start < end, got slice\\(10, 1, 1\\)'):
        ds.patch(dict(a=[(slice(10, 1, 1), list(range(10)))]))
    with pytest.raises(ValueError, match='Patch slices must have start < end, got slice\\(10, 1, -1\\)'):
        ds.patch(dict(a=[(slice(10, 1, -1), list(range(10)))]))
    with pytest.raises(ValueError, match='Patch slices must have non-negative \\(start, stop, step\\) values, got slice\\(1, 10, -1\\)'):
        ds.patch(dict(a=[(slice(1, 10, -1), list(range(10)))]))

def TestColumnDataSource_test_patch_good_slice_indices(self) -> None:
    ds = bms.ColumnDataSource(data=dict(a=[10, 11, 12, 13, 14, 15], b=[20, 21, 22, 23, 24, 25]))
    ds._document = 'doc'
    stuff = {}
    mock_setter = object()
...
def TestColumnDataSource_test_data_column_lengths(self) -> None:
...
def TestColumnDataSource_test_set_data_from_json_list(self) -> None:
    ds = bms.ColumnDataSource()
    data = {'foo': [1, 2, 3]}
    ds.set_from_json('data', data)
    assert ds.data == data

def TestColumnDataSource_test_set_data_from_json_base64(self) -> None:
    ds = bms.ColumnDataSource()
...
def TestColumnDataSource_test_set_data_from_json_nested_base64(self) -> None:
    ds = bms.ColumnDataSource()
    data = {'foo': [[np.arange(3, dtype=np.int64)]]}
    json = transform_column_source_data(data)
    ds.set_from_json('data', json)
...
def TestColumnDataSource_test_set_data_from_json_nested_base64_and_list(self) -> None:
    ds = bms.ColumnDataSource()
