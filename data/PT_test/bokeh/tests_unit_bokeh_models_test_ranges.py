...
def Test_Range1d_test_init_with_datetime(self) -> None:
...
def Test_Range1d_test_init_with_float(self) -> None:
    range1d = Range1d(start=-1.0, end=3.0)
    assert range1d.start == -1.0
    assert range1d.end == 3.0
    assert range1d.bounds is None

def Test_Range1d_test_init_with_int(self) -> None:
    range1d = Range1d(start=-1, end=3)
    assert range1d.start == -1
    assert range1d.end == 3
    assert range1d.bounds is None

def Test_Range1d_test_init_with_positional_arguments(self) -> None:
    range1d = Range1d(1, 2)
    assert range1d.start == 1
    assert range1d.end == 2
    assert range1d.bounds is None

def Test_Range1d_test_init_with_keyword_arguments(self) -> None:
    range1d = Range1d(start=1, end=2)
    assert range1d.start == 1
    assert range1d.end == 2
    assert range1d.bounds is None

def Test_Range1d_test_cannot_initialize_with_both_keyword_and_positional_arguments(self) -> None:
    with pytest.raises(ValueError):
        Range1d(1, 2, start=1, end=2)

def Test_Range1d_test_cannot_initialize_with_three_positional_arguments(self) -> None:
    with pytest.raises(ValueError):
        Range1d(1, 2, 3)

def Test_Range1d_test_with_max_bound_smaller_than_min_bounded_raises_valueerror(self) -> None:
    with pytest.raises(ValueError):
        Range1d(1, 2, bounds=(1, 0))
    with pytest.raises(ValueError):
        Range1d(1, 2, bounds=[1, 0])

def Test_Range1d_test_bounds_with_text_rejected_as_the_correct_value_error(self) -> None:
    with pytest.raises(ValueError) as e:
        Range1d(1, 2, bounds='21')
    assert e.value.args[0].startswith('expected an element of either')

def Test_Range1d_test_bounds_with_three_item_tuple_raises_valueerror(self) -> None:
    with pytest.raises(ValueError):
        Range1d(1, 2, bounds=(0, 1, 2))

def Test_DataRange1d_test_basic(self) -> None:
    r = DataRange1d()
    check_properties_existence(r, ['names', 'renderers', 'range_padding', 'range_padding_units', 'flipped', 'follow', 'follow_interval', 'default_span', 'start', 'end', 'bounds', 'min_interval', 'max_interval', 'only_visible'])

def Test_DataRange1d_test_init_with_no_arguments(self) -> None:
    datarange1d = DataRange1d()
    assert datarange1d.start is None
    assert datarange1d.end is None
    assert datarange1d.bounds is None

def Test_DataRange1d_test_init_with_timedelta(self) -> None:
...
def Test_DataRange1d_test_init_with_datetime(self) -> None:
...
def Test_FactorRange_test_basic(self) -> None:
    r = FactorRange()
...
def Test_FactorRange_test_init_defauls(self) -> None:
...
def Test_FactorRange_test_init_with_positional_arguments(self) -> None:
    factor_range = FactorRange('a', 'b')
    assert factor_range.factors == ['a', 'b']
...
def Test_FactorRange_test_init_with_keyword_arguments(self) -> None:
    factor_range = FactorRange(factors=['a', 'b', 'c', 'd', 'e'])
    assert factor_range.factors == ['a', 'b', 'c', 'd', 'e']

def Test_FactorRange_test_cannot_initialize_with_both_keyword_and_positional_arguments(self) -> None:
    with pytest.raises(ValueError):
        FactorRange(['a', 'b', 'c'], factors=['a', 'b', 'c'])

def Test_FactorRange_test_duplicate_factors_raises_validation_error(self) -> None:
