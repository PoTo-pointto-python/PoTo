...
from bokeh.models import Circle, ColumnDataSource, CustomAction, CustomJS, NumericInput, Plot, Range1d
...
def Test_NumericInput_test_display_number_input(self, bokeh_model_page) -> None:
...
def Test_NumericInput_test_displays_title(self, bokeh_model_page) -> None:
...
def Test_NumericInput_test_displays_placeholder(self, bokeh_model_page) -> None:
...
@flaky(max_runs=10)
def Test_NumericInput_test_server_on_change_no_round_trip_without_enter_or_click(self, bokeh_server_page) -> None:
...
@pytest.mark.skip
@flaky(max_runs=10)
def Test_NumericInput_test_server_on_change_round_trip(self, bokeh_server_page) -> None:
    page = bokeh_server_page(modify_doc)
    el = page.driver.find_element_by_css_selector('.foo input')
...
def Test_NumericInput_test_js_on_change_executes(self, single_plot_page) -> None:
...
def Test_NumericInput_test_low_high(self, single_plot_page) -> None:
    source = ColumnDataSource(dict(x=[1, 2], y=[1, 1]))
    plot = Plot(plot_height=400, plot_width=400, x_range=Range1d(0, 1), y_range=Range1d(0, 1), min_border=0)
    plot.add_glyph(source, Circle(x='x', y='y', size=20))
...
def Test_NumericInput_test_int_inputs(self, single_plot_page) -> None:
...
def Test_NumericInput_test_float_inputs(self, single_plot_page) -> None:
    values_to_enter = ['0', '1', '-1', '+5', '0.1', '-0.1', '+0.1', '-.1', '+.1', '1e-6', '1.e5', '-1e+3', '-1.e-5', 'a']
