...
def test_binary_array_types() -> None:
    ...
    for typ in [np.dtype(np.float32), np.dtype(np.float64), np.dtype(np.uint8), np.dtype(np.int8), np.dtype(np.uint16), np.dtype(np.int16), np.dtype(np.uint32), np.dtype(np.int32)]:
...
def Test_make_id_test_default(self) -> None:
    bus._simple_id = 999
    assert bus.make_id() == '1000'
    assert bus.make_id() == '1001'
    assert bus.make_id() == '1002'

def Test_make_id_test_simple_ids_yes(self) -> None:
    bus._simple_id = 999
