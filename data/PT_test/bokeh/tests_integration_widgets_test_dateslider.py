...
from bokeh._testing.util.selenium import RECORD, drag_slider, get_slider_bar_color, get_slider_title_text, get_slider_title_value
from bokeh.layouts import column
from bokeh.models import Circle, ColumnDataSource, CustomAction, CustomJS, DateSlider, Plot, Range1d
pytest_plugins = ('bokeh._testing.plugins.project',)
start = date(2017, 8, 3)
end = date(2017, 8, 10)
value = start + timedelta(days=1)

def Test_DateSlider_test_display(self, bokeh_model_page) -> None:
...
def Test_DateSlider_test_displays_title(self, bokeh_model_page) -> None:
    slider = DateSlider(start=start, end=end, value=value, css_classes=['foo'], width=300)
    page = bokeh_model_page(slider)
    el = page.driver.find_element_by_css_selector('.foo')
...
def Test_DateSlider_test_title_updates(self, bokeh_model_page) -> None:
    slider = DateSlider(start=start, end=end, value=value, css_classes=['foo'], width=300)
    page = bokeh_model_page(slider)
    assert get_slider_title_value(page.driver, '.foo') == '04 Aug 2017'
...
def Test_DateSlider_test_displays_bar_color(self, bokeh_model_page) -> None:
...
def Test_DateSlider_test_js_on_change_executes(self, bokeh_model_page) -> None:
    slider = DateSlider(start=start, end=end, value=value, css_classes=['foo'], width=300)
...
@flaky(max_runs=10)
def Test_DateSlider_test_server_on_change_round_trip(self, bokeh_server_page) -> None:
...
@flaky(max_runs=10)
def Test_DateSlider_test_server_callback_value_vs_value_throttled(self, bokeh_server_page) -> None:
    junk = dict(v=0, vt=0)
...
@flaky(max_runs=10)
def Test_DateSlider_test_server_bar_color_updates(self, bokeh_server_page) -> None:
...
@flaky(max_runs=10)
def Test_DateSlider_test_server_title_updates(self, bokeh_server_page) -> None:
