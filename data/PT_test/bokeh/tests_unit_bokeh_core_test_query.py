import pytest
pytest
from bokeh.models import Axis, BoxZoomTool, Circle, ColumnDataSource, DatetimeAxis, Glyph, GlyphRenderer, Grid, Line, LinearAxis, LogAxis, PanTool, Plot, Range1d, Rect, ResetTool, SaveTool, Tool, WheelZoomTool
import bokeh.core.query as q
typcases = {Range1d: 3, Plot: 1, Glyph: 3, Axis: 3, DatetimeAxis: 1, LinearAxis: 2, LogAxis: 1, Grid: 2, Tool: 5, BoxZoomTool: 1, PanTool: 1, SaveTool: 1, ResetTool: 1, WheelZoomTool: 1}
...
def test_disjuction() -> None:
    ...
    assert len(res) == 6
    res = list(q.find(plot.references(), {q.OR: [dict(type=Axis), dict(tags='foo'), dict(name='mycircle'), dict(name='bad')]}))
    assert len(res) == 6
