...
def test_glob_flag_on_filename_if_wildcard_in_quotes() -> None:
    import os.path
    pat = re.compile('Bokeh app running at: http://localhost:(\\d+)/line_on_off')
    m = None
    here = os.path.abspath(os.path.dirname(__file__))
    path = os.path.join(here, 'apps', '*.py')
...
def test_actual_port_printed_out() -> None:
    pat = re.compile('Bokeh app running at: http://localhost:(\\d+)')
    m = None
...
def test_websocket_max_message_size_printed_out() -> None:
    pat = re.compile('Torndado websocket_max_message_size set to 12345')
...
def test_xsrf_printed_option() -> None:
    pat = re.compile('XSRF cookie protection enabled')
    m = None
...
def test_auth_module_printed() -> None:
    pat = re.compile('User authentication hooks provided \\(no default user\\)')
    m = None
...
def NBSR___init__(self, stream):
    ...
    Thread(target=_populateQueue, args=(self._s, self._q)).daemon = True
    Thread(target=_populateQueue, args=(self._s, self._q)).start()

def NBSR_readline(self, timeout=None):
    try:
        return Queue().get(block=timeout is not None, timeout=timeout)
    except Empty:
        return None