...
def Test_PointDrawTool_test_selected_by_default(self, single_plot_page) -> None:
    plot = _make_plot()
...
def Test_PointDrawTool_test_can_be_deselected_and_selected(self, single_plot_page) -> None:
    plot = _make_plot()
    page = single_plot_page(plot)
    button = page.get_toolbar_button('point-draw')
    assert 'active' in button.get_attribute('class')
...
def Test_PointDrawTool_test_click_triggers_draw(self, single_plot_page) -> None:
    plot = _make_plot()
    page = single_plot_page(plot)
...
def Test_PointDrawTool_test_click_does_not_trigger_draw(self, single_plot_page) -> None:
...
def Test_PointDrawTool_test_drag_moves_point(self, single_plot_page) -> None:
    plot = _make_plot()
    page = single_plot_page(plot)
    page.click_canvas_at_position(200, 200)
    time.sleep(0.4)
...
def Test_PointDrawTool_test_drag_does_not_move_point(self, single_plot_page) -> None:
...
def Test_PointDrawTool_test_num_object_limits_points(self, single_plot_page) -> None:
...
@flaky(max_runs=10)
def Test_PointDrawTool_test_point_draw_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [1, 2, 1.6216216216216217], 'y': [1, 1, 1.5]}
...
@flaky(max_runs=10)
def Test_PointDrawTool_test_point_drag_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [1, 2, 2.1891891891891895], 'y': [1, 1, 1.1024999999999998]}
    page = bokeh_server_page(_make_server_plot(expected))
    page.click_canvas_at_position(200, 200)
    time.sleep(0.4)
...
@flaky(max_runs=10)
def Test_PointDrawTool_test_point_delete_syncs_to_server(self, bokeh_server_page) -> None:
    expected = {'x': [1, 2], 'y': [1, 1]}
    page = bokeh_server_page(_make_server_plot(expected))
    page.click_canvas_at_position(200, 200)
    time.sleep(0.4)
