...
@pytest.mark.parametrize('code', ['value += index', 'value += 1', 'value += index.prop', 'value += index[key]', 'value += index.method()', 'other += 1', 'index += value', 'value += "str"'])
def test_implicit_sum(assert_errors, parse_ast_tree, code, template, default_options, mode):
...
@pytest.mark.parametrize('template', [for_loop_template])
@pytest.mark.parametrize('code', ['sum(some)', 'value -= 1', 'print(index)', 'index += value\n        print(index)', 'obj.attr += 1', 'obj[key] += index'])
def test_regular_loops(assert_errors, parse_ast_tree, code, template, default_options, mode):
    template = for_loop_template
    code = 'sum(some)'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
