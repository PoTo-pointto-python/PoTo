import pytest
from wemake_python_styleguide.violations.oop import WrongDescriptorDecoratorViolation
...
@pytest.mark.parametrize('code', [correct_decorator1, correct_decorator2, correct_decorator3, correct_decorator4])
def test_method_decorators_correct(assert_errors, parse_ast_tree, code, default_options, mode):
    code = correct_decorator1
...
@pytest.mark.parametrize('code', [wrong_decorator1, wrong_decorator2, wrong_decorator3, wrong_decorator4])
def test_method_decorators_incorrect(assert_errors, parse_ast_tree, code, default_options, mode):
    code = wrong_decorator1
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    mode = mode()
