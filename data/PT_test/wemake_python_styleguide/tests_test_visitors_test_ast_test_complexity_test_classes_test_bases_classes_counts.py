import pytest
from wemake_python_styleguide.violations.complexity import TooManyBaseClassesViolation
...
@pytest.mark.parametrize('code', [correct_count, correct_count_with_keywords])
def test_correct_count(assert_errors, parse_ast_tree, code, default_options):
    code = correct_count
...
@pytest.mark.parametrize('code', [too_many_count])
def test_bad_number_default_option(assert_errors, assert_error_text, parse_ast_tree, code, default_options):
    code = too_many_count
    assert_errors = assert_errors()
...
@pytest.mark.parametrize('code', [too_many_count, correct_count, correct_count_with_keywords])
def test_bad_number_custom_option(assert_errors, parse_ast_tree, code, options):
    code = too_many_count
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    options = options()
