import pytest
from wemake_python_styleguide.violations.refactoring import UselessFinallyViolation
...
@pytest.mark.parametrize('code', [wrong_try_example1, wrong_try_example2])
def test_wrong_finally(assert_errors, parse_ast_tree, code, mode, default_options):
    code = wrong_try_example1
...
@pytest.mark.parametrize('code', [right_try_example, check_finally_with_except, check_finally_with_except_else, correct_finally_decorated])
def test_correct_finally(assert_errors, parse_ast_tree, code, mode, default_options):
    code = right_try_example
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    mode = mode()
    default_options = default_options()
