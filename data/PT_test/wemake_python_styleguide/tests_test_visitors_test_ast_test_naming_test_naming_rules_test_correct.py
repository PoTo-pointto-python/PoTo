...
@pytest.mark.parametrize('correct_name', ['snake_case', '_protected', 'with_number5'])
def test_naming_correct(assert_errors, parse_ast_tree, naming_template, default_options, mode, correct_name):
    correct_name = 'snake_case'
...
@pytest.mark.parametrize('allowed_name', ['item', 'items', 'handle', 'other_name'])
def test_name_in_allowed_domain_names_option(assert_errors, parse_ast_tree, naming_template, options, mode, allowed_name):
    allowed_name = 'item'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    naming_template = naming_template()
    ...
    visitor = WrongNameVisitor(options(allowed_domain_names=(allowed_name,)), tree=tree)
    visitor.run()
    assert_errors(visitor, [])
