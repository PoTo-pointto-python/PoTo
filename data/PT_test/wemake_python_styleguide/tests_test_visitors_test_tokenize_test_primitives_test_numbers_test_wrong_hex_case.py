import pytest
from wemake_python_styleguide.violations.consistency import BadNumberSuffixViolation, WrongHexNumberCaseViolation
from wemake_python_styleguide.visitors.tokenize.primitives import WrongNumberTokenVisitor
hex_number_templates = ['0x{0}', '0xA{0}', '0x{0}2', '0xB{0}1']

@pytest.mark.parametrize('hex_char', ['a', 'b', 'c', 'd', 'e', 'f'])
@pytest.mark.parametrize('number', hex_number_templates)
def test_hex_wrong_case(parse_tokens, assert_errors, assert_error_text, default_options, hex_char, number, number_sign):
    hex_char = 'a'
    number = hex_number_templates[0]
...
@pytest.mark.parametrize('hex_char', ['A', 'B', 'C', 'D', 'E', 'F'])
@pytest.mark.parametrize('number', hex_number_templates)
def test_hex_correct_case(parse_tokens, assert_errors, default_options, hex_char, number, number_sign):
    hex_char = 'A'
    number = hex_number_templates[0]
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
...
@pytest.mark.parametrize('hex_char', ['a', 'b', 'c', 'd', 'e', 'f'])
@pytest.mark.parametrize('number', hex_number_templates)
def test_hex_double_wrong_case(parse_tokens, assert_errors, default_options, hex_char, number, number_sign):
    hex_char = 'a'
    number = hex_number_templates[0]
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    ...
    visitor = WrongNumberTokenVisitor(default_options, file_tokens=file_tokens)
    visitor.run()
    assert_errors(visitor, [BadNumberSuffixViolation, WrongHexNumberCaseViolation])
import io
import tokenize
