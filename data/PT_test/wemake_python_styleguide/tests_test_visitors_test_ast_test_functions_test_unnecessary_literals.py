...
from wemake_python_styleguide.violations.consistency import UnnecessaryLiteralsViolation
from wemake_python_styleguide.visitors.ast.functions import UnnecessaryLiteralsVisitor
regular_call_without_args = '{0}()'
regular_call_with_args = '{0}(*args, **kwargs)'

@pytest.mark.parametrize('literal', LITERALS_BLACKLIST)
def test_unnecessary_literals(assert_errors, parse_ast_tree, literal, default_options, mode):
    literal = LITERALS_BLACKLIST[0]
...
@pytest.mark.parametrize('literal', LITERALS_BLACKLIST)
def test_literals_with_args_called(assert_errors, parse_ast_tree, literal, default_options, mode):
    literal = LITERALS_BLACKLIST[0]
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
