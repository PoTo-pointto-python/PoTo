import pytest
from wemake_python_styleguide.violations.naming import WrongUnusedVariableNameViolation
from wemake_python_styleguide.visitors.ast.naming import WrongNameVisitor

@pytest.mark.parametrize('wrong_name', ['__', '___'])
def test_wrong_unused_variable_name(assert_errors, assert_error_text, parse_ast_tree, naming_template, default_options, mode, wrong_name):
    wrong_name = '__'
...
@pytest.mark.parametrize('wrong_name', ['_'])
def test_correct_unused_variable_name(assert_errors, parse_ast_tree, naming_template, default_options, mode, wrong_name):
    wrong_name = '_'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    naming_template = naming_template()
