...
from wemake_python_styleguide.violations.naming import ReservedArgumentNameViolation
...
@pytest.mark.parametrize('code', [wrong_method_template, wrong_classmethod_template, wrong_function_template])
def test_reserved_argument_name(assert_errors, assert_error_text, parse_ast_tree, default_options, code, mode, argument):
    argument = SPECIAL_ARGUMENT_NAMES_WHITELIST[0]
    code = wrong_method_template
...
@pytest.mark.parametrize('argument', SPECIAL_ARGUMENT_NAMES_WHITELIST)
@pytest.mark.parametrize('code', [correct_method_template, correct_classmethod_template, correct_function_template])
def test_correct_argument_name(assert_errors, parse_ast_tree, default_options, code, mode, argument):
    argument = SPECIAL_ARGUMENT_NAMES_WHITELIST[0]
    code = correct_method_template
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    mode = mode()
