import pytest
from wemake_python_styleguide.violations.consistency import BadNumberSuffixViolation
from wemake_python_styleguide.visitors.tokenize.primitives import WrongNumberTokenVisitor

@pytest.mark.parametrize('number', ['0X1', '0XE', '1.5E10', '0O11', '0B1001'])
def test_bad_number_suffixes(parse_tokens, assert_errors, assert_error_text, default_options, primitives_usages, number, number_sign, mode):
    number = '0X1'
...
@pytest.mark.parametrize('number', ['1234567890', '0xFF', '1.5e10', '0o11', '0b1001', '0xE', '0x1E', '0xB1'])
def test_correct_number_suffixes(parse_tokens, assert_errors, default_options, primitives_usages, number, number_sign, mode):
    number = '1234567890'
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
