import pytest
from wemake_python_styleguide.violations.oop import MethodWithoutArgumentsViolation
from wemake_python_styleguide.visitors.ast.classes import WrongMethodVisitor
method_inside_class = '\nclass Example(object):\n    def some_name({0}): ...\n'
regular_function = 'def some_name({0}): ...'

def test_method_without_arguments(assert_errors, assert_error_text, parse_ast_tree, mode, default_options):
...
def test_function_without_arguments(assert_errors, parse_ast_tree, mode, default_options):
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    mode = mode()
    default_options = default_options()
...
@pytest.mark.parametrize('code', [method_inside_class, regular_function])
@pytest.mark.parametrize('arguments', ['arg1', 'arg1=True', '*args', '**kwargs', '*, kwonly', '*, kwonly=True', 'arg1, arg2'])
def test_with_arguments(assert_errors, parse_ast_tree, code, arguments, mode, default_options):
    code = method_inside_class
    arguments = 'arg1'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    mode = mode()
