import pytest
from wemake_python_styleguide.violations.consistency import RawStringNotNeededViolation
from wemake_python_styleguide.visitors.tokenize.primitives import WrongStringTokenVisitor

@pytest.mark.parametrize('raw_strings', ["r'some text\\\\'", "r'some text\\''", 'r\'some text\\"\'', 'r"some text\\\'"', "r'some text\\t'", "r'some text\\a'", "r'some text\\n'", "r'some text\\u041b'", "r'some text\\043'", "r'some text\\x23'"])
def test_necessary_raw_string(parse_tokens, assert_errors, default_options, raw_strings):
    raw_strings = "r'some text\\\\'"
...
@pytest.mark.parametrize('raw_strings', ["r'No escaped character'", 'r"Here neither"', "r'''Not here as well'''", 'r"""Not here as well"""'])
def test_unnecessary_raw_string(parse_tokens, assert_errors, default_options, raw_strings):
    raw_strings = "r'No escaped character'"
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
