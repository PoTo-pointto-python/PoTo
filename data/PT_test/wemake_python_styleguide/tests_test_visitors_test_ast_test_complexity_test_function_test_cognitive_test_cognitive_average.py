...
def test_complex_cognitive_module(assert_errors, parse_ast_tree, code, default_options, mode):
    ...
    visitor.run()
    assert_errors(visitor, [CognitiveModuleComplexityViolation], ignored_types=(CognitiveComplexityViolation,))
...
def test_complex_cognitive_options(assert_errors, assert_error_text, parse_ast_tree, code, options, mode):
    ...
    visitor.run()
    assert_errors(visitor, [CognitiveModuleComplexityViolation], ignored_types=(CognitiveComplexityViolation,))
    assert_error_text(visitor, '1.0', option_values.max_cognitive_average, multiple=True)

@pytest.mark.parametrize('code', [function_example, method_example, empty_module])
def test_cognitive_average_default_options(assert_errors, parse_ast_tree, code, default_options, mode):
    code = function_example
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
