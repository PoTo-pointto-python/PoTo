...
from wemake_python_styleguide.violations.consistency import UselessCompareViolation
from wemake_python_styleguide.visitors.ast.compares import CompareSanityVisitor
create_variables = '\nvariable = 1\nanother_variable = 2\n{0}\n'
correct_comparators = [('variable', '"test"'), ('variable', 'variable.call()'), ('variable', 'variable.attr'), ('variable', 'len(variable)'), ('variable', 'another_variable'), ('variable', '222')]
wrong_comparators = [('variable', 'variable'), ('another_variable', 'another_variable')]
if PY38:
    correct_comparators.extend([('(x := variable)', 'some()'), ('(x := some())', 'variable')])
    wrong_comparators.extend([('(x := variable)', 'variable'), ('variable', '(x := variable)'), ('(x := variable)', '(x := variable)')])

@pytest.mark.filterwarnings('ignore::SyntaxWarning')
@pytest.mark.parametrize('comparators', correct_comparators)
def test_not_useless(assert_errors, parse_ast_tree, simple_conditions, comparators, default_options):
    comparators = correct_comparators[0]
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    simple_conditions = simple_conditions()
    default_options = default_options()
    'Testing that compares work well.'
    tree = parse_ast_tree(create_variables.format(simple_conditions.format(*comparators)))
    visitor = CompareSanityVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])

@pytest.mark.parametrize('comparators', wrong_comparators)
def test_useless(assert_errors, parse_ast_tree, simple_conditions, comparators, default_options):
...
@pytest.mark.parametrize('comparators', wrong_comparators)
def test_useless_with_in(assert_errors, parse_ast_tree, in_conditions, comparators, default_options):
    comparators = wrong_comparators[0]
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    ...
    default_options = default_options()
    'Testing that violations are when comparing identical variable.'
    tree = parse_ast_tree(create_variables.format(in_conditions.format(*comparators)))
    visitor = CompareSanityVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [UselessCompareViolation])

def test_multiple_compare(assert_errors, parse_ast_tree, default_options):
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
