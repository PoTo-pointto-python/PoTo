...
from wemake_python_styleguide.visitors.ast.naming import MODULE_METADATA_VARIABLES_BLACKLIST, WrongModuleMetadataVisitor
module_metadata = "\n{0} = 'Nikita'\n"
...
@pytest.mark.parametrize('code', [module_metadata, module_type_metadata])
def test_wrong_metadata(assert_errors, assert_error_text, parse_ast_tree, bad_name, code, default_options):
    bad_name = MODULE_METADATA_VARIABLES_BLACKLIST[0]
    code = module_metadata
...
@pytest.mark.parametrize('code', [module_metadata, module_type_metadata, nested_metadata])
def test_correct_metadata(assert_errors, parse_ast_tree, code, correct_name, default_options):
...
def test_correct_startup_metadata(assert_errors, parse_ast_tree, default_options):
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
