...
@pytest.mark.parametrize('for_statement', [for_loop1, for_loop2, for_loop3])
@pytest.mark.parametrize('context', [for_template1, for_template2, for_template3, for_template4])
@pytest.mark.parametrize('variable_name', ['should_raise'])
def test_for_block_overlap(assert_errors, assert_error_text, parse_ast_tree, for_statement, assign_and_annotation_statement, context, variable_name, default_options, mode):
    for_statement = for_loop1
    context = for_template1
    ...
    code = context.format(for_statement.format(variable_name), assign_and_annotation_statement.format(variable_name))
...
@pytest.mark.parametrize('context', [for_template1, for_template2, for_template3, for_template4])
@pytest.mark.parametrize('variable_name', ['should_raise'])
def test_for_block_usage(assert_errors, parse_ast_tree, for_statement, context, variable_name, default_options, mode):
    for_statement = for_loop1
    context = for_template1
    variable_name = 'should_raise'
    assert_errors = assert_errors()
    ...
    code = context.format(for_statement.format(variable_name), 'print({0})'.format(variable_name))
    tree = parse_ast_tree(mode(code))
    visitor = BlockVariableVisitor(default_options, tree=tree)
    visitor.run()
...
@pytest.mark.parametrize('for_statement', [for_loop1, for_loop2, for_loop3])
@pytest.mark.parametrize('context', [for_template1, for_template2, for_template3, for_template4])
@pytest.mark.parametrize(('first_name', 'second_name'), [('unique_name', 'other_name'), ('_', '_')])
def test_for_block_correct(assert_errors, parse_ast_tree, for_statement, assign_and_annotation_statement, context, first_name, second_name, default_options, mode):
    for_statement = for_loop1
    context = for_template1
    (first_name, second_name) = ('unique_name', 'other_name')
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    assign_and_annotation_statement = assign_and_annotation_statement()
    default_options = default_options()
    ...
    code = context.format(for_statement.format(first_name), assign_and_annotation_statement.format(second_name))
    tree = parse_ast_tree(mode(code))
    visitor = BlockVariableVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])
