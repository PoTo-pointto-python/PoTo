...
from wemake_python_styleguide.violations.best_practices import PositionalOnlyArgumentsViolation
...
@pytest.mark.parametrize('code', [correct_lambda, correct_function, correct_method])
def test_not_posonlyargs(assert_errors, parse_ast_tree, code, mode, default_options):
    code = correct_lambda
...
@pytest.mark.parametrize('code', [wrong_lambda1, wrong_lambda2, wrong_function1, wrong_function2, wrong_function3, wrong_method1, wrong_method2])
def test_posonyargs(assert_errors, parse_ast_tree, code, mode, default_options):
    code = wrong_lambda1
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    mode = mode()
