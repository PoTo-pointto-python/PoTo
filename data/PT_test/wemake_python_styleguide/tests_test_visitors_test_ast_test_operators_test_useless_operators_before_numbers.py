import pytest
from wemake_python_styleguide.violations.consistency import UselessOperatorsViolation
...
tuple_definition = '({0}, )'
usages = (assignment, assignment_addition, assignment_increment, function_definition, list_definition, dict_definition_key, dict_definition_value, set_definition, tuple_definition)

@pytest.mark.parametrize('code', usages)
@pytest.mark.parametrize('number', ['+5', '-+5', '+-5', '~+5', '+~5', '--5', '---5', '~~5', '~~~5', '(not not 5)', '(not not None)', '(not not not True)', '(not not False)'])
def test_plus_sign_before_numbers(assert_errors, parse_ast_tree, code, number, default_options, mode):
    code = usages[0]
    number = '+5'
...
@pytest.mark.parametrize('code', usages)
@pytest.mark.parametrize('number', ['5', '-5', '~5', '(not 5)', '(not True)'])
def test_plus_sign_before_numbers_valid(assert_errors, parse_ast_tree, code, number, default_options, mode):
    code = usages[0]
    number = '5'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
