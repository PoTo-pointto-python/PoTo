...
from wemake_python_styleguide.violations.best_practices import YieldInComprehensionViolation
from wemake_python_styleguide.visitors.ast.loops import WrongComprehensionVisitor
list_comprehension = '\ndef container():\n    nodes = [{0} for xy in "abc"]\n'
generator_expression = '\ndef container():\n    nodes = ({0} for xy in "abc")\n'
set_comprehension = '\ndef container():\n    nodes = {{{0} for xy in "abc"}}\n'

@pytest.mark.skipif(PY38, reason='yield inside a generator is a syntax error in python3.8')
@pytest.mark.filterwarnings('ignore:DeprecationWarning')
@pytest.mark.parametrize('code', [list_comprehension, generator_expression, set_comprehension])
def test_yield_keyword_in_comprehension(assert_errors, parse_ast_tree, code, default_options, mode):
...
@pytest.mark.skipif(not PY38, reason='We check for syntax error here, which is true for python3.8+')
@pytest.mark.parametrize('code', [list_comprehension, generator_expression, set_comprehension])
def test_yield_keyword_syntax_error(assert_errors, parse_ast_tree, code, default_options, mode):
    code = list_comprehension
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
...
@pytest.mark.parametrize('code', [list_comprehension, generator_expression, set_comprehension])
def test_comprehension_without_yield(assert_errors, parse_ast_tree, code, default_options, mode):
    code = list_comprehension
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
