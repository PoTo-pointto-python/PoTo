import pytest
from wemake_python_styleguide.violations.refactoring import WrongIsinstanceWithTupleViolation
from wemake_python_styleguide.visitors.ast.functions import WrongFunctionCallVisitor
isinstance_call = 'isinstance(some, {0})'

@pytest.mark.parametrize('code', ['int', '(int, float)', '(int, float, )', 'call(1, 2)', 'some.attr', 'some.method()'])
def test_correct_isinstance_tuple(assert_errors, parse_ast_tree, code, default_options):
    code = 'int'
...
@pytest.mark.parametrize('code', ['(int, )'])
def test_wrong_isinstance_tuple(assert_errors, parse_ast_tree, code, default_options):
    code = '(int, )'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
