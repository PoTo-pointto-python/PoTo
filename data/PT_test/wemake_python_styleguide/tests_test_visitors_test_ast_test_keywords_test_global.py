import pytest
from wemake_python_styleguide.violations.best_practices import WrongKeywordViolation
...
@pytest.mark.parametrize('code', [global_in_module, global_in_function, global_in_method])
def test_global_keywords(assert_errors, assert_error_text, parse_ast_tree, code, default_options, mode):
    code = global_in_module
...
@pytest.mark.parametrize('code', [nonlocal_in_function, nonlocal_in_method])
def test_nonlocal_keywords(assert_errors, assert_error_text, parse_ast_tree, code, default_options, mode):
    code = nonlocal_in_function
    assert_errors = assert_errors()
    assert_error_text = assert_error_text()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
