import pytest
from wemake_python_styleguide.violations.best_practices import InfiniteWhileLoopViolation
...
@pytest.mark.parametrize('template', [template_simple, template_nested_while1, template_nested_while2, template_nested_if, template_function, template_other])
...
@pytest.mark.parametrize('template', [template_function])
...
@pytest.mark.parametrize('template', [template_other])
...
@pytest.mark.parametrize('template', [template_simple, template_nested_while1, template_nested_while2, template_nested_if, template_function])
@pytest.mark.parametrize('keyword', ['print(some)', 'attr.method()', 'a = 1'])
...
@pytest.mark.parametrize('template', [template_double_while])
...
def test_double_while_correct_loops(assert_errors, parse_ast_tree, keyword, template, default_options):
    template = template_double_while
    keyword = 'break'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
