import string
import pytest
from wemake_python_styleguide.violations.best_practices import StringConstantRedefinedViolation
...
@pytest.mark.parametrize('code', [string.digits, string.hexdigits, string.octdigits, string.ascii_uppercase, string.ascii_lowercase, string.ascii_letters])
@pytest.mark.parametrize('prefix', ['', 'b', 'u', 'r', 'rb'])
def test_alphabet_as_string_violation(assert_errors, assert_error_text, parse_ast_tree, code, prefix, default_options):
    code = string.digits
    prefix = ''
...
def test_alphabet_as_fstring_violation(assert_errors, parse_ast_tree, default_options):
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    ...
    visitor = WrongStringVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [StringConstantRedefinedViolation], (FormattedStringViolation,))

@pytest.mark.parametrize('code', ['ABCDE', '1234', 'random text', '\\n', '!@#', ''])
