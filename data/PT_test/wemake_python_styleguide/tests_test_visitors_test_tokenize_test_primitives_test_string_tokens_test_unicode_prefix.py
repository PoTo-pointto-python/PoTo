...
@pytest.mark.parametrize('primitive', ['u"text"', "u'unicode'", 'u"5_5"', 'u""'])
def test_unicode_prefix(parse_tokens, assert_errors, assert_error_text, default_options, primitives_usages, primitive, mode):
    primitive = 'u"text"'
...
@pytest.mark.parametrize('primitive', ['"name"', 'r"text with escape carac \n"', "b'unicode'", '"u"', '"12"', 'b""'])
def test_correct_strings(parse_tokens, assert_errors, default_options, primitives_usages, primitive, mode):
    primitive = '"name"'
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
...
def test_unicode_regression(parse_tokens, assert_errors, default_options, primitives_usages, mode):
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
    primitives_usages = primitives_usages()
