import pytest
from wemake_python_styleguide.violations.complexity import TooManyRaisesViolation
...
@pytest.mark.parametrize('code', [module_many_raises, lambda_many_raises])
def test_asserts_correct_count1(assert_errors, parse_ast_tree, options, code):
    code = module_many_raises
...
@pytest.mark.parametrize('context', [function_template, instance_method_template, class_method_template, static_method_template])
@pytest.mark.parametrize(('first', 'second', 'third'), [('...', '', ''), ('if some:', '    raise SomeException', 'raise SomeOtherException'), ('def helper():', '    raise SomeException', 'raise SomeOtherException')])
def test_raises_correct_count2(assert_errors, parse_ast_tree, context, first, second, third, default_options, mode):
...
@pytest.mark.parametrize('context', [function_template, instance_method_template, class_method_template, static_method_template])
@pytest.mark.parametrize(('first', 'second', 'third'), [('if some:', '    raise SomeException', 'raise SomeOtherException'), ('def helper():', '    raise SomeException', 'raise SomeOtherException')])
def test_raises_wrong_count(assert_errors, assert_error_text, parse_ast_tree, options, context, first, second, third, mode):
    context = function_template
    (first, second, third) = ('if some:', '    raise SomeException', 'raise SomeOtherException')
    assert_errors = assert_errors()
