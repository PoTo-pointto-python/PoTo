...
def _safe_output(output: str) -> str:
    ...
    current_version_url = WemakeFormatter._doc_url
...
@pytest.mark.parametrize(('cli_options', 'output'), [([], 'regular'), (['--statistic'], 'regular_statistic'), (['--show-source'], 'with_source'), (['--show-source', '--statistic'], 'with_source_statistic'), (['--statistic', '--show-source'], 'statistic_with_source')])
def test_formatter(snapshot, cli_options, output):
    (cli_options, output) = ([], 'regular')
    ...
    process = subprocess.Popen(['flake8', '--disable-noqa', '--isolated', '--select', 'WPS', '--format', 'wemake', *cli_options, filename1, filename2], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding='utf8')
    (stdout, _) = process.communicate()
    snapshot.assert_match(_safe_output(stdout), 'formatter_{0}'.format(output))
...
def test_formatter_correct(snapshot, cli_options, output):
    ...
    process = subprocess.Popen(['flake8', '--disable-noqa', '--isolated', '--format', 'wemake', *cli_options, filename], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding='utf8')
    (stdout, _) = process.communicate()
    snapshot.assert_match(_safe_output(stdout), 'formatter_correct_{0}'.format(output))