...
from wemake_python_styleguide.visitors.base import BaseFilenameVisitor, BaseNodeVisitor, BaseTokenVisitor, BaseVisitor

def _is_visitor_class(cls) -> bool:
    base_classes = {BaseFilenameVisitor, BaseNodeVisitor, BaseTokenVisitor, BaseVisitor}
...
def _import_module_by_path(path: str):
    ...
    spec = importlib.util.spec_from_file_location(module_name, loader=importlib.machinery.SourceFileLoader(module_name, path))
...
def _visitors_paths():
    ...
    return [path for path in base_path.glob('**/*.py') if path not in excluded_paths]
...
def test_all_visitors_contained_in_checker(all_visitors):
    ...
    checker_visitors = {klass.__qualname__ for klass in Checker._visitors if not klass.__qualname__.startswith('_')}
