import pytest
from wemake_python_styleguide.violations.naming import UpperCaseAttributeViolation, WrongVariableNameViolation
from wemake_python_styleguide.visitors.ast.naming import WrongNameVisitor

@pytest.mark.parametrize('wrong_name', ['SOME', 'Value', 'nO', 'dATa'])
def test_wrong_variable_name_case_insensitive(assert_errors, assert_error_text, parse_ast_tree, naming_template, default_options, mode, wrong_name):
    wrong_name = 'SOME'
    ...
    visitor.run()
    assert_errors(visitor, [WrongVariableNameViolation], ignored_types=(UpperCaseAttributeViolation,))
    assert_error_text(visitor, wrong_name, multiple=True)

@pytest.mark.parametrize('wrong_name', ['value', 'no', 'data'])
def test_wrong_variable_name(assert_errors, assert_error_text, parse_ast_tree, naming_template, default_options, mode, wrong_name):
    wrong_name = 'value'
    assert_errors = assert_errors()
    assert_error_text = assert_error_text()
    parse_ast_tree = parse_ast_tree()
...
@pytest.mark.parametrize('forbidden_name', ['visitor', 'name_validator'])
def test_name_in_forbidden_domain_names_option(assert_errors, assert_error_text, parse_ast_tree, naming_template, options, mode, forbidden_name):
    forbidden_name = 'visitor'
    assert_errors = assert_errors()
    assert_error_text = assert_error_text()
    parse_ast_tree = parse_ast_tree()
    naming_template = naming_template()
    ...
    visitor = WrongNameVisitor(options(forbidden_domain_names=(forbidden_name,)), tree=tree)
    visitor.run()
    assert_errors(visitor, [WrongVariableNameViolation])
