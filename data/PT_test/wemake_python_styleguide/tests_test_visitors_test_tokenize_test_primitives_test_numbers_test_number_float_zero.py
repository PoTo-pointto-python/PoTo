...
def test_float_zero(parse_tokens, assert_errors, default_options, primitives_usages, mode):
...
@pytest.mark.parametrize('primitive', ['0', 'float(0)', '5', '30.4'])
def test_correct_zero_and_non_zero_numbers(parse_tokens, assert_errors, default_options, primitives_usages, primitive, mode):
    primitive = '0'
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
