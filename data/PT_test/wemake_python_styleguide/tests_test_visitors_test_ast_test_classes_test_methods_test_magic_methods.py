...
def test_wrong_magic_used(assert_errors, assert_error_text, parse_ast_tree, method, default_options):
    method = MAGIC_METHODS_BLACKLIST[0]
...
@pytest.mark.parametrize('code', [magic_method, regular_function])
@pytest.mark.parametrize('method', ['__add__', '__init__'])
def test_correct_magic_method_used(assert_errors, parse_ast_tree, code, method, default_options):
    code = magic_method
    method = '__add__'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
...
@pytest.mark.parametrize('code', [magic_method, regular_function])
@pytest.mark.parametrize('method', ['next', 'regular'])
def test_regular_method_used(assert_errors, parse_ast_tree, code, method, mode, default_options):
    code = magic_method
    method = 'next'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
