...
@pytest.mark.parametrize('definition', ['xy[0]', 'xy.attr', 'xy["key"]', '(valid, invalid.attr)', '(invalid.attr, valid)'])
def test_wrong_definition_loop(assert_errors, parse_ast_tree, definition, default_options, mode):
    definition = 'xy[0]'
...
@pytest.mark.parametrize('definition', ['xy', '(valid1, valid2)', 'valid1, *valid2'])
def test_correct_definition_loop(assert_errors, parse_ast_tree, definition, default_options, mode):
    definition = 'xy'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
