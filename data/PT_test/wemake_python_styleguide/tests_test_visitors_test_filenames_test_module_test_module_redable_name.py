import pytest
from wemake_python_styleguide.violations.naming import UnderscoredNumberNameViolation, UnreadableNameViolation, WrongModuleNamePatternViolation
from wemake_python_styleguide.visitors.filenames.module import WrongModuleNameVisitor

@pytest.mark.parametrize('filename', ['the1long'])
...
@pytest.mark.parametrize('filename', ['ordinary', 'first_module'])
...
@pytest.mark.parametrize('filename', ['TestO_0'])
def test_corner_case(assert_errors, filename, default_options):
    ...
    visitor = WrongModuleNameVisitor(default_options, filename=filename)
    visitor.run()
    assert_errors(visitor, [WrongModuleNamePatternViolation, UnderscoredNumberNameViolation])
