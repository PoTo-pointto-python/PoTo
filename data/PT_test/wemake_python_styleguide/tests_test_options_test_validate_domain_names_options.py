import pytest
from wemake_python_styleguide.options.validation import validate_domain_names_options

@pytest.mark.parametrize(('allowed_names', 'forbidden_names'), [(['items'], []), ([], ['items']), (['item'], ['handle'])])
def test_passes_when_any_option_not_passed(allowed_names, forbidden_names):
    (allowed_names, forbidden_names) = (['items'], [])
