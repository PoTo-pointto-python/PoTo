import pytest
from wemake_python_styleguide.violations.consistency import MultipleInCompareViolation
from wemake_python_styleguide.violations.refactoring import InCompareWithSingleItemContainerViolation
...
@pytest.mark.parametrize('code', [if_without_multiple_in_compares, ternary, while_construct])
@pytest.mark.parametrize('comparators', [('x_coord', '6'), ('status', {True}), ('letter', {'a', 'b'})])
def test_compare_with_in(assert_errors, parse_ast_tree, code, comparators, default_options, in_not_in):
    code = if_without_multiple_in_compares
    comparators = ('x_coord', '6')
    ...
    visitor.run()
    assert_errors(visitor, [], ignored_types=(InCompareWithSingleItemContainerViolation,))

@pytest.mark.parametrize('code', [if_with_multiple_in_compares, ternary_with_multiple_in, while_with_multiple_in])
@pytest.mark.parametrize('comparators', [('line', 'sqaure', 'shape'), ('output', 'status', {True}), ('letter', 'line', 'book')])
def test_compare_with_multiple_in(assert_errors, parse_ast_tree, code, comparators, default_options, in_not_in):
    code = if_with_multiple_in_compares
    comparators = ('line', 'sqaure', 'shape')
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    in_not_in = in_not_in()
    ...
    visitor = InCompareSanityVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [MultipleInCompareViolation], ignored_types=(InCompareWithSingleItemContainerViolation,))

def test_compare_with_mixed_in(assert_errors, parse_ast_tree, default_options):
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
