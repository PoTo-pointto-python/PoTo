import pytest
from wemake_python_styleguide.violations.consistency import UselessContinueViolation
...
@pytest.mark.parametrize('code', [wrong_for_loop, wrong_nested_for_loop])
def test_wrong_continue_in_loop(assert_errors, parse_ast_tree, code, default_options, mode):
    code = wrong_for_loop
...
@pytest.mark.parametrize('code', [correct_for_loop, correct_while_loop])
def test_correct_continue_usage(assert_errors, parse_ast_tree, code, default_options, mode):
    code = correct_for_loop
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    mode = mode()
