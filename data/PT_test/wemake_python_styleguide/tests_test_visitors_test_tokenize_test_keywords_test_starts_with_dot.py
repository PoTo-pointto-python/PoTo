import pytest
from wemake_python_styleguide.violations.consistency import LineStartsWithDotViolation
...
@pytest.mark.parametrize('code', [wrong_dot_start1, wrong_dot_start2])
def test_wrong_dot_start(parse_tokens, assert_errors, default_options, code):
    code = wrong_dot_start1
...
@pytest.mark.parametrize('code', [correct_dot_attr, correct_ellipsis, correct_string_dot])
def test_correct_dot_start(parse_tokens, assert_errors, default_options, code):
    code = correct_dot_attr
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
