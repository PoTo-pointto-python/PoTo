import pytest
from wemake_python_styleguide.violations.consistency import BadComplexNumberSuffixViolation
from wemake_python_styleguide.visitors.tokenize.primitives import WrongNumberTokenVisitor

@pytest.mark.parametrize('number', ['1J', '2J + 10'])
def test_bad_complex_suffix(parse_tokens, assert_errors, assert_error_text, default_options, primitives_usages, number, number_sign, mode):
    number = '1J'
...
@pytest.mark.parametrize('number', ['1j', '2j + 10'])
def test_correct_complex_suffix(parse_tokens, assert_errors, default_options, primitives_usages, number, number_sign, mode):
    number = '1j'
    parse_tokens = parse_tokens()
    assert_errors = assert_errors()
    default_options = default_options()
