...
@pytest.mark.parametrize('with_statement', [with1, with2, with3, with4, with5])
@pytest.mark.parametrize('context', [with_template1, with_template2, with_template3, with_template4])
@pytest.mark.parametrize('variable_name', ['should_raise'])
def test_with_block_overlap(assert_errors, assert_error_text, parse_ast_tree, with_statement, assign_and_annotation_statement, context, variable_name, default_options, mode):
    with_statement = with1
    context = with_template1
    ...
    code = context.format(with_statement.format(variable_name), assign_and_annotation_statement.format(variable_name))
...
@pytest.mark.parametrize('context', [with_template1, with_template2, with_template3, with_template4])
@pytest.mark.parametrize('variable_name', ['should_raise'])
def test_with_block_usage(assert_errors, parse_ast_tree, with_statement, context, variable_name, default_options, mode):
    with_statement = with1
    context = with_template1
    variable_name = 'should_raise'
    assert_errors = assert_errors()
    ...
    code = context.format(with_statement.format(variable_name), 'print({0})'.format(variable_name))
    tree = parse_ast_tree(mode(code))
    visitor = BlockVariableVisitor(default_options, tree=tree)
    visitor.run()
...
@pytest.mark.parametrize('with_statement', [with1, with2, with3, with4, with5])
@pytest.mark.parametrize('context', [with_template1, with_template2, with_template3, with_template4])
@pytest.mark.parametrize(('first_name', 'second_name'), [('unique_name', 'unique'), ('_', '_')])
def test_with_block_correct(assert_errors, parse_ast_tree, with_statement, assign_and_annotation_statement, context, first_name, second_name, default_options, mode):
    with_statement = with1
    context = with_template1
    (first_name, second_name) = ('unique_name', 'unique')
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    assign_and_annotation_statement = assign_and_annotation_statement()
    default_options = default_options()
    ...
    code = context.format(with_statement.format(first_name), assign_and_annotation_statement.format(second_name))
    tree = parse_ast_tree(mode(code))
    visitor = BlockVariableVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])
