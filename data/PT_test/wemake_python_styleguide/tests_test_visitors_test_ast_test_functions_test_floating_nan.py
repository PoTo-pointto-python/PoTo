import pytest
from wemake_python_styleguide.violations.best_practices import FloatingNanViolation
from wemake_python_styleguide.visitors.ast.functions import FloatingNanCallVisitor

@pytest.mark.parametrize('code', ['float(4.5) + 4.2', 'float(2) + 0', 'result = "NaN".join(["a", "a"])', 'a = math.nan', 'my_function("NaN")', 'float("123") + math.nan'])
def test_correct_nan_acquisition(assert_errors, parse_ast_tree, default_options, code, mode):
    code = 'float(4.5) + 4.2'
...
@pytest.mark.parametrize('nan_variant', ['NaN', 'NAN', 'nan', 'Nan', 'nAn', 'nAN', 'naN', 'NAn'])
@pytest.mark.parametrize('code', ['float({0})', 'float({0}) < 0.0', 'sorted([5.0, float({0}), 10.0, 0.0])'])
def test_floating_nan(assert_errors, parse_ast_tree, prefix, nan_variant, code, default_options):
    prefix = 'b'
    nan_variant = 'NaN'
    code = 'float({0})'
    assert_errors = assert_errors()
