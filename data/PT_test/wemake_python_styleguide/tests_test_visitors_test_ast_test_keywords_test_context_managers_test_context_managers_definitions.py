...
@pytest.mark.parametrize('code', ['xy[0]', 'xy.attr', 'xy["key"]', '(valid, invalid.attr)', '(invalid.attr, valid)'])
def test_context_manager_wrong_definitions(assert_errors, parse_ast_tree, code, default_options, mode):
    code = 'xy[0]'
...
@pytest.mark.parametrize('code', ['xy', '(valid1, valid2)', '(valid, *star)', '(first, second, *star)'])
def test_context_manager_correct_definitions(assert_errors, parse_ast_tree, code, default_options, mode):
    code = 'xy'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
