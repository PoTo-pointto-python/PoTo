import pytest
from wemake_python_styleguide.violations.consistency import MultilineLoopViolation
...
@pytest.mark.parametrize('code', [incorrect_loop1, incorrect_loop2, incorrect_loop3])
def test_incorrect_multiline_loops(assert_errors, parse_ast_tree, code, default_options, mode):
    code = incorrect_loop1
...
@pytest.mark.parametrize('code', [correct_loop1, correct_loop2, correct_loop3])
def test_correct_multiline_loops(assert_errors, parse_ast_tree, code, default_options, mode):
    code = correct_loop1
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
