...
@pytest.mark.parametrize('code', [wrong_multiline_return1, wrong_multiline_return2, wrong_multiline_return3])
def test_wrong_return_annotation(assert_errors, parse_ast_tree, code, default_options, mode):
    code = wrong_multiline_return1
...
@pytest.mark.parametrize('code', [correct_function_without_annotations, correct_simple_return, correct_compound_return, correct_multiline_return])
def test_correct_return_annotation(assert_errors, parse_ast_tree, code, default_options, mode):
    code = correct_function_without_annotations
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    default_options = default_options()
    mode = mode()
