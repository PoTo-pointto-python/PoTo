...
wrong_comparators = [('some', '[]'), ('some', '{}'), ('some', '()'), ('[]', 'some'), ('{}', 'some'), ('()', 'some')]
if PY38:
    wrong_comparators.extend([('some', '(x := [])'), ('(x := [])', 'some')])
correct_walrus = pytest.param(['(x := [1, 2])', 'some'], marks=pytest.mark.skipif(not PY38, reason='walrus appeared in 3.8'))

@pytest.mark.parametrize('comparators', wrong_comparators)
def test_falsy_constant(assert_errors, parse_ast_tree, comparators, eq_conditions, default_options):
...
@pytest.mark.parametrize('comparators', wrong_comparators)
def test_falsy_constant_is(assert_errors, parse_ast_tree, comparators, is_conditions, default_options):
    comparators = wrong_comparators[0]
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    ...
    visitor = WrongConstantCompareVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [FalsyConstantCompareViolation, WrongIsCompareViolation])

@pytest.mark.parametrize('comparators', wrong_comparators)
def test_falsy_constant_not_eq(assert_errors, parse_ast_tree, comparators, other_conditions, default_options):
    comparators = wrong_comparators[0]
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
...
@pytest.mark.filterwarnings('ignore::SyntaxWarning')
@pytest.mark.parametrize('comparators', [('some', '[1, 2]'), ('some', '{1, 2}'), ('some', '{"1": 2}'), ('some', '(1, 2)'), ('some', 'None'), ('some', 'False'), ('some', 'True'), ('some', '0'), ('some', '1'), ('some', '""'), ('some', '"a"'), ('some', 'b"bytes"'), ('some', 'other'), ('some', 'other()'), ('some', 'other.attr'), ('some', 'other.method()'), ('some', 'other[0]'), ('None', 'some'), correct_walrus])
def test_correct_constant_compare(assert_errors, parse_ast_tree, comparators, simple_conditions, default_options):
    comparators = ('some', '[1, 2]')
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
