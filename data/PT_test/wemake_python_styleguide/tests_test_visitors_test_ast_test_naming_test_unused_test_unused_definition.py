...
import pytest
from wemake_python_styleguide.violations.naming import UnusedVariableIsDefinedViolation
from wemake_python_styleguide.visitors.ast.naming import UnusedVariableDefinitionVisitor
module_context = '{0}'
function_context = '\ndef function():\n{0}\n'
method_context = '\nclass Test(object):\n    def function():\n    {0}\n'

@pytest.mark.parametrize(('context', 'indentation'), [(function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['(_first, _second)'])
def test_unused_variable_tuple_definition(assert_errors, parse_ast_tree, context, indentation, bad_name, forbidden_tuple_unused_template, default_options, mode):
    (context, indentation) = (function_context, 4)
    bad_name = '(_first, _second)'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    forbidden_tuple_unused_template = forbidden_tuple_unused_template()
    default_options = default_options()
    mode = mode()
    'Testing tuples with all unused variables cannot be defined.'
    tree = parse_ast_tree(mode(context.format(indent(forbidden_tuple_unused_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [UnusedVariableIsDefinedViolation])

@pytest.mark.parametrize(('context', 'indentation'), [(module_context, 0), (function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['(first, second)', '(first, _second)', '(_first, second)'])
def test_used_variable_tuple_definition_allowed(assert_errors, parse_ast_tree, context, indentation, bad_name, forbidden_tuple_unused_template, default_options, mode):
    (context, indentation) = (module_context, 0)
    bad_name = '(first, second)'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    forbidden_tuple_unused_template = forbidden_tuple_unused_template()
    default_options = default_options()
...
@pytest.mark.parametrize(('context', 'indentation'), [(module_context, 0), (function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['_', '__'])
def test_raw_unused_variable_definition(assert_errors, parse_ast_tree, context, indentation, bad_name, forbidden_raw_unused_template, default_options, mode):
    (context, indentation) = (module_context, 0)
    bad_name = '_'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    forbidden_raw_unused_template = forbidden_raw_unused_template()
    default_options = default_options()
    mode = mode()
    'Testing raw variable definition is forbidden in some cases.'
    tree = parse_ast_tree(mode(context.format(indent(forbidden_raw_unused_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [UnusedVariableIsDefinedViolation])
...
@pytest.mark.parametrize(('context', 'indentation'), [(module_context, 0), (function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['_', '__'])
def test_raw_unused_variable_definition_allowed(assert_errors, parse_ast_tree, context, indentation, bad_name, allowed_raw_unused_template, default_options, mode):
    (context, indentation) = (module_context, 0)
    bad_name = '_'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    allowed_raw_unused_template = allowed_raw_unused_template()
    default_options = default_options()
    mode = mode()
    'Testing raw variable definition is allowed in some cases.'
    tree = parse_ast_tree(mode(context.format(indent(allowed_raw_unused_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])

@pytest.mark.parametrize(('context', 'indentation'), [(function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['_protected'])
def test_protected_unused_variable_definition(assert_errors, parse_ast_tree, context, indentation, bad_name, forbidden_protected_unused_template, default_options, mode):
    (context, indentation) = (function_context, 4)
    bad_name = '_protected'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    forbidden_protected_unused_template = forbidden_protected_unused_template()
    default_options = default_options()
    mode = mode()
    'Testing protected variable definition is forbidden in certain cases.'
    tree = parse_ast_tree(mode(context.format(indent(forbidden_protected_unused_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [UnusedVariableIsDefinedViolation])
...
@pytest.mark.parametrize(('context', 'indentation'), [(module_context, 0), (function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['_protected'])
def test_protected_unused_var_definition_allowed(assert_errors, parse_ast_tree, context, indentation, bad_name, allowed_protected_unused_template, default_options, mode):
    (context, indentation) = (module_context, 0)
    bad_name = '_protected'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    allowed_protected_unused_template = allowed_protected_unused_template()
    default_options = default_options()
    mode = mode()
    'Testing protected variable definition is allowed in certain cases.'
    tree = parse_ast_tree(mode(context.format(indent(allowed_protected_unused_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])

@pytest.mark.parametrize(('context', 'indentation'), [(module_context, 0), (function_context, 4), (method_context, 8)])
@pytest.mark.parametrize('bad_name', ['regular', 'list_'])
def test_used_variable_definition_allowed(assert_errors, parse_ast_tree, context, indentation, bad_name, naming_template, default_options, mode):
    (context, indentation) = (module_context, 0)
    bad_name = 'regular'
    assert_errors = assert_errors()
    parse_ast_tree = parse_ast_tree()
    ...
    tree = parse_ast_tree(mode(context.format(indent(naming_template.format(bad_name), ' ' * indentation))))
    visitor = UnusedVariableDefinitionVisitor(default_options, tree=tree)
    visitor.run()
    assert_errors(visitor, [])
