...
from wemake_python_styleguide.logic.naming.alphabet import get_unreadable_characters
...
@pytest.mark.parametrize('expression', ['My1Item', 'Element0Operation', 'O0S', 'S0O'])
def test_unreadable_name(assert_errors, assert_error_text, parse_ast_tree, code, expression, default_options, mode):
    code = class_template
    expression = 'My1Item'
    ...
    visitor.run()
    unreadable = get_unreadable_characters(expression, constants.UNREADABLE_CHARACTER_COMBINATIONS)
